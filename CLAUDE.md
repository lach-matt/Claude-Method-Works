...
duplicate claim is corroborated rather than assumed. They still cannot be *seated*: that needs a
`MANIFEST.tsv` row, the manifest is generated by `drive_sync.py`, and its only path is an OAuth flow
for which **there is deliberately no token — the Colab mount is the route**, and a mount cannot see a
same-titled duplicate. See `docs/DRIVE-SYNC.md`.
