...
the salt block as fallback. See `docs/HELIOS3.md`. **The register is then walked row by row with the author** (R-01 to R-10 adopted 2026-09-11, each with the author's design written into the row: system geometry for attrition, the compound quartz aperture with its cooling lattice, CO₂ kept as the fluid, the alloy ladder, cold-shell silos not buried). **R-11, receiver scale, is flagged for simulation and `tools/receiver.py` is the first**: the curtain is a per-metre machine so scale is bought in width and aperture count; the loss fraction at fixed flux does not change with size while the edge losses shrink as perimeter/area; the R-02 dome **loses on the model and pays on the measured record**, both printed; and the 100 MWe first module is already **0.55** of a fleet tower, so a **pilot aperture at ~30 MW_th (a 10 m curtain)** belongs before it. **It runs under the author's standing rule — simulate critical, not optimal**: every banded constant carries a nominal and a critical value, every result prints both, the design is held to critical, and the critical open receiver (**0.795** against the chain's 0.90) is handed upstream as a field factor of **1.13** rather than adopted silently. The grade stays MAJOR / HOURS. See `docs/RECEIVER.md`. **The chain and the register then run at critical too** (`cspchain.py --critical` case, `helios3.py priced("critical")`): every band at its adverse end and the receiver at 0.795, Helios-3 mitigated is **$191/MWh** against $125 at mid — **$1,153 per household against $1,177 today** — the mirror **1.48×** larger. Mid stays the default and the critical column is printed beside it, never in its place. **R-12 provenance is then listed rather than asserted in `tools/studies.py`**: twelve technologies, **48** study rows each with a closed-set status and a source (complete over technologies, a floor over studies, not queried live), one recommendation per technology on the ladder's rungs — **7.5 years** of study on the critical path, run against the build — and the cost of delay at the 3 % construction escalation, **$0.97 B a year at mid and $1.54 B at critical**. See `docs/STUDIES.md`. See `docs/HELIOS.md`, `docs/HELIOCOST.md`, `docs/FIRMPOWER.md` and
`proposals/README.md`.
