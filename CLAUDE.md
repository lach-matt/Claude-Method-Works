...
witness, not the read path. 502 files, ~509 MB. `drive/MANIFEST.tsv` inventories 444 of them
(repo path, Drive id, title, mime type, size, modified time, md5, status); 31 rows are
`not-transferred` and live only in Drive. `drive/PENDING.tsv` lists 323 files still outstanding and
why. **The manifest is behind the tree**: about 86 mirrored CORPUS files have no manifest row yet.
It is regenerated by `tools/drive_sync.py`, never by hand.
