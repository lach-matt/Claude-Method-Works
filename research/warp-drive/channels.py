#!/usr/bin/env python3
r"""
channels.py -- THE CHANNEL INDEX: the corpus's largest banked measurement set,
and a data fault in its most authoritative rows.

    THIS FILE WAS COMMITTED AS `spectra.py` ON 2026-09-14 AND OVERWROTE ONE.
    `spectra.py` already existed -- "information as the SPECTRUM of a charge
    state, not the charge", 435 lines, answering a prediction of M's, cited by
    `obstruct.py`'s SPECTRAL-CURRENCY row and by five findings in `index3.py`.
    Writing this file to that name destroyed it, and neither the commit nor the
    verification noticed, because a new file and a clobbered file look identical
    to `--selftest`.  The original is restored at `spectra.py`; this is the
    channel index under a name that was free.  See DOCKET 14.
...
built in the same fashion and criterion as the rest."
    ...
    python3 channels.py             the reading
    python3 channels.py --selftest  fixtures
