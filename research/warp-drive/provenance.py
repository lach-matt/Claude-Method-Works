...
SWEPT = "READ-BY-SWEEP"         # read from source by a subagent this session,
                                # with page-level quotes, but not by this thread
...
SOURCES = [
    ...
    ("ks20", "E.-A. Kontou and K. Sanders, Class. Quantum Grav. 37 (2020) "
     "193001, arXiv:2003.01815",
     "Proposition 2.1 -- a state-independent lower bound collapses to zero "
     "whenever the quantifier set is invariant under positive rescaling; the "
     "normalised-timelike remark that it then does not; and eq. (86), a "
     "CLASSICAL smeared inequality with a negative right-hand side", VERIFIED),
    ("tipler78", "F. J. Tipler, Phys. Rev. D 17 (1978) 2521-2528",
     "the original classical argument that a negative bound on the WEC "
     "collapses to zero; reported verbatim by ks20, paywalled", NAMED),
    ("curiel14", "E. Curiel, arXiv:1405.0403; Einstein Stud. 13 (2017) 43",
     "the geometric / physical / effective formulation grid crossed with "
     "pointilliste / impressionist -- a two-axis taxonomy, claimed as his own "
     "classification at his footnote 9", SWEPT),
    ("clm14", "S. Capozziello, F. S. N. Lobo and J. P. Mimoso, Phys. Rev. D 91 "
     "(2015) 124019, arXiv:1407.7293",
     "a subsection titled 'The classification of energy conditions': the "
     "conditions are generated by contracting with the Ricci, Einstein and "
     "energy-momentum tensors, attributed to Hawking & Ellis 1973", SWEPT),
    ("vb00", "M. Visser and C. Barcelo, arXiv:gr-qc/0001099",
     "the effective stress-energy tensor as the object the energy conditions "
     "should be imposed on", SWEPT),
    ("mmv17", "P. Martin-Moruno and M. Visser, arXiv:1702.05915",
     "classical and semi-classical energy conditions -- the regime axis, in a "
     "title", SWEPT),
    ("fo08", "C. J. Fewster and L. W. Osterbrink, J. Phys. A41 (2008) 025402",
     "the classical averaged bound with a negative state-dependent RHS for the "
     "non-minimally coupled scalar; read through ks20 eq. (86)", NAMED),
...
ATTRIB = [
    ...
    ("H94a  the five-slot decomposition of the family", PRIOR, "ks20",
     "four of the five are explicit organising dimensions in ks20 alone and "
     "the fifth (T_eff) is vb00 in 2000.  Four skeptics were told to refute "
     "novelty and all four did, at high confidence.  NOT OURS"),
    ("H94a  'a bound below zero requires a quantum regime'", PRIOR, "ks20",
     "REVOKED AT H98.  Its forward half is tipler78 and ks20 Proposition 2.1, "
     "and the mechanism is SCALE INVARIANCE of the quantifier set, not the "
     "regime.  Its converse is FALSE: ks20 eq. (86) is a classical smeared "
     "inequality with a negative bound"),
    ("H98  the corrected family: a state-dependent bound needs a state",
     FOLK, "ks20", "a near-tautology once eq. (86) has removed the regime "
     "reading -- an entropy variation is an entanglement entropy's second "
     "variation and a classical configuration has none.  Claimed as neither "
     "new nor ours"),
    ("H98  a closure can certify a family that excludes published physics",
     OURS, "-", "the 192 and the 240 both close all five languages at E = 0 "
     "and only one is right.  A statement about the cypher rather than about "
     "gravity, and no source found makes it"),
...
def selftest():
    ...
    print()
    chk("sources", len(SOURCES), 26)
    chk("read from source this session", sum(1 for s in SOURCES if s[3] == VERIFIED), 15)
    chk("named but not read", sum(1 for s in SOURCES if s[3] == NAMED), 7)
    chk("read by a subagent, not by this thread",
        sum(1 for s in SOURCES if s[3] == SWEPT), 4)
    chk("attribution rows", len(ATTRIB), 33)
    chk("prior art", sum(1 for a in ATTRIB if a[1] == PRIOR), 12)
    chk("ours as a connection", sum(1 for a in ATTRIB if a[1] == COMBO), 9)
    chk("ours outright", sum(1 for a in ATTRIB if a[1] == OURS), 2)
    chk("elementary", sum(1 for a in ATTRIB if a[1] == FOLK), 10)
    ...
    chk("Kontou-Sanders was read by this thread", status_of("ks20"), VERIFIED)
    chk("Tipler 1978 was NOT read -- paywalled", status_of("tipler78"), NAMED)
    chk("Curiel was read by the sweep only", status_of("curiel14"), SWEPT)
