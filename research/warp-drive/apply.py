...
def dec_cases_with_a_seat_and_lead():
    """Under the DEC, which case admits one?  None."""
    return [c for c, dec, verdict, _w in DEC_BRANCH
            if dec and verdict not in ("FORBIDDEN", "MINKOWSKI", "COLLAPSE")]


def dichotomy_is_exhaustive():
    """Every DEC-respecting case is now blocked by a named theorem."""
    return dec_cases_with_a_seat_and_lead() == []
...
def selftest():
    ...
    print("\n2. WHICH MAKES THE DICHOTOMY EXHAUSTIVE, NOT ENUMERATED")
    for case, dec, verdict, why in DEC_BRANCH:
        print("     %-12s DEC %-5s %-12s %s"
              % (case, "holds" if dec else "FAILS", verdict, why))
    chk("DEC-respecting cases admitting a seat-and-lead",
        dec_cases_with_a_seat_and_lead(), [])
    chk("so the dichotomy is exhaustive over the DEC branch",
        dichotomy_is_exhaustive(), True)
    import dichotomy
...
def report():
    ...
    print("  %-30s %s" % ("the DEC branch", "exhausted"))
  ...
  theorems and the scope decision is still M's and still unchosen.  NOT
