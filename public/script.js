/* The Method Research — a zoomable reading of the index.
 *
...
(() => {
  ...
  function pathText(node) {
    return pathOf(node).map((n) => n.kind === 'root' ? 'The Method Research' : n.kind === 'channel' ? `ℓ=${n.l}` : n.kind === 'cell' ? `2S+1=${n.mult}` : label(n)).join(' › ');
  }
  ...
  function citation(node) {
    ...
    if (isParticleNode(node)) {
      ...
      if (!px) return `${pathText(node)}. The Method Research.`;
      const src = px.family === 'pdg' ? `${ps.citation || ''}${ps.doi ? ', DOI ' + ps.doi : ''}` : (px.source.text || '');
      return `${pathText(node)}. The Method Research, particle indexes, read at build from ${(tree.instruments || []).join(', ')}${tree.commit ? ' at ' + String(tree.commit).slice(0, 12) : ''} and written by tools/webindex.py; commit ${m.commit || '?'}, built ${m.built || '?'}. Source: ${src}. ${location.origin && location.origin !== 'null' ? location.origin : ''}${location.pathname}${hashOf(node)}`;
    }
    const src = (state.index.sources || []).map((s) => `${s.file.split('/').pop()} ${s.md5_measured ? s.md5_measured.slice(0, 8) : '?'}`).join(', ');
    return `${pathText(node)}. The Method Research, read by tools/populate.py and written by tools/webindex.py; commit ${m.commit || '?'}, built ${m.built || '?'}. Sources: ${src}. ${location.origin && location.origin !== 'null' ? location.origin : ''}${location.pathname}${hashOf(node)}`;
  }
...
const WALK_FIELD_LABEL = { hf: 'Hartree–Fock, non-local exchange (the paper\'s field, rebuilt)', lx: 'local exchange (Hartree–Fock–Slater)' };
  ...
  function paperCite(pp) {
    ...
    return `${c.author || 'Lach, M.'} (${c.year || ''}). ${pp.title}. In ${c.title || 'The Method Research'}, edition ${c.commit || '?'}. ${c.url || ''}#paper=${pp.slug}`;
  }
  ...
  function setupEdition() {
    ...
    const foot = `<span><b>The Method Research</b> — every element on every axis of every index.</span>
...
<El> is a symbol, a Z or a name; the element is loaded if it is not yet. An unknown input prints this text.`;
  ...
  function claudePrompt(question) {
    ...
    return `You are answering a question about ONE record of The Method Research (read by tools/populate.py and serialised by tools/webindex.py). Strict rules:
...
QUESTION: ${question}`;
  ...
  function askSystem() {
    return `You answer questions about chemistry and physics for readers of The Method Research, a public research site whose data you are handed below as cited lines. Method:
...
(function () {
'use strict';
/* solvers.js -- the six browser-side solvers of The Method Research.
 *
