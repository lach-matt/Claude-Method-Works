...
const WALK_FIELD_LABEL = { hf: 'Hartree–Fock, non-local exchange (the paper\'s field, rebuilt)', lx: 'local exchange (Hartree–Fock–Slater)' };
  ...
  function renderPapers(slug) {
    ...
    ensurePapers().then((papers) => {
      ...
      if (!slug) {
        host.innerHTML = `<p class="note">The papers the author has released to this site, as written. Each is rendered at build from its own text; its figures travel with the md5 the repository's ledger records. Ten are carried as the PDFs the author released, mirrored from the folder that is the record's original-input witness: their title, standfirst, byline, abstract and section headings are read from the PDF at build, and the PDF itself is the paper. ${sum.papers ? badge('READ', 'the paper\'s own text; nothing in it is edited for the site') : ''}</p>
          <div class="paper-list">${papers.map((pp) => `<div class="paper-card${pp.held ? '' : ' is-slot'}">
            ...
            <p class="note">${esc(pp.author || '')}${pp.held ? ` · ${pp.kind === 'pdf' ? `${pp.pages} pages · ` : ''}${(pp.words || 0).toLocaleString()} words · ${pp.kind === 'pdf' ? 'a PDF' : `${(pp.figures || []).length} figure${(pp.figures || []).length === 1 ? '' : 's'}`} · ${(pp.arxiv || []).length + (pp.doi || []).length} linked identifiers · md5 <span class="mono">${esc((pp.md5 || '').slice(0, 12))}</span>${pp.md5_recorded ? (pp.md5 === pp.md5_recorded ? (pp.kind === 'pdf' ? ' <span class="ok">matches the mirror</span>' : ' <span class="ok">matches the store</span>') : ' <span class="bad">DRIFT from the store</span>') : pp.tree ? ` · from the research tree at <span class="mono">${esc(pp.tree.commit || '?')}</span>` : ''}${pp.masked && pp.masked.length ? ` · <b>${pp.masked.reduce((n, m) => n + m.count, 0)} citations of unpublished material masked</b> at build, each with a visible mark` : ''}${pp.pdf_note ? ' · PDF withheld' : ''}` : ` · <b>not yet held</b> — ${esc(pp.note || '')}`}</p>
      ...
      host.innerHTML = `<div class="paper-head"><button type="button" class="ghost" data-act="papers-back">← all papers</button>
          <span class="note">${esc(pp.author || '')} · md5 <span class="mono">${esc((pp.md5 || '').slice(0, 12))}</span> ${pp.kind === 'pdf' ? badge('READ', 'the paper\'s own PDF, as released; its abstract and headings read from it at build') : badge('READ', 'the paper\'s own text, rendered at build')}</span></div>
        <div class="paper-layout">
          <nav class="paper-toc" aria-label="Contents">${pp.kind === 'pdf' ? `<div class="note">${pp.subtitle ? esc(pp.subtitle) + ' · ' : ''}${pp.pages} pages${pp.mirror ? `; mirrored from the witness folder, manifest ${esc(pp.mirror.status || '')}` : ''}</div>` : ''}${pp.headings.filter((h) => h.level >= 2 && h.level <= 3).map((h) => pp.kind === 'pdf' ? `<span class="toc-${h.level} toc-plain">${esc(h.text)}</span>` : `<a href="#${esc(h.id)}" data-h="${esc(h.id)}" class="toc-${h.level}">${esc(h.text)}</a>`).join('')}
          ...
          <article class="paper" id="paper-article">${pp.html}${pp.kind === 'pdf' && pp.pdf ? `<object class="paper-pdf" data="${DATA}${esc(pp.pdf.file)}" type="application/pdf" aria-label="${esc(pp.title)}"><p class="note">The PDF does not display inline here. <a href="${DATA}${esc(pp.pdf.file)}" target="_blank" rel="noopener">Open ${esc(pp.title)} (PDF, ${Math.round(pp.pdf.bytes / 1024).toLocaleString()} KB, md5 ${esc(pp.pdf.md5.slice(0, 12))})</a>.</p></object>` : ''}</article>
        </div>`;
