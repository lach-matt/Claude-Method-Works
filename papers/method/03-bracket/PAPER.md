...
*Table 6. The deficit of each threshold against the Coulomb value, and the leading relativistic term, for the thresholds the data set holds. Li III is held twice, as the database's published ionization energy and as a limit fitted to the series of theoretical levels (Yerokhin and Shabaev 2015; CITED); the two differ by 1.28 cm⁻¹, 3.5 times the fit's stated uncertainty, and the ratio moves from 0.865 to 0.876 between them. The paper takes the published ionisation energy as Li III's threshold, as it takes the database's published limit for every other species, and prints the fitted limit as the sensitivity of the ratio to that choice. (E9.) MEASURED, on four thresholds.*
