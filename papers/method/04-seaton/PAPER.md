...
SAMPLED is not among them, and deliberately: no claim in this paper rests on a pseudorandom sweep. The one randomised step anywhere in the verification is a guard on the machine check (§6), not a result.
...
**D4 (the dipole polarisation model).** Outside the core the running electron moves in the Coulomb field of charge z and, to leading order in the core's static dipole polarisability `α_d`, in the additional potential `V_pol = −α_d / (2r⁴)` (atomic units, with `α_d` in units of `a₀³`). The model treats `V_pol` in first-order perturbation theory on the hydrogenic state (n, ℓ) of charge z.
...
**Notation.** `K(ℓ) := ℓ(ℓ+1)(2ℓ−1)(2ℓ+1)(2ℓ+3)`. Atomic units — ħ = m_e = e = 4πε₀ = 1, lengths in Bohr radii `a₀`, energies in hartrees — are used in §2; the data of §3 are in cm⁻¹.
...
**Provenance.** Every level is taken from the NIST Atomic Spectra Database (Kramida, Ralchenko, Reader and the NIST ASD Team, version 5.12), retrieved in August 2026, CITED. The ionisation limits are the tabulated limits of the parent ions (Cd II 5s ²S₁/₂, In II 5s² ¹S₀, Rb II ¹S₀, Sr III ¹S₀); none is fitted from its own series. The atomic masses are the tabulated atomic weights (Cd, In, Rb) and the ⁸⁸Sr isotope mass. The series and their members are those the tables resolve without a perturber crossing the fitted range; §5 names the one series whose residual says otherwise. Seventy-nine of the eighty levels of Table 2 were confirmed level by level against a second, independent retrieval of the same tables; the eightieth, Cd I 5s5p ¹P°₁ at 43692.384 cm⁻¹, lies below the lowest member that retrieval carries and was taken from the tables directly.
...
- *The limit.* A shift ΔI in the ionisation limit changes δ_n by `n*³ ΔI / (2z²R_M)`, a term growing as n³ that the two-term Ritz fit absorbs into both coefficients. The estimate is exact to first order and holds on all eighty levels of Table 2 to better than one part in 10⁵ at ΔI = 0.01 cm⁻¹. The limits of Table 1 are tabulated, not fitted, and the fit residuals of the three p = 0 series are 0.0002 to 0.0003 in δ, so no such term is present at the level the fit can see.
- *Perturbers.* A level of another channel crossing the series distorts δ(n) locally. In I nd is the one series in Table 3 whose residual, 0.01755, is an order of magnitude above the next largest, 0.00141, and its defect rises monotonically across its six members from 2.18 at n = 5 to 2.31 at n = 10; its ρ = 0.219 is in the penetrating class and does not enter the non-penetrating figure.
- *Fields.* A residual growing as n*⁷ is the signature of a magnetic field, which needs tens of tesla to matter, and one growing as n*¹⁰ of an electric field, which begins above n of order 60. The highest member here is n = 12, and neither can fire on this sample.
...
|---|---|---|---|---|
| Lemma 1, ⟨r⁻⁴⟩ closed form | — | **435 states**, n ≤ 30, 1 ≤ ℓ ≤ n−1; the control ⟨r⁻²⟩ = 1/(n³(ℓ+½)) on the same 435 | — | closed form for general n CITED |
| Lemma 2, the exact energy–defect identity | ✓ | 64-point grid above degree (2, 2, 2) | — | — |
| Lemma 3, the Ritz denominator against 1/n² | ✓ | 60-point grid above degree (2, 3, 1) | — | — |
| **Theorem 1**, c₂/c₀ = −ℓ(ℓ+1)/3 | ✓ | **2,280** exact (n, ℓ, α_d, z) points | **✓ 8 obligations**, ℓ ∈ {1..8}, α_d and z real | — |
| Theorem 1's prefactor, 6/K(ℓ) = (3/4)/[(ℓ−½)ℓ(ℓ+½)(ℓ+1)(ℓ+3/2)] | ✓ | 200 values of ℓ, exact; ℓ⁵·6/K(ℓ) rises to 0.740718 below 3/4 at ℓ = 200 | — | — |
...
| §5, cores that would separate p = 0 from ℓ ≥ 3 | — | an argon-like core at ℓ = 2 against the krypton-like core here; 20 cores of 60 to 79 electrons with p = 1 at ℓ = 3 | — | — |
| Table 1, the thirteen channels | — | **13 series, 80 levels** loaded and classified | — | limits, masses and terms CITED |
| Table 2, the levels | — | **79 of 80** confirmed against a second retrieval of the same tables | — | CITED, NIST ASD 5.12 |
| Table 3, the fits | — | **13 certified minimisers**, one local minimum each on its scan | — | MEASURED |
| Proposition 1, the class statistics | — | class sizes 3 and 10; ρ defined on 3 and on 6 | — | MEASURED |
| Proposition 2, the sign rule | — | 13 series, six values of p; the positive fraction non-decreasing in p | — | MEASURED |
| §5, the limit sensitivity n*³ΔI/(2z²R_M) | — | all 80 levels at ΔI = 0.01 cm⁻¹, worst relative error 5.5 × 10⁻⁶ | — | MEASURED |
| §5, In I nd as the one perturbed series | — | 13 residuals compared; its defect monotone over its six members | — | MEASURED |

**The exhausted families, named.** 435 states: every hydrogenic (n, ℓ) with 2 ≤ n ≤ 30 and 1 ≤ ℓ ≤ n − 1. 2,280 points: every (n, ℓ) with n ≤ 20, times three values of α_d and four of z. 200 values of ℓ: 1 to 200. 13 minimisers: every series of Table 1, each certified as in §4. 16 cells: the four cores at ℓ = 0, 1, 2, 3. 80 levels: every member of every series of Table 1.

**The obligation count.** Twenty-two obligations run and none fails: three PROVED as exact identities on a rational grid above their degree, one MACHINE-CHECKED in eight instances with its two guards, fifteen EXHAUSTIVE with the family named beside each, and three comparisons of the fitted coefficients and the class statistics against an independent implementation of the same fit. No claim in this paper is SAMPLED: there is no pseudorandom sweep anywhere in it, and the one randomised step — the 200 points of the encoding guard — is a guard and not a result. The self-test adds three negative controls, each of which must be reported as refuted: a wrong closed form for ⟨r⁻⁴⟩, which fails on every one of the 28 states it is tried on; the false claim c₂/c₀ = −2ℓ(ℓ+1)/3, which Z3 returns satisfiable with a model; and one p = 0 level displaced by 5 cm⁻¹, which moves the class median from 1.150 to 1.296.

**The two guards on the machine check.** *Non-vacuity*: the hypothesis α_d > 0 ∧ z ≠ 0 is satisfiable. *Encoding fidelity*: the Z3 terms for c₀ and c₂, evaluated at 200 random rational (α_d, z, ℓ), agree with an independent exact implementation in every case.

**What is not machine-checked, and why.** Propositions 1 and 2 are measurements: they are recomputed from the cited levels by the stated procedure and are not of a shape a solver decides. Lemma 1's closed form for general n is cited; the exact computation reaches n = 30. The least-squares certificate of §4 is a finite check — the minimiser beats every scanned point and both ends of its final bracket — and not a proof of global optimality over the real line; each scan showed a single basin. Every arithmetic step outside the one square root of D1 is exact rational arithmetic; that square root is taken to forty significant digits.

---

## References

- Bethe and Salpeter (1957). *Quantum Mechanics of One- and Two-Electron Atoms*, §3. — the closed form of the hydrogenic ⟨r^s⟩ for general n, cited in Lemma 1.
- Born and Heisenberg (1924). Über den Einfluss der Deformierbarkeit der Ionen auf optische und chemische Konstanten. *Zeitschrift für Physik* **23**, 388–410. — core polarisation as the origin of the high-ℓ quantum defect.
- Drake and Swainson (1991). *Physical Review A* **44**, 5448. — second-order polarisation corrections to the Rydberg energy, named in §5 as one of the terms outside Theorem 1.
- Hartree (1928). *Proceedings of the Cambridge Philosophical Society* **24**, 89–110. — the penetration argument by which the defect falls with ℓ.
- Kramida, A., Ralchenko, Yu., Reader, J. and the NIST ASD Team (2024). *NIST Atomic Spectra Database*, version 5.12. https://physics.nist.gov/asd, DOI 10.18434/T4W30F. — every level of Table 2 and every limit of Table 1.
- Mayer and Mayer (1933). The polarizabilities of ions from spectra. *Physical Review* **43**, 605–611. — the systematic extraction of ionic polarisabilities from Rydberg spectra.
- Ritz, W. (1903). The two-term quantum-defect expansion δ = δ₀ + δ₂/n². — the series form fitted in D2.
- Seaton (1958). The quantum defect method. *Monthly Notices of the Royal Astronomical Society* **118**, 504–518. — the quantum defect method, and the relation of Theorem 1 as the spectroscopic literature attributes it.
- Seaton (1983). Quantum defect theory. *Reports on Progress in Physics* **46**, 167–257. — the multichannel form of the same method.
- Theodosiou, Inokuti and Manson (1986). *Atomic Data and Nuclear Data Tables* **35**, 473–486. — asymptotic quantum defects of s, p, d and f orbitals for all ionisation stages of all ions with Z ≤ 50, the published survey against which a defect of the kind fitted here may be compared.
