...
**Table 3.** The fits. ρ is D6; rms is the root-mean-square residual of the fit in units of δ; for the p = 0 series, "α implied" is the dipole polarisability, in a₀³, that the fitted δ₀ implies through Theorem 1, δ₀K(ℓ)/(6z²).
...
**The obligation count.** Thirty-two obligations run and none fails: seven PROVED as exact identities on rational grids above their degree; eleven EXHAUSTIVE with the family named beside each; two MACHINE-CHECKED, in eight and seven instances, each with its two guards; eight MEASURED, recomputed from the cited levels by the stated procedure; one cross-check of the exact fit against an independent Gauss–Newton implementation; and three comparisons with figures previously computed for the same data (the thirteen coefficient pairs to four decimals, the four class statistics to three, and one mean defect), on which no claim of the paper rests. No claim in this paper is SAMPLED: the only pseudorandom step — the 200 points of each encoding guard, seed 5 — is a guard and not a result. The self-test adds four negative controls, each of which must be reported as refuted: a wrong closed form for ⟨r⁻⁴⟩, which fails on every one of the 28 states it is tried on; Lemma 2's rearranged factor without its factor 2, which fails at the first grid point; the false claim c₂/c₀ = −2ℓ(ℓ+1)/3, which Z3 returns satisfiable with a model; and one p = 0 level displaced by 5 cm⁻¹, which moves the class median from 1.150 to 1.296.
...
- Bethe and Salpeter (1957). *Quantum Mechanics of One- and Two-Electron Atoms*, §3. — the hydrogenic ⟨rˢ⟩ for general n, cited for ⟨r⁻²⟩ in Lemma 1.
...
- Kramida, A., Ralchenko, Yu., Reader, J. and the NIST ASD Team (2024). *NIST Atomic Spectra Database*, version 5.12. https://physics.nist.gov/asd, DOI 10.18434/T4W30F [retrieved August 2026]. — every level of Table 2 and every limit of Table 1.
...
- Mitroy, J., Safronova, M. S. and Clark, C. W. (2010). Theory and applications of atomic and ionic polarizabilities. *Journal of Physics B* **43**, 202001. — the compilation against which the implied polarisabilities of Table 3 should be compared (§5.2; not done here).
...
- Seaton (1958). The quantum defect method. *Monthly Notices of the Royal Astronomical Society* **118**, 504–518. — the quantum defect method in whose terms δ(n) is used.
