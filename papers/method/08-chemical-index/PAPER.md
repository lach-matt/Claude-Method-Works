...
**D1 (chain, box, index).** A *chain* is a finite totally ordered set; its elements are written 0 < 1 < … < m − 1. Fix n ≥ 2 chains A₁, …, Aₙ; n is the number of coordinates (in §8 the letter n is also a principal quantum number, as in 2p or 4f, and the two uses never meet). A *box* is the product B = A₁ × ⋯ × Aₙ, and ∣B∣ = ∏ ∣Aᵢ∣ is its size. An *index* is a non-empty finite set X ⊆ B of *cells*. For a cell x write xᵢ for its i-th coordinate.
...
Lemma 4 is why the sweeps below halve one axis: reversing every axis maps closing orderings to closing orderings, so closing orderings come in pairs and it suffices to visit one of each. Unless a sweep is stated to be full, an "EXHAUSTIVE over N orderings" below visits N/2 orderings — those in which the last axis's first value precedes its last — and counts each for itself and for its reversal. The sweeps run in full, and the one place the halving is checked against a full sweep, are named in §10.
...
**Proposition 3 (what the closure consists of).** The valence seat occupies every one of the twelve cells of its 4 × 3 block, and the subvalence seat occupies exactly two cells, (size, A) and (count, A) (**EXHAUSTIVE**, 22 properties). Consequently Theorem 1 reduces to the position of those two cells: a full block is closed under every ordering (it is its own box), and beside a full block a pair of subvalence cells closes under some ordering if and only if the pair shares a kind or a dependency.
...
**EXHAUSTIVE** at every clause, the 288 orderings visited up to reversal as in Theorem 1 (clause 2's split of sixteen and four is taken from a full sweep); the negative control of clause 1 — the claim that the thirteen cells close — is reported as **REFUTED**, with the value E = 1 that refutes it.
...
**It does not route anything.** A classification of this kind invites a use: sending an unexplained residual of a fitted model to the property it is thought to track. Nothing of that kind is done here: no residual is defined in this paper, no variation is measured, and the index returns a cell, not an explanation.
...
| G7 | a list compiled outside this paper sends four residuals of a fitted model to a property of the closed seats, each with a class; the check verifies only that each named property lies in the closed seats and carries its listed class | 4 rows; retained in the checks, and no sentence of this paper rests on it |
...
**Which sweeps are halved.** Every sweep over orderings above visits half its family and counts each ordering for itself and its reversal (Lemma 4), except four that run in full: B3b, which fixes the seat axis and visits every ordering of the other two; F2b; H10; and J2, which runs the fourteen-cell sweep in full and finds the same minimum, family and closing count as the halved B2. B5 and C1 to C3c visit subsets, not orderings, and visit every one.
