...
Every number printed above is recomputed by the paper's check program, which imports the construction by path and never copies it; the reference implementations used by the guards are written independently inside the check program itself. Below, *family* names what an EXHAUSTIVE row visited and *domain* names what a MACHINE-CHECKED row ranged over.

| object | status | family or domain |
|---|---|---|
| Lemma 1, `ℛ(X)` is a sublattice | PROVED | — |
| Lemma 2, distributivity | PROVED | — |
| Lemma 3, a seed is a cover of steps and slots | PROVED; GUARD | 40 pseudorandom subsets of `Λ₈`, seed 11, both readings (fixed box; own box), plus a one-cell-short control |
| **Theorem 1**, one coordinate per bound | PROVED; MACHINE-CHECKED ×2 | the unbounded integers, 8 free variables; and every one of the `2⁹` subsets of a `3 × 3` box that is closed, against every monotone `h` into `{−1,…,2}` |
| **Theorem 2**, the monotone envelope | PROVED | — (not of a shape a solver decides; see below) |
| **Theorem 3**, the triangle | PROVED; MACHINE-CHECKED (join); REFUTATION (meet) | the unbounded integers, 6 free variables; two explicit witnesses re-verified by hand arithmetic |
| **Theorem 4**, the bands | PROVED; MACHINE-CHECKED (i), (ii); REFUTATION (iii) | the unbounded integers, with `k` itself free in (i) |
| **Theorem 5**, the dichotomy; Corollary 1 | PROVED; EXHAUSTIVE | the thirteen bounds of D10 |
| Prop. 1, projection | PROVED; EXHAUSTIVE | five projections, as set equalities |
| **Prop. 2**, every stage closes | PROVED; EXHAUSTIVE ×2 | staircase swept over 6,912 / 27,648 / 110,592 / 663,552 / 5,308,416 / 47,775,744 ambient cells; every unordered pair at the first four stages, 97,323,996 pairs |
| Prop. 3, fill decay | EXHAUSTIVE | six stages |
| Props. 4–6, the terms and their maxima | EXHAUSTIVE; CITED | the eight subshells the caps admit; the construction of the term multiset is CITED |
| **Table 2**, the envelope gap | EXHAUSTIVE | all cells of the stage below, at each of the six axes |
| Props. 7–9, the constraint graph | EXHAUSTIVE | six stages; 286 vertex triples for the triangle census; exact elimination search for treewidth |
| Prop. 10, exact factorisation | PROVED; EXHAUSTIVE | six stages for the sum, set identity at the first and thirteenth |
| Prop. 11, the profile and `⟨q⟩` | EXHAUSTIVE | six stages, four transfers each |
| Props. 12–13, the price of the second parent | EXHAUSTIVE; REFUTATION | 248,076,675 pairs of the exact-triangle twelfth stage; one exhibited failing meet |
| Prop. 14, orientation | PROVED; EXHAUSTIVE | all 8,018 cycles of length 3 to 8 of the complete graph on eight vertices |
| Props. 15–17, rank, generators, Birkhoff | EXHAUSTIVE | 976 cells × 17 generators; all down-sets of the 17-element poset |
| **Theorem 6**, 1,113,045,672 maximal chains | PROVED; EXHAUSTIVE | the recursion evaluated at all 976 cells; a negative control reported by the selftest |
| Props. 18–19, the bracket system | EXHAUSTIVE | 287,809 cells across the five upper stages; all 199,130 top-stage cells for the composition |
| **Theorem 7**, the seed 7, 8, 9 | EXHAUSTIVE; MACHINE-CHECKED | branch and bound over 264 / 442 / 688 maximal covering signatures of 77 / 105 / 138 steps; the solver refuting a 6- / 7- / 8-cover over every subset of the reduced family, 59 / 61 / 63 signatures against 27 / 28 / 29 kept steps; two independent staircases over the exhibited seed; the packing bound 6 / 7 / 8 and the greedy figure 7 / 9 / 9 by exact search and by the greedy rule |

**The two guards, stated.** No machine-checked row above is reported unless both pass, and the check program refuses to print any of them otherwise.
