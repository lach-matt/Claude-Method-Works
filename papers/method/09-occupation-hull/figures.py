...
def fig1(Z=57, N=8, lmax=4):
    ...
    for j, g0, g1 in seen:
        ...
        b.text((g0 + g1) / 2, 0.38, check.name(S[j]), ha="center", va="bottom", fontsize=9,
               color=TEXT if j == i else TEXT2, rotation=90)
    b.axvline(L, color=TEXT2, lw=1)
    b.axvline(U, color=TEXT2, lw=1)
    b.text(L, -0.45, check.closed(lo), ha="center", va="top", fontsize=9, color=TEXT2)
    b.text(U, -0.45, check.closed(hi), ha="center", va="top", fontsize=9, color=TEXT2)
...
def fig3(out):
    ...
    ax.set_title("the 106 corridors, the 19 distinct endpoints, and the band on which one slope "
                 "covers the most corridors (%d of 106)" % best, fontsize=9.5, color=TEXT, loc="left")
    style(ax)
    fig.tight_layout()
    ...
    for t in TR:
        if t["Z"] in real:
            ...
            ax.annotate(check.G.GROUND[t["Z"]][0], (t["Z"], t["a"]), xytext=(dx, dy),
                        textcoords="offset points", fontsize=9.5, color=ORANGE, ha=ha)
        elif t["Z"] in touch:
...
def main():
    ...
    fig1()
    fig2(out)          # figures/fig2-walk-and-resets.png
    fig3(out)          # figures/fig3-corridors.png
