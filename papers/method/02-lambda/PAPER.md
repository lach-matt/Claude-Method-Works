...
**What is not claimed.** Λ without caps is infinite, and every *count* above is a count at one cap setting, named wherever it appears. The structural statements divide into two kinds and the paper never merges them: Theorems 1 and 11 and Lemmas 1 and 6 are machine-checked over the integers and hold at every cap; Theorems 4, 5, 6, 7, 8, 15 and 16 are exhaustive computations at the stated caps and are not proved to persist. The seed of 7 is a measured minimum at one cap setting; the general problem is NP-complete (Karp 1972), and this paper makes no claim about its value elsewhere. The physics enters only in §1, in one paragraph, and only to say where the seven bounds come from; nothing after that paragraph uses it.

**One finding worth stating in front.** Exactly one cell lies in all 24,585 minimum generating sets, and yet **no covering obligation is met by a unique cell** — so the elementary rule that would force a set into every minimum cover (Chvátal 1979) does not apply anywhere in this instance. The cell is not forced locally; it is common to every minimum cover for a global reason the enumeration exhibits and this paper does not otherwise explain (§7).
...
and the **staircase closure** is ℛ(X) := { x ∈ B(X) : xᵢ ≤ φ̂ᵢⱼ^X(xⱼ) for all i ≠ j }. X is **closed** when ℛ(X) = X; the **closure defect** is E(X) := |ℛ(X)| − |X|.
...
*Proof.* Computation. Λ is built twice, once by the construction of D4 and once by sieving all 6,912 points of the box against the seven bounds, and the two agree cell for cell. The staircase closure of the result has 976 cells and is equal to Λ as a set, and ℛ(ℛ(Λ)) = Λ. ∎ **EXHAUSTIVE** (6,912 box points; the closure is computed over the same box).
...
*Proof.* In a chain, min(a, max(b, c)) = max(min(a, b), min(a, c)): the three orderings of a, b, c are checked directly, and there are no others. Hence the ambient box B, a product of chains with coordinatewise operations, is distributive. By Theorem 1, Λ is a sublattice of B and its ∨ and ∧ are the restrictions of B's, so the identity restricts to Λ. ∎ **PROVED**; the chain identity is **EXHAUSTIVE** over all 289 triples of values drawn from the eight alphabets, and a **SAMPLED** sweep of 200,000 cell triples (seed 20260921) found no failure.
...
*Proof.* For integers a, b, max(a, b) + min(a, b) = a + b — one of the two is the maximum and the other the minimum. Summing that identity over the eight coordinates and using that ∨ and ∧ are coordinatewise (Theorem 1) gives the displayed equality. For the grading: if y covers x in Λ then x < y, and any coordinate in which they differ can be lowered one step, so y − x is a unit vector and rank rises by 1; the extremes are computed. ∎ **PROVED**; **EXHAUSTIVE** on all 475,800 unordered pairs, zero violations, and on all 3,749 cover relations.
...
summing to 976, and it is log-concave at every interior rank — aᵣ² ≥ aᵣ₋₁aᵣ₊₁ — hence unimodal. Each rank level is an antichain, so the largest antichain is at least 122. For the upper bound, Dilworth's theorem (1950), CITED, says the largest antichain equals the minimum number of chains covering the poset, and by König's theorem the minimum chain cover of an N-element poset is N minus a maximum matching in the bipartite graph of its strict order. That matching, computed by the Hopcroft–Karp algorithm (1973), CITED, has 854 edges, so the minimum chain cover is 976 − 854 = 122 and no antichain exceeds it. ∎ **EXHAUSTIVE** (the exact matching over all 115,162 strict comparabilities) and **CITED** (Dilworth; König; Hopcroft–Karp).
...
*Proof.* Existence of the minimum: the set {x ∈ Λ : x_c ≥ v} is non-empty and closed under ∧ by Theorem 1, and a finite non-empty meet-closed set has a least element, namely the meet of all of them. That cell covers exactly one cell — lowering any coordinate that can be lowered either leaves the set or falls below v in c, and minimality leaves a single admissible step — so it is join-irreducible. Conversely, let j be join-irreducible, covering the single cell j⁻. They differ in one coordinate c by one step, so j = j(c, j_c); if some cell x ∈ Λ with x_c ≥ j_c failed to dominate j, then j ∧ x would lie in Λ, be strictly below j and have c-coordinate at least j_c, contradicting minimality. So the 17 cells j(c, v) are exactly the join-irreducibles, and the count is Σᵢ(|Aᵢ| − 1) by construction. The meet-irreducible count is computed and equals 17. ∎ **PROVED** for the form and the closed count; **EXHAUSTIVE** for the equality of the two counts (976 cells, both cover sets computed).
...
*Proof.* For a finite distributive lattice the order dimension equals the width of its poset of join-irreducibles (Dushnik and Miller 1941, with Dilworth 1950), CITED. That width is computed to be 7, certified both ways by the same matching argument as Theorem 5: a partition of the seventeen into seven chains, and seven pairwise incomparable letters. One letter per coordinate is incomparable to the rest except for g: every letter `g ≥ v` lies above the atom `q ≥ 1`, because g ≤ q, so g contributes no independent member to any antichain of letters. The order pays seven dimensions for eight axes. ∎ **CITED** and **EXHAUSTIVE** (the width of a 17-element poset, by exact matching).
...
Because G is a tree, it always has a leaf. Summing out a leaf coordinate touches only the single factor on its one edge, and produces a quantity that depends on the leaf's neighbour alone; deleting the leaf leaves a smaller tree, which again has a leaf. Iterating eight times consumes every coordinate, and at no step does any intermediate depend on two coordinates. Explicitly for Λ: eliminate 2S (a leaf at k), then n (a leaf at ℓ), then ℓ, then k, then q, then g, then f, and sum over e. The two pendant sums are intervals clipped by a bound,

> #{2S} = max(0, min(hi₈, k) − lo₈ + 1),  #{g} = max(0, min(hi₇, q, 4f + 2) − lo₇ + 1),

and the rest are single-argument sums. A tree has no cycle, so no configuration is counted twice and no correction term arises. ∎ **PROVED**, and **EXHAUSTIVE**: the elimination is compared with direct enumeration on **every one of the 1,944,000 coordinate boxes of the ambient box**, with zero disagreements.
...
**Corollary 3 (the three specialisations).** F(1) = 976; F(−1) = 2; F′(1)/F(1) = 10801/976 = 11.0666…, the mean rank.
...
*Proof.* The finite geometric series, for z ≠ 1. ∎ **PROVED**; the identity is verified in exact rational arithmetic at 60 points — six degrees k = 0..5 against ten distinct rationals — which exceeds the degree in the one variable, so a rational function agreeing there agrees identically. **EXHAUSTIVE.**
...
**Remark.** The practical content is that no recursion is needed. A 976-element lattice has 116,138 comparable pairs, and every Möbius value among them is read off a 17-element poset by one antichain test on at most 17 elements.
...
**Theorem 18 (seed(Λ) = 7).** At the caps of D3, seed(Λ) = 7. There are exactly **24,585** minimum seeds. Exactly **one** cell — (2,1,3,3,2,1,3,0), which is the generator `g ≥ 3` — lies in all of them, and **no** element of the 102 is witnessed by a unique cell.
