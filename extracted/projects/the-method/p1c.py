...
print("\n  --- IS 'LOCALLY CONCAVE AT p=2,3,5' THE CONDITION? ---")
def loc(v,idx): return all(v[i+1]-v[i] <= v[i]-v[i-1]+1e-12 for i in idx)
for idx in ([2],[3],[5],[2,3],[2,5],[3,5],[2,3,5],[1,2,3,4,5,6]):
    tp=sum(1 for v,c in feas if loc(v,idx)); fn=len(feas)-tp
    fp=sum(1 for v,c in infeas if loc(v,idx))
    print(f"   concave at {str(idx):<16} covers {tp:>5}/{len(feas)} feasible"
          f"   admits {fp:>6}/{len(infeas)} infeasible")
print("\n  clique among feasible AND locally concave at 2,3,5:",
      Counter(c for v,c in feas if loc(v,[2,3,5])))
