...
| `forward_markers.py` | — | appended the thirteen forward markers; editorial only |

## WRITTEN IN 1.7 — new work, not reconstruction

| file | what it measures | standing |
|---|---|---|
| `scorer.py` | the walk end to end | 99 in sample, 90 held out |
| `balance.py` | the running intersection | R 1401's fourteen forced moves, exactly |
| `a5c.py` | the cell as source·target·q | closes A5c |
| `null_madelung.py` | the Madelung null | 96 conditional, 94 free-running |
| `belokolos_spectrum.py` | his ordering against ours | 87 · 99 · 88 agreeing |
| `group_rule.py` | a on the binding ceiling | 88 on it, 93 just below |
| `proximity.py` | the trigger | zero threshold, 14 of 18, no false positives |
| `delta_grouping.py` | does the block separate δ | 87% within-ℓ, but not constant |
| `figures_xray.py` · `figures_walk.py` | the seven unresolved figures | six settled |
| `per_run.py` · `per_atom.py` · `transition.py` · `a_inward.py` | placement rules | all fail to beat 96 |

*Every one of these is new measurement rather than recovery. None is an original
of anything lost.*
...
| `xray_index.py` | 1378, 1379, **1619** | ✔ **RECOVERED.** Rebuilt from the specification alone at R 1619 and reproduces all three recorded numbers — 33 lines, 9 cells, E = 0 |
...
## A CLASS THIS RECORD CANNOT SEE — added 2026-08-11, R 1477

**This file classifies FILES. It cannot see a lost EDIT inside a file that
survived.** Register 1399 recorded a correction to `T.tower`'s figures and says
"Corrected at source". The correction was made in the 1.6.1 container and lost
with it — `mathreg.py` itself came across in the restore point, so nothing here
flagged it, and the object went on printing 70905, 199130 against a generator
computing 22275, 64290.

**Checked systematically:** of the six bridged registers claiming a source
change, five are present — 1383, 1384, 1386, 1388, 1433 — and only 1399's was
missing. It was the only one that changed a FIGURE rather than a FILE.

**Standing rule.** A register entry saying "corrected at source" is a CLAIM ABOUT
THE TREE and must be checked against the tree, not trusted. The forty-three
objects graded COMPUTED whose statements carry figures are the population to
check; T.tower was found by scanning all of them.
