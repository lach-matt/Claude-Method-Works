...
| `The_Method_1_6_BUILD231_compendia_papers_audits.md` | Live compendia bundle — 14,985,043 B · `399bc45eec94ced1d9fb47b561ddfe68` · 122,535 lines · 662 members |
| `members/` | All 664 members extracted from those two bundles, byte-exact. Instruments read these by name. |
...
| `bin/stage-gate` | Stages the tree at `/home/claude` so the path-bound instruments can run (`gate.py census` stays red by construction; the live census step is `gate_live.py --census`) |
...
**Where the store stands (5 September 2026, W-238).** The Register runs 1 to 1835; R3's Q5 is
...
`close_census2.py` (the derived census, its ids matched by content). Every tool the repository holds is a
member too, by M's ruling of 5 September (W-236): `tools/` and `bin/` keep byte-identical working copies, and
a change to one is a successor member. `WORKING-REGISTER.md` ends at W-238 and records every one of
...
Current state: `members checked: 664  mismatched: 0`, both bundles recovered, `VERIFY OK`.
...
content-keyed successors. The census is regenerated by `close_census2.py` running `census2.py` (W-237):
the fresh rows are matched onto the seated ids by content in four printed tiers, a row whose defect no
longer measures is never dropped by the tool — it is carried with a RETIRED mark only under a typed
`--retire`, a ruling — and `gate_live.py --census` asserts the seated census a fixed point. `census.py`
is a seated member that hard-codes `/home/claude/members/`
