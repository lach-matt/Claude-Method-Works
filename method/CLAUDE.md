...
- **The repository as store.** `method/` holds the two live bundles, `method/members/`
  holds all 422 members extracted from them byte-exact, and `method/MEMBER-INDEX.tsv` records
...
## 3. Current state (live files as of chat 151-R's second close, MEASURED in the repo)

- Live files, both in `method/` and asserted by `method/verify.py`: **BUILD90 main** (1,983,081 B · 49065309b0c4fe8e055f693aed295cca · 18,470 lines · 2 members, unchanged since chat 62) and **BUILD186 compendia** (6,590,914 B · 7144d18deb508e5f43852b74648885af · 74,800 lines · 420 members). 422 members in all. Register 1 to 1792. **W-195 seated**, 196 W entries.
- BUILD181 through BUILD184 were rebuilt here from BUILD180 plus the chat-151-B parts fetched from Drive, and every assertion in their four `REBUILD-BUILDNNN.md` documents held: **37 of 37 parts byte-exact**, every golden regenerated by *running* its instrument, each reverse guard recovering its predecessor. Derivations, not transcriptions. The documents are kept at `method/rebuild/`.
- **BUILD185 and BUILD186 were made here, not carried.** BUILD185 seats the Register read end to end — entries 1–1792, thirteen source-order units plus `r2-regsweep`, 57 new members — and M's compendia-scope ruling. BUILD186 seats **chat 152's six cypher members** (`cypher.py`, `audit_lambda.py`, `close_main.py`, `register_counts.py`, `CYPHER.md`, `AUDIT-LAMBDA.md`), from branch `claude/cypher-analysis-method-books-826x0x`.
- **Two numbering collisions, both RULED by M and closed.** (i) **There is no chat 153** — chat 152's own W entry heads itself *"W-190 — chat 153"* and names its block *DEF-153*; M corrected it, and the entry is carried into W-195 **verbatim and unedited**, with the correction made in the framing above it. (ii) **W-190 and BUILD181 were each spent twice**, by chat 151-B and chat 152, both descending from BUILD180 `ea5becc4…` — siblings, not a chain (`2fbcd461…` 345 members against `bfa0d975…` 347). **The Drive chain keeps the numbers**: 151-B's stand, chat 152's entry is re-keyed **W-195**, its BUILD181 is retired as a sibling in git history, and nothing of its content is lost.
- **PARKED, AND THE NEXT RULING M OWES: did BUILD91 cross the chat-67 hold?** Chat 152's `BUILD91` main bundle (`7ff88249adc3b00f95a30b97e879e500`) appends Register entries **1793–1796** and **repairs the front-matter counts**. The chat-67 hold forbids editing, corrections and Register entries until the review closes. **Until M rules, BUILD91 is not carried, the main bundle stays BUILD90, and reg1-04 is not touched.** If it is adopted: `r2-reg8a`, `r2-reg12` and `r2-regsweep` re-bank mechanically, but **`r2-reg1a` must be re-TAKEN** — its printed-value constants are BUILD90's.
- **W-194's "fork on the Register" is WITHDRAWN as wrong.** Measured over all 6,612 lines: BUILD91's Register is BUILD90's plus four entries and two count-site lines, nothing else in 1–1792 differing. Ten of the fourteen seated Register instruments reproduce against it unchanged; the four that differ do so only on extents or stale constants, with **no verdict changing**; `r3-em` is wholly unaffected (246 repairs, 29 declines, +360 B).
- **This tree is AHEAD of Drive, which stands at BUILD184.** Chat 152 is paused by M, so nothing is racing it, but Drive still has to be brought level and only a session reaching both can do it.
- Next work: **R2 owes the four remaining compendia by class sweep** (Mathematical, Physics, Index of Indices, Spectra) under M's ruling — the Register leg is discharged. **DEF-143 item 11's order is empty**; what it waits on is R3's rulings. Two R3 classes are specified, dry-run and held: `R3-CLASS-WL.md` (approved at chat 128) and `R3-CLASS-EM.md`. **R3-CLASS-EM must not be applied until a checker is seated into the gate** — entry 1617 made this repair once and it did not hold, because `register_review.py` never became a member (reg13-01). Register entry **1797** is staged and not seated, in `method/REGISTER-QUEUE-APPEND-cypher-audit.md`, with `tools/audit_math.py`.
...
1. **§0 gate**, each step its own call: confirm the checkout (`git -C <repo> rev-parse HEAD` and `git status --porcelain` — a dirty tree is reported before anything else); `python3 method/verify.py` (422 members and both bundle md5s — this replaces the Drive fetch, the spill decode and the extraction in one deterministic step); `gate.py census`; `gate.py run --core`; `gate.py manifest`; `gate.py run` on the previous chat's instruments; `gate.py cert NN`. Any FAIL stops the chat with a report. Nothing is fetched from Drive.
