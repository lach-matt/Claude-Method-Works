...
- **The repository as store.** `method/` holds the two live bundles, `method/members/`
  holds all 351 members extracted from them byte-exact, and `method/MEMBER-INDEX.tsv` records
...
## 3. Current state (live files as of chat 151-B's second close, MEASURED in the repo)

- Live files, both in `method/` and asserted by `method/verify.py`: **BUILD90 main** (1,983,081 B · 49065309b0c4fe8e055f693aed295cca · 18,470 lines · 2 members, unchanged since chat 62) and **BUILD183 compendia** (5,920,198 B · d215c184aee08b884d8748fb6d79c0cb · 66,990 lines · 353 members). 355 members in all. Register 1 to 1792. **W-192 seated**, 193 W entries.
- BUILD181, BUILD182 and BUILD183 were rebuilt here from BUILD180 plus the chat-151-B parts fetched from Drive, and every assertion in their three `REBUILD-BUILDNNN.md` documents held: **25 of 25 parts byte-exact** against their published md5s, all three goldens regenerated byte-exact (`r2-26c.out` c1e19648 · 156 lines; `r2-27a.out` 38e11288 · 125 lines; `r2-28a2.out` ef7253ed · 184 lines), and each reverse guard recovering its predecessor. Derivations, not transcriptions.
- **This tree is level with Drive as of `W-192`.** It goes stale again the moment 151-B closes once more, and only a session that reaches both Drive and GitHub can carry that back — the Cowork container reaches neither the repository nor Drive directly.
- Next work: DEF-143 item 11's order has **one family left — 28b-06's reading of Register 1721**. Then M's compendia-scope ruling, staged at `method/RUL-151B-PENDING.md` and **not yet seated**: the Register read in full in source order under the chat-81 cadence, the other four compendia by class sweep. It is keyed to chat 151-B and becomes W-193 or later, appended to `RULINGS-R2.md`, which still ends at the chat-128 block.
