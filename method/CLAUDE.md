...
- **The repository is the store.** `method/` holds the two live bundles, `method/members/`
  holds all 345 members extracted from them byte-exact, and `method/MEMBER-INDEX.tsv` records
  every member's bundle, size, md5 and byte offset in that bundle. **`method/BUNDLES.tsv`
  (tag, filename, md5) is the live-bundle set** — written only by `method/reseat.py` at a build and
  read by `verify.py` and `stage-gate`, neither of which hard-codes a bundle name or md5.
- **Every build ends in `python3 method/reseat.py`,** which re-extracts the live bundles over
  `method/members/`, regenerates `MEMBER-INDEX.tsv` and rewrites `BUNDLES.tsv`, guarded by splicing
  each body back and asserting the bundle md5 before anything is written. Then `verify.py` must say
  VERIFY OK. A build that did not end this way has not been seated. Retired bundles leave the
  checkout and are preserved in git history.
...
## 3. Current state (live files as of chat 153, MEASURED in the repo)

- Live files, both in `method/` and asserted by `method/verify.py`: **BUILD92 main** (1,985,481 B · 0c2d4d946b779428104caee463f3ba06 · 18,490 lines · 2 members) and **BUILD182 compendia** (5,786,231 B · e866ccefcb62b90399750b4f936f9cb3 · 65,746 lines · 343 members). 345 members in all. Register 1 to 1795. **W-190 seated.**
...
- **R3 opened at chat 153, for two classes only** (M's word in the repository session of 3 September 2026): the withdrawn-law class 16z-01, executed as BUILD90 → BUILD91 by `r3-wl.py`, and arithmetic item 153-01, executed as BUILD91 → BUILD92 main / BUILD180 → BUILD181 compendia by `r3-arith-01.py`. Entries 1793, 1794 and 1795 seated. **The chat-67 hold stands for the other 35 docket items** until R3 is shifted here from chat 152 and 152 closes.
- Next work: the shift of R3, then the docket in RUL-128 item 1's order (mathematics, then the prose that reflects it, then the appendices and indices). Still owed from stage (ii): DEF-143 item 11's **26b-02 and 26b-03**, then 27a-02's seven entries, 28a-06's author-and-year match, 28b-06's reading of Register 1721 — one instrument per family — and the Register's full read plus the four compendia class sweeps the chat-152 block scopes.
- The R2 read-position line below was re-measured from WORKING-REGISTER.md in the repository session of 3 September 2026 and corrected; the stale line read *in progress: next §12.11.1.1, main L3177*, carried from chat 76.
...
- The review plan: R1 instrumented census → **R2 full read in source order (the main volume CLOSED in full at chat 141, W-181; stage (ii) still running)** → **R3 corrections by class (OPEN at chat 153 for two classes only, both executed)** → R4 cross-volume audit. Then the compendium task line: renumber → bibliography → Register expansions → main-volume prose.
...
- Chat 67: every volume read in full, no editing until the review closes. **Lifted at chat 153 for the withdrawn-law class and arithmetic item 153-01 on M's word, and for nothing else; it stands for every other class.**
