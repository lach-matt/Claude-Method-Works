...
- Live files, both in `method/` and asserted by `method/verify.py`: **BUILD92 main** (1,987,447 B · ac49200f5a8a02511865260202e75cbb · 18,485 lines · 2 members) and **BUILD191 compendia** (6,640,248 B · 072cc2b825eb52102a23ab657e680ccd · 75,219 lines · 422 members). 424 members in all. **Register 1 to 1796. W-200 seated**, 201 W entries.
...
- **KNOWN RED, and it is named at every close until it is fixed.** `gate.py run --all` carries **three** instruments that assert the Register's extent as an **invariant** instead of scoring it: `r2-26b` and `r3-wl` exit 1, and **`r2-regsweep` exits 0 while printing `INSTRUMENT FAULT - STOP`** — the dangerous shape, because a tool checking only the exit code would re-bank a self-declared fault. **Extent is data.** `r2-regsweep2`, `r2-26b2` and `r3-wl2` are owed and are **readings, not re-banks**.
...
- **Drive stands at BUILD184; this tree at BUILD191 / BUILD92.** Chat 152 is paused by M. Under his **hybrid** ruling — Drive is the store for Cowork chats, the repository for Claude Code sessions, one session that reaches both reconciles — that split is legitimate, but the next reconciliation is owed before either surface advances two more builds.
- Next work: **R2 owes the four remaining compendia by class sweep** (Mathematical, Physics, Index of Indices, Spectra) — the Register leg is discharged. **DEF-143 item 11's order is empty.** Two R3 classes are specified, dry-run and held: `R3-CLASS-WL.md` (approved at chat 128, now needing `r3-wl2`) and `R3-CLASS-EM.md`. **R3-CLASS-EM must not be applied until a checker is seated into the gate** — entry 1617 made that repair once and it did not hold, because `register_review.py` never became a member (reg13-01). Register entry **1797** is staged and not seated, in `method/REGISTER-QUEUE-APPEND-cypher-audit.md`, with `tools/audit_math.py`.
