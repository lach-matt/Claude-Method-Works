...
for k, l in enumerate(UL):
    for m in fp.finditer(l):
        ...
        if m.group(1) == 'I' and not l[:m.start()].strip() and k and re.search(r'(?<![A-Za-z])[A-Z][a-z]?\s*$', UL[k - 1]): continue   # fault 1 (self-caught): the wrapped species "Ne / I (¹⁄₂)" is read on the two-line join
...
code = [unit[0] + k for k, l in enumerate(UL) if l.startswith('    ') and not re.match(r'^ {28,}\S', l)]; print('  4-space code lines: %d %s (fault 2 self-caught: the B.1 wrapped continuation line, ≥ 28 leading spaces inside a whitespace table, is not code)' % (len(code), code))
...
print('  B.1 table: header L%s, DATA rows (compilation lines, 2-space indent + ≥3-space column gap, header excluded) %s, citation lines %s, wrapped continuation L%s (fault 3 self-caught: the row rule is the column gap, not a capitalised word)' % (uline(r'^  compilation', 0), [i for i in uline(r'^  \S.*?\s{3,}\S', 0) if i not in uline(r'^  compilation', 0)], uline(r'^  (\d{4}|JPCRD)', 0), uline(r'^ {28,}\S', 0)))
...
print('  Register pointers in the unit: %s → all present (r2-ch20a §3, §7), no WARNING on any' % sorted(set(int(x) for l in UL for x in re.findall(r'(?<![\d,.])(1[5-7]\d\d|6[0-9]\d)(?!\d)(?!,\d{3})', l) if int(x) in (630, 631, 1578, 1699, 1700, 1763, 1768))))
