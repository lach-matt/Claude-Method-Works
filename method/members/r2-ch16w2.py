...
# r2-ch16w2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch16w.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (26 anchors); nothing else changes. r2-ch16w.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch16w.out byte-exact on those bundles (G0c)
...
A, B = _L('how the method itself did the work.*', 2), _L('reproduces the order.', 1)                      # unit, inclusive; re-measured below
U = MAIN[A - 1:B]
...
for i, t in enumerate(MAIN[_L(' It contains a page by the collaborator that did the computing, describing the single pattern behind'):_L('elements, 6g at 70, 7g at 57, 8g at 28 — sits at its hydrogenic depth −1/(2n²) to')], start=_L(' all one thousand six hundred and thirty-five failures and naming the four results that exist because a conclusion was')):
    if t.startswith('#'): print('   heading', i, t[:80])
print('\nchapter 34 body opens L%d; "## 35." body opens L%d; chapter 34 = %d lines'
      % (_L('how the method itself did the work.*', 2), _L("entry point, read from the table's own coordinates with one observation per", -3), _L("entry point, read from the table's own coordinates with one observation per", -3) - _L('how the method itself did the work.*', 2)))
...
seqline = MAIN[_L('> 1s 2s 2p 3s 3p 4s 3d 4p 5s 4d 5p 6s **5d 4f** 6p 7s **6d 5f** 7p') - 1]
...
print('printed:', MAIN[_L('radicand the scatter in a within a subshell falls from 0.187 to 0.041 at 3p, 0.119') - 1].strip(), '/', MAIN[_L('to 0.028 at 4p, 0.088 to 0.018 at 5p, 0.059 to 0.021 at 6p, 0.086 to 0.047 at 5d.') - 1].strip())
...
FORM = r'ν\(n,ℓ,q\)\s*=.*?\)\s*\)'
f1 = norm(re.search(FORM, MAIN[_L('> **ν(n,ℓ,q) = n − a·√( n − ℓ − 1 + q/2(2ℓ+1) )**') - 1].replace('**', '')).group(0))
f2 = norm(re.search(FORM, MAIN[_L('> **ν(n,ℓ,q) = n − a·√( n − ℓ − 1 + q/2(2ℓ+1) )**,  **and the incoming electron takes the least ν.**') - 1].replace('**', '')).group(0))
print('  L9518:', f1)
print('  L9583:', f2)
...
print('  L9520-21 admissibility: %s' % norm(' '.join(MAIN[_L('>'):_L('> q < 2(2ℓ+1).**')]).replace('>', '')))
print('  L9580-81 admissibility: %s' % norm(' '.join(MAIN[_L('### 34.4 The rule', 1):_L('q < 2(2ℓ+1),')])))
...
rows = [r for r in MAIN[_L('| body | input | intermediate | output |') - 1:_L('| **nucleus + rydberg** | **—** | **—** | **—** |')] if r.strip().startswith('|')]
...
lad = [r for r in MAIN[_L('| ladder | fixes | varies | reaches |') - 1:_L('| **isotopic** | **Z, Nₑ and c** | the neutron number | **the nucleus** |')] if r.strip().startswith('|')]
...
for i in range(_L('### 34.5 The corridor'), _L('### 34.8 The provenance')):
...
for i in (_L('**f is outside the domain and the law says so.** At any f opening p = n−ℓ−1 = 0,', -3), _L('> **107 of 107.** Across Z = 2–108 — every element with a measured ground')):
