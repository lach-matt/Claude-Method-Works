...
for ln in (8458, 8459, 8460, 8470, 8471, 8545):
...
if anchor:
    ...
    for i in a2:
        ...
        block_m = [M[j - 1].rstrip() for j in range(8563, 8568)]
    ...
    print('  table header printed twice: main L8483 %r' % M[8482].rstrip()[:60])
    hdr_pp = [i for i, t in enumerate(PP, 1)
              if t.rstrip() == M[8482].rstrip() and 8390 < i < 8420]
