...
# r2-ch28a4.py — R3 (chat 153-R) — SUCCESSOR to r2-ch28a.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (1 anchors); nothing else changes. r2-ch28a.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch28a.out byte-exact on those bundles (G0c)
...
print('  L11600 printed: %s' % M[_L("   method applied to a nuclear-physics subject; source of §22.1.1's singleton bracket. Every computed")].strip()[:160])
