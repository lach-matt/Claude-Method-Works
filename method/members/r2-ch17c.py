...
A, B = 9806, 9891
assert heading_line(M, '35.4') == 9806 and body_range(M, '35.6')[1] == 9892 and heading_line(M, '36') == 9892
...
twelve = re.findall(r'\b[A-Z]{4,}\b', norm(' '.join(M[1023:1026])))                          # the twelve named in the L1024–L1026 sentence (all-caps tokens)
...
print('  L79 (front matter) says "world outside the book"; L9824 says "world outside the work": %s / %s' % ('outside the book' in M[78], 'outside the work' in M[9823]))
...
rows = [l for l in U[9860 - A:9869 - A] if l.startswith('|') and not re.match(r'^\|\s*-', l)]
...
print('  "corridor" in the unit: L%s; in Chapter 35 (L9716–L9891): %d; Register entries 1701–1712 carrying "corridor": %s' % ([i for i, l in enumerate(U, A) if has_token(l, 'corridor')], sum(1 for l in M[9715:9891] if has_token(l, 'corridor')), reg_has(r'corridor')))
