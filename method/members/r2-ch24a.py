...
GAP2 = re.compile(r' {2,}')
def rows2(lo, hi, ncol):   # chat-136 convention: a long-column row may carry a 2-space gap — parse on the row's grammar (ncol columns, split on ≥ 2 spaces) and PRINT the rows the ≥ 3-space rule would mis-split
    out = []
    for i in range(lo + 1, hi):
        l = M[i - 1].strip()
        if not l or not GAP2.search(l): continue
        ...
        out.append((i, c2))
    return out
def two(lo, hi):   # rows with a 2-space gap and no 3-space gap
...
lo, hi = drange('E.1.1'); r2 = rows2(lo, hi, 3); d2 = [(i, c) for i, c in r2 if c[0] != 'when']
...
lo, hi = drange('E.1.2'); r3 = rows2(lo, hi, 6); d3 = [(i, c) for i, c in r3 if c[0] != 'item']
...
# §5b volume-wide sites of every E.2 figure, with the numeric heading each site sits under (the claim's home, wherever the pointer sends)
NUMH = [(i, m.group(1)) for i, l in enumerate(M, 1) for m in [re.match(r'^#{2,4}\s*(\d+(?:\.\d+)*)\s', l)] if m]
def home(i):
    h = [x for x in NUMH if x[0] <= i]; return h[-1][1] if h else '-'
print('§5b E.2 figures — every site in the volume (line, section): ')
for lab, pat in (('1,585', r'1,585'), ('25.96', r'25\.96'), ('0.0164', r'0\.0164'), ('518', r'(?<![\d,.])518(?![\d,])'), ('157', r'(?<![\d,.])157(?![\d,.])'), ('3.2 × 10⁶', r'3\.2 ?× ?10'),
                 ('560 tests', r'(?<![\d,.])560(?![\d,])'), ('interior optimum', r'interior optimum'), ('admissibility cap', r'admissibility cap'), ('Richardson', r'Richardson'),
                 ('(p−1)', r'\(\*?p\*?\s?[−-]\s?1\)'), ('Gröbner', r'Gr[öo]bner'), ('basis of eight', r'basis of eight'), ('0.972', r'0\.972'), ('0.154', r'0\.154'), ('descendants', r'descendants'), ('F(−1) = 2', r'F\(−1\) ?= ?2')):
    st = [(i, home(i)) for i, l in enumerate(M, 1) if re.search(pat, l) and not (s <= i < t)]
    print('   %-18s %d sites outside the unit: %s' % (lab, len(st), st[:10]))
T = L.load_tower(); att = [a for a in dir(T) if not a.startswith('_')]
print('§8 Λ: register 461 *73, 146 and 731 closed subsets of ambients at 8, 9 and 16 cells*; *2⁹⁷⁶ by brute force*; tower-2 attributes present:', att[:12])
print('   register 461 *73, 146 and 731 closed subsets of ambients at 8, 9 and 16 cells* *2⁹⁷⁶ by brute force* (976 = |Λ₈|, tower-2.out):', [l for l in L.read_member('tower-2.out').split('\n') if '976' in l][:1])
print('done')
