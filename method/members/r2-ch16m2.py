#!/usr/bin/env python3
# r2-ch16m2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch16m.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (6 anchors); nothing else changes. r2-ch16m.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch16m.out byte-exact on those bundles (G0c)
...
M = V['main']; R = V['reg']
U0, U1 = _L('### 32.1.2 Four of the seven, answered by computation'), _L(' Register 373.', 1)                     # the unit, measured by heading scan in this chat
UNIT = M[U0 - 1:U1]
...
for ln, row in ((_L('  165–239                    88%                  5%                  6%'), (88, 5, 6)), (_L('  240–289                    73%                 23%                  2%'), (73, 23, 2)), (_L('  290–319                    55%                 25%                 18%'), (55, 25, 18))):
...
p(f'      {len(wf)} sites: {wf}   (L8918 in the set: {_L(' second is a rate that should fall. Conflating them is what makes one thousand six hundred and thirty-five look') in wf})')
