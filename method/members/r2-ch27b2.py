...
# r2-ch27b2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch27b.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (2 anchors); nothing else changes. r2-ch27b.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch27b.out byte-exact on those bundles (G0c)
...
for i in (_L("              ≥ 3 makes a defect possible.* **Verified against T §8.2 (Appendix G)'s own table**: arity 2 gives"), _L('              **Verified against T §8.4 (Appendix G)**, which replaces seven vocabularies with Brunetti–Fredenhagen–')):
