...
# r2-ch17d2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch17d.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (8 anchors); nothing else changes. r2-ch17d.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch17d.out byte-exact on those bundles (G0c)
...
def hr(t): print('\n' + '=' * 96 + '\n' + t + '\n' + '=' * 96)
A, B = _L('### 35.4 How the method did it'), _L('reached from both ends, closed in the middle.*', 3); U = M[A - 1:B]; UT = '\n'.join(U); norm = lambda s: re.sub(r'\s+', ' ', s.strip())
...
print('  §35.4\'s heading: %r; its opener L9808: %r — the declared subject is how the method ran on the Löwdin problem' % (norm(M[_L('completes the figure: *the silence was not only the finding — it was the address.*', 1)]), norm(M[_L('### 35.4 How the method did it', 1)])))
...
print('  L9808 italic one-liner: %r (an opener, not a heading)' % norm(M[_L('### 35.4 How the method did it', 1)]))
print('  L9870 bold one-liner: %r (a claim, not a heading; DEF-128 item 8: the derivation\'s own, not a 16z-01 site — 16z-01 sites L9596, L9659 recorded, not scored twice)' % norm(M[_L('| **zero unexplained residue** | Pulay 1969 · Löwdin 1950 · exact quartics |', 1)]))
print('  horizontal rules "---" in unit: L%s; italic coda L9884–L9888: starts "*" %s, ends ".*" %s' % ([i for i, l in enumerate(U, A) if l.strip() == '---'], M[_L("*Chapter 34 read the order out of the table's own coordinates. This chapter", -1)].startswith('*'), M[_L("the walk's carried state is the corridor's missing computability. One order,")].rstrip().endswith('.*')))
