...
# r2-ch16i2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch16i.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (29 anchors); nothing else changes. r2-ch16i.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch16i.out byte-exact on those bundles (G0c)
...
lst = M[_L(' the involution by construction, and three published counts measure it:', 1)].strip() + ' ' + M[_L('    248,305 with h¹¹ ≥ 140 · 248,305 with h²¹ ≥ 140 · 495,515 with at least one · and 248,305 +')].strip()
print('        %s' % lst[:150])
...
print('   L8770 "two named confirmations": L8768 names %d'
      % len(re.findall(r'X[\u2080-\u2089\d,_]*', M[_L('  falsified                                                                0')])))
print('        L8768 raw: %s' % M[_L('  falsified                                                                0')].strip()[:120])
...
print('   L8604: %s' % M[_L("  §31.2's string objects                      five, two of them measured in this work")].strip()[:140])
print('   §31.3.3 heading L8720: %s' % M[_L(" and unlike the belt's six.", 1)].strip())
for i in range(_L('### 31.3.3 The Pareto strength, against the right denominator'), _L('### 31.3.4 E(X) on a specifiable slice, and 540 predictions')):
    if M[i - 1].strip():
...
for i in (_L(' D — verifying 540 Kreuzer–Skarke predictions. **Everything the slice specifies is recomputed and'), _L(' reproduces**, at register 387: 208 cells from 13 ≤ h ≤ 128 less twelve exclusions; **E(X) = 540**;'), _L(' 498 join and 498 meet failures of 21,528 pairs; five χ values, only 0 and ±2 and ±4; 112 on the'), _L(' diagonal from (13,13) to (131,131); h¹¹ + h²¹ from 26 to 262. All three falsification tests hold at'), _L(' 540 of 540 — every prediction has h ≥ 1 on both indices, every sum is under the published maximum of'), _L(' 502, and every mirror partner is itself predicted or present. §12.11.6 supplies the calibration: an'), _L(' kind. **What remains is one file read.** The 30,108 pairs are in a gzipped catalogue this session')):
...
print('   text  L8732: %s' % M[_L(' specified exactly in print, and a slice is enough for the operator.', 1)].strip()[:150])
print('   bibl  L11804: %s' % M[_L(' used in §31.3.1.', 1)].strip()[:150])
...
for i in (_L(' **And the index distinguishes them, by asking whether E is removable.**'), _L(' **The same 118 elements. Order the blocks f, d, p, s and define the period by *n* + ℓ, and the defect vanishes.** So the 36 belong to the drawing.'), _L('    A defect of the index vanishes under some re-coordinatisation. A feature of the world'), _L("    survives every one. The test is computable from the cells alone, and §6.2's Janet table is that test")):
    print('      L%-6d %s' % (i, M[i - 1].strip()[:126]))
print('   §31.3.2 invokes it at L8716-L8718:')
for i in (_L(" CICYs and conifold transitions. By §16.7.1's test the absence is removable — not by"), _L(" re-coordinatising but by re-constructing — so it is a defect of the index, like the periodic table's 36"), _L(" and unlike the belt's six.")):
    print('      L%-6d %s' % (i, M[i - 1].strip()[:126]))
...
for p in ('structural', 'accidental', 'normal form'):
    s = [i for i in range(_L(' Between two cells sits a box. Not all of it is Λ.', -1), _L(' Every cell of Λ is definable in one expression, and the reason it is possible is worth more than', -1)) if re.search(p, M[i - 1], re.I)]
    print('      "%s" in §10: %s' % (p, s or 'ZERO SITES'))
print('      L8785: %s' % M[_L(' 49. The gaps in the published list are not noise with respect to closure: removing them moves the set')].strip()[:126])
