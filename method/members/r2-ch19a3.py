# r2-ch19a3.py — successor of r2-ch19a2.py (md5 7246ddce98b82f9002743ede87c0277a), W-235 / DEF-153N: the predecessor read the Mathematical Compendium at
# VOL['mc'][_L(<main-volume text>)] — an MC index keyed on MAIN text by tools/reanchor.py (its documented blind spot), so when
# main moved +3 at L1105 the read slid to MC L1257 (blank) and the banked golden says False where the original r2-ch19a
# read True at MC L1254. One replacement: that index is keyed on the MC line's own text (_M). Nothing else changes.
#!/usr/bin/env python3
...
# r2-ch19a2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch19a.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (6 anchors); nothing else changes. r2-ch19a.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch19a.out byte-exact on those bundles (G0c)

# --- re-anchoring helper (tools/reanchor.py): a main-volume line found by its own text, never by a number ---
def _M(t, d=0):
    """the Mathematical Compendium line carrying text t (1-based), from this script's own directory — a content key for a
    read the predecessor keyed, through _L, on the MAIN volume's text at the same line number (reanchor.py's blind spot)."""
    import os as _o
    global _MM
    try: _MM
    except NameError: _MM = open(_o.path.join(_o.path.dirname(_o.path.abspath(__file__)), 'The_Method_1_6___Mathematical_Compendium-2.md'), encoding='utf-8').read().split('\n')
    h = [i for i, l in enumerate(_MM, 1) if l == t]
    assert len(h) == 1, ('successor: MC line text is not unique or is absent', t[:60], h)
    return h[0] + d
def _L(t, d=0):
    import os as _o
    global _LM
...
a, b = body_range(M, '29.12'); s2 = section_span(M, '29.12'); print('  §29.12 body_range', (a, b - 1), 'section_span', (s2[0], s2[1] - 1), '; U4 lines:', [i for i in range(a, b) if 'U4' in M[i - 1]], norm(M[_L(' *Falsifier.* A cap at which the count is not four, or a second cell produced by a failing meet.', 1)])[:80])
...
print('  §8.3/§8.4 site of the eight (L1887):', norm(M[_L(' Low ranks are cut by floors (*n* ≥ 1, *k* ≥ 1, *e* ≥ 1); high ranks by caps (ℓ ≤ *n*−1, *k* ≤ 2(2ℓ+1), *q* ≤ *k*, *g* ≤ *q*). **There are more caps than floors, so the top is pruned harder.** The skew is how much more Pauli constrains than counting does.', 1)])[:140], '; other main sites of "max − x":', sites(r'max − x')['main'])
...
print('  where admissibility lives — main lines with "admissib" before Appendix A (line, section):', [(i, L.enclosing(M, i)) for i, l in enumerate(M, 1) if re.search(r'admissib', l, re.I) and i < _L('*The three-body problem is solved to exactly the precision a closed index permits, and the book had already computed that precision before the problem was posed to it.*', 3)][:12])
print('  main lines with "monotone" naming single-coordinate/one-coordinate bounds:', [(i, L.enclosing(M, i)) for i, l in enumerate(M, 1) if re.search(r'monotone', l) and re.search(r'single[- ]coordinate|one[- ]coordinate|both directions|either direction', l) and i < _L('*The three-body problem is solved to exactly the precision a closed index permits, and the book had already computed that precision before the problem was posed to it.*', 3)][:8])
print('  MC L1254 cites "the admissibility condition (§14.4)":', 'admissibility condition (§14.4)' in VOL['mc'][_M("**Two constraint forms were already excluded — sums and differences (§17.2) — and the tower requires a third: symmetric bounds. Every coupling axis's exact ceiling is symmetric under particle-hole conjugation, terms(ℓᵏ) = terms(ℓ^{4ℓ+2−k}), verified exactly for every k on the p and d shells, and a symmetric non-constant function is not monotone, so the admissibility condition (§14.4) cannot carry it: min(g, 4f+2−g) for spin, the conjugation ceiling for seniority, and max2J(ℓ, k) — unimodal, peaking at half filling, 25 at f⁷ and zero at closure — for the core's J. One reflection, three appearances. The envelope gap decomposes into three distinct parts, and at the caps of §7.4 the ninth axis's gap is parity 5, fold 1, both 1 while the eleventh's is ceiling 16, parity 24, triangle 17 — three machines, the reflection (conjugation), the congruence (fermion parity), and the triangle (coupling additivity, one sum and one difference, precisely §17.2's two exclusions wearing physics), and no coupling axis escapes all three. Behind them is one law, three instances deep before the triangle made it four: every coupling coordinate's exact physical bound requires either two parents or a congruence, and the tree carries only one — spin needing min(g, 4f+2−g), seniority the conjugation ceiling, the core's J a non-monotone envelope. Each admissible extension is the monotone envelope of its physics. One half of the triangle survives the four-cap check: the region {|2L−2S| ≤ 2J ≤ 2L+2S} is join-closed — zero failures at caps 6, 8, 10 and 12 — and meet-broken at every one, by 2,862, 12,489, 40,887 and 110,229 failing meets, so certainty survives upward and dies downward, which is the reflection asymmetry (§8.4) stated as an inequality. And the check sharpens the statement: impose the parity congruence as well and the join closure dies too — 1,848 failures at cap 6 — so the surviving half belongs to the parity-free triangle and to nothing larger. Three bodies carry all three excluded forms at once; §36.3 runs the method on that case and finds the maximal forbidden case is the solved one.**") - 1], '— the same pointer in the Mathematical Compendium')
