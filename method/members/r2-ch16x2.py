...
# r2-ch16x2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch16x.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (10 anchors); nothing else changes. r2-ch16x.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch16x.out byte-exact on those bundles (G0c)
...
A, B = _L('how the method itself did the work.*', 2), _L('reproduces the order.', 1)
U = MAIN[A - 1:B]
UT = '\n'.join(U)
...
mseg = [norm(x) for x in MAIN[_L('protactinium. *Seventeen of nineteen openings agree; two do not.*'):_L('**Raw occupancy makes every one worse.** *The law needs no new coordinate.*', 1)]]
...
seg = norm(' '.join(MAIN[_L('> q < 2(2ℓ+1).**', 1):_L('lower shells of the same ℓ, plus the fraction of the current one already filled.*')]))
print('    ', seg)
...
for i, t in joins(MAIN, _L("**Madelung's rule inverts 5d/4f and 6d/5f.** Lanthanum opens 5d at Z = 57 and 4f"), _L('that names the order exactly is stated, derived and attributed at §34.4; what')):
...
print('  Chapter-34 span L9494-L9715 register citations:',
      [(i, m.group(0)) for i in range(_L('how the method itself did the work.*', 2), _L("entry point, read from the table's own coordinates with one observation per", -3)) for m in re.finditer(r'[Rr]egisters? \d+', MAIN[i - 1])])
