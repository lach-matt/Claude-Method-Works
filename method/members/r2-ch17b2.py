...
# r2-ch17b2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch17b.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (5 anchors); nothing else changes. r2-ch17b.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch17b.out byte-exact on those bundles (G0c)
...
def hr(t): print('\n' + '=' * 96 + '\n' + t + '\n' + '=' * 96)
A, B = _L("entry point, read from the table's own coordinates with one observation per", -3), _L('completes the figure: *the silence was not only the finding — it was the address.*', 1); U = M[A - 1:B]; UT = '\n'.join(U); norm = lambda s: re.sub(r'\s+', ' ', s.strip())
...
print('  1703 body (the collapse condition; L9797 "two coexisting solutions"):', re.sub(r'\*', '', rbody(_L(' 388, 390.')))[:420])
...
print('  italic epigraph L9718–L9725: starts "*" %s, ends ".*" %s; blockquote L9746–L9750: %d "> " lines' % (M[_L("entry point, read from the table's own coordinates with one observation per", -2)].startswith('*'), M[_L('states what was delivered and — because the method is the subject — how the method')].rstrip().endswith('.*'), sum(1 for l in U if l.startswith('> '))))
