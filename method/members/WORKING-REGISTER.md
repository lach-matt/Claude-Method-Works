...
### W-190 — repository session (Cowork), 2026-09-24 — R3 class CINF: register 1706's eleven corrected by entry 1793; BUILD90 → BUILD91, BUILD180 → BUILD181

**Finding.** `docs/LOWDIN-RECOVERED.md`: the paper session computed 1706's eleven as Λ_chain against `rt/cinf.jsonl`, the table F59.3 had voided as a restart walk at c = 137.035999; re-run with the constant removed by the record's own instrument — recovered from the chat export by `tools/lowdin_recover.py` and byte-checked against its sealed digests — the walk differs at Th, Rf and Z = 120 only, and the record's scorer returns 96 of 107 steps at either setting. **Ruling (M, 2026-09-24):** repaired, not recorded only; wording of `docs/R3-CLASS-CINF.md` approved as put. **Applied by `tools/r3_cinf.py`:** 18 count-asserted substitutions over six members (§35 ×3 and the extent line; the paper's abstract, relativistic clause, §VIII and Figure 5 caption; the Mathematical Compendium's twin entry; the Physics Compendium's Λ_cinf entry ×4; the Index of Indices' ×3); entry 1793 appended in the Register's form citing 1701, 1703, 1706, 1712; the extent lines driven to 1,636 entries, 1 to 1793; kinds table recounted (corrections 149 → 150); MANIFEST.tsv regenerated as `close.py` regenerates it; member reverse guards recovered 4aef772b…, 79aaf239…, 4a06e893…, 99a616ec…, 5a964610…, 27d387ef…, and the bundle reverse guards 49065309… (BUILD90) and ea5becc4… (BUILD180). **Not done:** Figure 5 redrawn (its generator is in no file held); `r2-scf.out` and the DEFECT-CENSUS rows for the twin entries re-banked (next gated close); the withdrawn-law class's drafts renumber to 1794–1795 when it applies. **Pre-existing states measured and left** (W-168): `gate.py`'s MAIN default is the BUILD90 path; `census.py` hard-codes `n>1792`. Dry run banked at `docs/R3-CLASS-CINF.dryrun.txt`.
