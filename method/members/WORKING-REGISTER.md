...
### W-190 — chat 153 (Claude Code, repository session, not a Method chat) — the cypher analysis built as an instrument, Λ audited against it, and the main-bundle route built; instruments cypher.py, audit_lambda.py, register_counts.py, close_main.py; BUILD90 → BUILD91 main and BUILD180 → BUILD181 compendia
...
### W-191 — chat 153 (Claude Code, repository session, not a Method chat) — the audit of the mathematics across five volumes; instruments audit_math.py, audit_mc.py, audit_pc.py, audit_sc.py, audit_ioi.py, reseat.py; BUILD91 → BUILD92 main and BUILD181 → BUILD183 compendia

- **No gate.** A repository session, as W-190's was. `method/verify.py` was the standing check and passed throughout. Read from `method/members/` directly. Nothing was put to M beyond the volume order he set and the one grading question he raised, which became entry 1799.
- **Segment A — the mathematics read as an index (entry 1797).** M's belief, that the mathematics is itself a closed index built alongside the periodic one, tested on the 77 elements `dclose.py` carries, in all five operator-bearing languages rather than the one it uses. **Supported at the fibre level and more strongly than recorded: all 24 (kind, discipline) fibres close at E = 0 in EVERY language**, where `dclose.py` shows order alone. Refuted with kind and discipline on axes — E = 492, 1 of 10 pairs — but that is register 1356's fault, those coordinates being nominal, which is why `dclose.py` fibres over them; recorded so the reading is not mistaken for a result. **And the grading alone — (status, verification, precedent) over all 77, every coordinate ordinal by construction — holds 11 cells of a 16-cell box while all five languages return an IDENTICAL admitted set of 13.** The languages agree in the strongest sense available and E = 2. Register 1176 states E(X) = 0 if and only if the languages agree, on six indexes and three operators, without exception; this is agreement without closure, so the "only if" half does not hold in general. The "if" half is untouched. Census row 1321 had already flagged that wording as C9-OVERGENERALISATION-WORD; this is the case it was flagged for.
- **Segment A — the Mathematical Compendium (entry 1798).** 24 checks, 23 pass. All eighteen section object counts and their total of 299; the eighteen roots; and all three of Λ_law's ordering figures rebuilt from nothing but the printed multiplicities — 7! × 4! = 120,960 orderings, defect range 0 to 21, exactly 288 reaching zero. **E = 1 is unreachable for Λ_law**, the distribution running 0 then 2 upward, which is recorded nowhere. The finding: the Index of Indices prints Λ_const's fifth centre as 5.4 where this volume prints u₀ = 4 and the Physics Compendium carries u₀ = 4 in its own table. Not settled by preponderance — both volumes defend the role axis with *every centre is a small integer or half-integer*, and 2 × 5.4 = 10.8 is not an integer, so under the claim's own test only 4 can stand. Λ_const's own *E = 0 at 2 of 576* is NOT checked: the per-constant (role, carrier) assignment is printed in neither volume.
- **Segment A — the Physics Compendium.** 19 checks, all pass, no findings. Λ_phys's 27 entries and its kind × domain table are independent statements of the same thing and agree on all 13 cells; 8 parameters are this work's own by the source field, splitting 0 universal / 1 all elements / 7 a region or one species as claimed; the six heaviest carry 72, 66, 18, 18, 16, 15. Λ's partition identity, Λ_chain's 119 rows and 107 + 12, Λ_cinf's eleven named entrants, and Λ_V5's five rows all hold — every V5 point inside its declared envelope and every envelope entirely positive, which is the claim that a widening cannot flip a positive sign.
- **Segment A — the Spectra Compendium with the Löwdin solution (entry 1799).** 16 checks, 15 pass. The supply grades sum to the index twice over; all four pinned g depths equal −1/(2n²) at printed precision; the g spans 65, 70, 57, 28 agree between two volumes; the ordering law's two clauses give a total order on the subshells whose first sixteen ARE the Madelung sequence, and the filling order on (n+ℓ, n) closes at E = 0. **The finding is the one M named.** The twelve rows above Z = 108 are graded UNWITNESSED and offered as theoretically secure on the ground that a worst-case spin-orbit narrowing of 0.083 Ha is cleared by every margin; the table directly above that sentence gives the 7p band at 0.058 → 0.210, and the main volume prints both figures in ONE sentence. 0.058 < 0.083. Only the 7p band is at issue, and **how many of its six rows fail cannot be read off, because the table prints the band's range and not its rows** — which is the repair. The grade stands; the clearance argument does not.
- **Segment A — the Index of Indices.** 18 checks, 17 pass; the one failure is 1798, found from this side. The tower table checks entirely: all six fractions equal composable / cells, the rule that counting axes raise the fraction and coupling axes lower it holds with no exception, and Λ₁₀ is the peak at 0.8087. **Two figures were recomputed rather than compared: Λ₁₀ has exactly 485 cells with g = 0, which is 2,535 − 2,050 and so the non-composable cells exactly; and Λ₉'s composable count comes out at 1,169 by enumerating the cells whose target is some cell's source.**
- **Segment A — twelve items book right / instrument wrong, self-caught, against seven findings.** Among them: section IX's fourteen `## Λ` headings against register 1387's fifteen, where the section's own second sentence states the convention — *fourteen closed are stated below; Λ_spectra is stated in Part VIII*; the eighteen-roots table read past its boundary because `# II · THE OPERATORS` intervenes before the next `##`; Λ_phys's 27th entry missed because L's line reads *1 object RESTS on it*, singular; the Spectra Compendium's *57* printed across a line break behind a blockquote marker; and the Index of Indices' tower table having no box column at all, so 47,775,744 is not a figure it restates. **Every one reconciled exactly, and from a convention the volumes state in their own words.**
- **Not done, by design.** The Register itself is not audited — 1,635 entries, the largest volume and the one the other five cite. Λ_const's 576 orderings, Λ_spectra's 596 channels over 70 spectra, and Λ_phys's *objects rest on it* counts are all unreconstructible from print and are recorded as such rather than skipped.
- **Segment B — three closes.** **Main:** BUILD91 → BUILD92 by `close_main.py`, appending 1797–1799 and recounting from the appended Register — 1,642 entries, 1 to 1799, mature 165 to 1799 — the Register the only changed member, reverse guard recovering 7ff88249adc3b00f95a30b97e879e500. **Compendia, first:** BUILD181 → BUILD182 by `reseat.py`, a route built this chat because `close.py` cannot rewrite a seated member and `CYPHER.md` had been revised after seating; CYPHER.md the only changed member, +43 lines, reverse guard recovering bfa0d975c6c915c58c513b00de0e5d2b. **Compendia, second:** BUILD182 → BUILD183 with this text, DEF-154 and six new members, under `--main` the BUILD92 path, which regenerates the MANIFEST rows both earlier builds left stale. No ruling taken.

### W-192 — chat 153 (Claude Code, repository session, not a Method chat) — the Register's structural pass, and the second count table; instruments register_counts.py, close_main.py, kinds.py, register_cites.py; BUILD92 → BUILD93 main and BUILD183 → BUILD185 compendia

- **No gate.** As W-190 and W-191. `method/verify.py` was the standing check and passed throughout.
- **Segment A — the Register's structure, and it holds.** 1,642 headings carrying 1,667 numbers to a highest of 1799, **ascending with no repeat**; 132 numbers unused, accounted for by the count being of headings and by the seven grouped headings of docket 30 carrying 32 numbers between them. **The superseded block's own promise holds exactly: all 70 entries of 95–164 are present and every one carries a supersede marker**, as the front matter says each does. The 31 register numbers cited with no entry are ALREADY DOCKETED — census class `C5-REGISTER-POINTER-UNRESOLVED`, DOCKET §77(c), and DEFERRED's *cited but no entry* class, whose list begins with the same 344, 571, 1002, 1149 — so they are re-confirmed here and not raised as new.
- **Segment A — entry 1800, and it is a fault of this work's own making.** The front matter carries TWO sets of counts. Entry 1796 found the extent stale by one and supplied `register_counts.py` to keep it. **The same front matter carries a second table, "What the entries are", counting entries by kind, and nothing maintained it.** Measured with the seated classifier `kinds.py`: seven of its eight rows reproduce exactly — a finding 1,356, a measurement 499, a withdrawal 33, prior art 61, a new protocol 23, an open question 23, a fault of mine 16 — and **a correction printed 149 against 156 measured**. The gap is exactly the seven entries 1793–1799, every one tagged *(a correction.)*; run over BUILD91's Register the classifier returns 153, and over the Register as it stood before all seven, 149 — the printed figure, correct until this work seated its own findings. **A count that no instrument maintains goes stale at the first thing that moves it, and here that thing was the correction of the other count.**
- **Segment A — both count sets are now maintained together.** `register_counts.py` checks the kind table by INVOKING `kinds.py` rather than reimplementing it, a second classifier that disagreed with the seated one being worse than none; `close_main.py --recount` now rewrites both sets from the APPENDED Register, so entry 1800 and the count it corrects landed in one operation — *a correction* 149 → 157, counting 1800 itself. That is 1796's shape repeated deliberately.
- **Two readings that were mine, not the Register's.** Counting entries with no *(a kind.)* suffix gave 1,538 and measures nothing: `kinds.py` classifies by HEADLINE PATTERN with a body tag overriding, and drops 95–164 by ruling. And 117 forward citations look anachronistic until the block doing most of them is seen — 95–164, every one citing 313, which is the supersede marker working exactly as the front matter describes.
- **Not done, by design.** The Register's 1,643 entries are not read one by one; this is a structural pass and the per-entry read is the chat-81 cadence's work. The main volume is not audited at all.
- **Segment B — three closes.** **Main:** BUILD92 → BUILD93 by `close_main.py`, entry 1800 appended and both count sets recounted — 1,643 entries, 1 to 1800, mature 165 to 1800, *a correction* 157 — the Register the only changed member, reverse guard recovering b96df937c7d44383ed83fe196083733c. **Compendia, first:** BUILD183 → BUILD184 by `reseat.py`, replacing `register_counts.py` and `close_main.py`, reverse guard recovering bbaf464c01d72675bb15e7f9e58e4dd8. **Compendia, second:** BUILD184 → BUILD185 with this text and DEF-155 under `--main` the BUILD93 path, regenerating the MANIFEST rows the two earlier builds left stale. No new members. No ruling taken.

### W-193 — chat 153 (Claude Code, repository session, not a Method chat) — the main volume's structural pass, the last unaudited volume; instrument audit_main.py; BUILD93 → BUILD94 main and BUILD185 → BUILD186 compendia

- **No gate.** As W-190 to W-192. `method/verify.py` was the standing check and passed throughout.
- **Segment A — the volume checks, with one exception.** 15 checks, 13 pass. The contents and the body are independent statements of one structure, so each checks the other: the body carries **36 chapters numbered contiguously**, the contents lists all 36, and **no title differs between them**. The seven appendices appear in both. **Appendix A is exactly self-consistent**: it says *nine proofs are given in full in the chapters and are cross-referenced here, the remaining ten are written out below — nineteen in all*, and that is what it holds — ten `### A.N` sections (3, 8–13, 15, 18, 19), nine tabulated cross-references (1, 2, 4–7, 14, 16, 17), no overlap, together covering A.1 to A.19.
- **Segment A — entry 1801.** The contents lists chapters 20 and 21 as **plain indented lines** where the other thirty-four are `## N. Title` headings. Nothing is missing and no title differs; what differs is the marker. Counted as headings the parts give I 5, II 8, III 6, V 6, VI 5, VII 4 — correct in every part but one — **and PART IV zero**, because its only two chapters are the two plain lines. On the rendered contents page they set as body text beside thirty-four headings, and any apparatus built from heading structure omits both and shows the part empty. §20 is 46 lines and owns the languages; §21 is 342 lines and owns translation and its cost. The repair is two characters twice.
- **Three readings that were mine, not the volume's.** The principles: only P20–P23 carry `PN — ` lines, P1–P19 being tabulated in Chapter 1 — principle numbering is docket 33 and was not re-raised. Appendix A's nine "gaps" are its nine cross-referenced theorems, named in its own table with where each is proved. And the per-part chapter counts in the body came out inflated because the pattern matched numbered list items as well as chapter headings.
- **Segment A — the audit family is complete.** All six volumes now have a pass: `audit_lambda.py` (Λ and its neighbours), `audit_math.py` (the mathematical objects as an index), `audit_mc.py`, `audit_pc.py`, `audit_sc.py`, `audit_ioi.py`, `register_counts.py` and the Register's structural pass, and `audit_main.py`. **153 checks across the six volumes; 9 findings, entries 1793–1801; and 16 checks that failed and were the audit's own fault, every one reconciled exactly and from a convention the volumes state in their own words.**
- **Not done, by design.** No volume is read entry by entry or line by line; every pass is structural or numeric, against figures the volumes record. The per-entry read is the chat-81 cadence's work and is untouched.
- **Segment B — two closes.** **Main:** BUILD93 → BUILD94 by `close_main.py`, entry 1801 appended and both count sets recounted — 1,644 entries, 1 to 1801, *a correction* 158 — the Register the only changed member, reverse guard recovering 58e1f1310cef77fb33b952c87f361d77. **Compendia:** BUILD185 → BUILD186 with this text, DEF-156 and one new member, `audit_main.py`, under `--main` the BUILD94 path. No ruling taken.

### W-194 — chat 153 (Claude Code, repository session, not a Method chat) — the compendia closed by class sweep, three staged duplicates removed from the Register, and a failure of method recorded; instruments sweep.py, reseat.py, close.py; BUILD94 → BUILD95 main and BUILD186 → BUILD187 compendia

- **No gate.** As W-190 to W-193. `method/verify.py` was the standing check and passed throughout.
- **Segment A — a failure of method, recorded first because it governs everything before it.** The nine entries 1793–1801 were audited and seated **without reading `DOCKET.md`, `DEFERRED.md` or the sixty-odd `READ-*.md` chapter reads** — the corpus's own record of what has already been found. Checked against it afterwards: **1799 duplicates docket 17a-03** (`DOCKET.md` L150; `DEFERRED.md` L3345 already carries three further sites and rules the per-row question undecidable until REQUEST-LOWDIN item 10); **1796 duplicates docket 30** (`DEFERRED.md` L2903, `READ-ch17a` L100); **1801 duplicates DEFERRED item 9**, which is docket 28 gaining a member on the same two chapters and the same heading-scan consequence; and **1795 is contradicted** by `READ-ch10` L24, which records the volume already naming both boxes — *bounding box 6,912 (9,216 with k = 0): true, and the box is a second, independent count*. 1793, 1797, 1798 and 1800 show no prior hit. Every one of the nine says *measured this chat* as though it were first sight. The four duplicates are not withdrawn here: they are seated, the Register is append-only, and what supersedes what is a ruling, not a repair.
- **Segment A — three staged duplicates in the Register, and why nothing caught them.** BUILD92 appended entries 1798 and 1799 twice over, once carrying the scaffolding heading `### 1798 — STAGED, not seated` and once seated, so the Register has since printed **1798 twice and 1799 three times**. `register_counts.py` never saw them: its heading pattern is end-anchored, `^### (\d{1,4})(...)\s*$`, so a heading with a suffix does not match and the three blocks counted as prose. **That is why the volume printed the same entry three times while the counts read current** — the count was right about the entries and blind to the duplicates, and nothing else was looking. Removed through `reseat.py`, twelve lines, no other member touched; every printed figure unchanged, as it must be, since none of them ever counted the duplicates.
- **Segment A — the compendia closed by class sweep, under RUL-152 item 2.** `sweep.py` runs the thirteen classes of `DEFECT-CENSUS.tsv` over the four volumes that close by sweep rather than by reading. **No detector is invented**: each is calibrated against the rows the census already records, on the volume the census swept, before it is run anywhere else. `--selftest` asserts eleven reproductions and every one is exact — C1 on ioi, C2 on mc / main / reg, C6a, C10 and C11 on mc, C9 on main / ioi / pc / sc. **The sweep finds nothing new on any of the four.**
- **Segment A — the apparent under-sweep was not one.** Ten census rows on the Physics Compendium and one on the Spectra against 691 on the Mathematical Compendium read as three volumes never swept. **Five of the thirteen classes are Mathematical-Compendium-form classes** — C6a, C10, C11 read the R-form entry, C13 and C6 its back matter and its sources — and the Index of Indices and the Spectra Compendium open no `###` block at all. The Physics Compendium opens 27 and they are a **different form**: a status line, *What it is*, *Where it comes from*, *Where it fails*, and none of the R-form's grade, scope or prior-art fields. Read with the R-form detector all 27 report as missing a grade they were never asked for, which is a false population and not a finding. **An empty census row for those volumes is correct, not a gap** — and the sweep says NOT-APPLICABLE rather than zero, because the two are different answers.
- **Segment A — six classes NOT-RUN, and the distinction is the point.** Where the census records rows this program cannot reproduce, its predicate is not recovered and none is guessed: C13 (286 rows on mc reconcile with none of the 384 handle occurrences, 250 distinct handles or 152 lines in the recorded window), C6, C8, C7, C2b, C3, each with its reason printed. **A resource or definition limit is not a finding** — the rule `cypher.py` already keeps under the name REFUSED, here as a third verdict beside RUN and NOT-APPLICABLE.
- **Two faults found in the reader, not in the books, and both the same fault.** A scan that reads only headings misses what the book tabulates. **§4.1 to §4.10 are a tabulated list** — `  4.1   **attributed outward before checking inward** — …` — and a heading-only inventory calls them unresolved at **42 sites** across main, the Register and the Mathematical Compendium; chapter 4 records that history itself, *an earlier form was a heading with no body at all, carrying the word SEVEN while the register held ten — cited as §4.1 and §4.2 throughout a book that never defined them. Register 655.* It has a body now, and the body is the list. And **`Theorem 7.1 is absent — withdrawn` is not Theorem 7.1 being printed**: a loose anchor read that sentence as the theorem's own statement and silenced the census's own row on it. Both are the same class as the derived coordinate and the tabulated principles caught in W-191 and W-193 — the third and fourth time this session that a structure the book states in prose was missed by a reader looking only for a marker.
- **Not done, by design.** The sweep adjudicates nothing: a row is a class and a site, and what to do about it is the docket's. It does not touch `DEFECT-CENSUS.tsv`, which is mirrored Drive content; `--tsv` emits rows in that file's own schema so the sync can carry them, never this program.
- **Segment B — two closes.** **Main:** BUILD94 → BUILD95 by `reseat.py`, the Register's body replaced under the reverse guard, 1,999,899 → 1,995,160 B, reverse recovering BUILD94's md5 before writing. **Compendia:** BUILD186 → BUILD187 by `close.py`, seating `sweep.py` and `SWEEP.md`. `restage.py` rebuilt the extracted tree from both, `verify.py` spliced every member back and recovered each bundle's own md5, and `register_counts.py` reports COUNTS CURRENT on both tables — 1,644 entries, 1 to 1801, unchanged by the repair.
