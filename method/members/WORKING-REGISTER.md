...
### W-190 — chat 151-B (Cowork) — 26b-02 and 26b-03, the withdrawn Appendix F read against what still cites it (DEF-143 item 11's sixth re-derivation), instrument r2-26c.py; BUILD180 -> BUILD181

- **This is the second run of chat 151. Chat 151 seated nothing and the chat number does not advance**; the W number does, this being W-190. At its open M ruled the standing loop: **a chat opens from Drive, works with Graphify as the substrate for state and cross-check, and closes by writing only the small build parts and the handoff back to Drive.** The repository store-of-record ruling of chat 150 is suspended for as long as GitHub is unreachable from Cowork, and the reasons were measured rather than assumed — see the environment note below.
- **Gate:** ran in full and in order from a clean container; PASS (GATE-ch151B, 02:37 UTC). Nothing was attached. **BUILD180 was rebuilt for the third time**, from BUILD179 plus the nine BUILD180-PARTS, and every assertion in REBUILD-BUILD180.md held: parts byte-exact, r2-26b.py 27,320 B / bbcf1ed1, the golden regenerated at 21,029 B / d55bf6f5 / 190 lines, close.py giving 5,757,241 B / ea5becc4 / 65,420 lines / 341 members with the reverse guard recovering 6251dc13. Re-extract 343 members; census byte-identical (11 s); run --core 5/5 OK; MANIFEST OK 342 listed / 343 extracted.
- **§0a IS STRUCK, not shortened, and this is the chat's second result.** All **68** banked goldens reproduce. r2-ch20a and r2-ch26a were never blocked by the book: both look for the coordinate file at the MEMBERS directory or at `/mnt/project/`, and placing COORDINATES-2_13.csv at `/mnt/project/COORDINATES-2_13.csv` — its original attachment path — makes both reproduce in 1 s while keeping members/ clean so the manifest still counts 343. The file came through the claude.ai Project's own doc list at 10,912,381 B / 906d08be, a route neither chat 150 nor 151 had and one the Drive connector's 10 MB cap does not reach. **r2-ch16n/s/t/u and r2-ch17c also reproduce** (4, 1, 3, 1, 3 s): their §0a status was inherited and never re-measured — chat 150 recorded that it did not re-run them, and chat 151's own 66-of-68 sweep already had them passing while still carrying them on the list.
- **Segment A — done.** The family was located by reading before anything was designed: DEF-139 item 6's record, READ-ch26a's 26b-02 and 26b-03 lines, then §32.1.3 (L8933–L8962), §32.1.4 (L8963–L8996) with its table, §32.1.4.1 (L8997–L9029), the rebuilt Appendix F entire (L11222–L11360) and Prints & Proofs' withdrawn appendix (P10761–P10991), then registers 296, 297, 373, 374, 375, 1762, 1780 and 1786. **What was owed was the reading of the 188 only-PP lines that READ-ch26a §29 named and did not take**, and it is taken.
- **Both recorded items STAND, and two new findings are larger than either.** 26b-02: F.4.1 declares its content withdrawn and carries 0 occurrences of either ratio, while §32.1.3 and §32.1.4.1 cite it for that content — the label resolves, the claim does not. 26b-03: the rebuilt unit prints **3** numeral occurrences against Prints & Proofs' **87**, and §32.1.4's row and its fifty-five total were computed on the withdrawn appendix. **26c-01 NEW:** F.4.1 says the entries that established its *two* ratios stand in the Register; register 1762 establishes the withdrawal ratio with a value (4.21 : 1, p 0.192, 0.705 bits) but *method ratio* has **exactly one site in the whole work**, register 296, which names it and carries no numeral — for one of the two there is no establishing entry. **26c-02 NEW, and the largest:** register 375 records that §32.1.4's fifty-five *is no longer written as a number* and is replaced by a shape sentence; the number is still written at L8978, the shape sentence is in no volume, and three totals stand at once — the printed fifty-five, §32.1.4.1's and register 373's 48, and 375's own 47. **A change the Register records and the volume did not receive is the mirror of a silent change.**
- **26c-03 — the inference replaced by a measurement.** The only-PP lines print 103 numeral occurrences, 64 distinct. Of the **38 substantive figures, 19 have a Register site and 19 do not**, and **16 of the nineteen appear in no volume at all** — among them **24,162 : 54,241 = 0.445**, the withdrawal ratio exactly as F.4.1 first printed it, numerator, denominator and value alike, while register 1762 carries a later ratio computed on a different population. READ-ch26a §29's inference is **false as stated**, and the standard it is measured against is the book's own: register 374's protocol at §2.21, *any figure withdrawn must have its data represented another way*.
- **Census:** nine rows engaged (main 1190, 1226–1230; reg 1465, 1466, 1473) and **all nine were closed in earlier units** — the ch16o, ch26a and 26b units. The earlier line governs (G0b), so none is closed here and CENSUS-CLOSURES-26c.tsv records them as engaged-and-governed.
- **Three instrument faults self-caught and named, none the book's:** a case-sensitive token count that briefly made *method ratio* absent from the work entirely (register 296 prints it in capitals) — the finding was withdrawn before it was recorded and 26c-01 is its correct narrower form; a *withdrawal ratio* count that included F.4.2's own heading; and an only-PP count over raw lines giving 189 against the record's 188 under r2-ch26b §0's set convention. **Book right, instrument wrong, three times.**
- **Environment, MEASURED, correcting chat 151's two recorded conclusions.** GitHub is not blocked by an organisation egress proxy: api.github.com and raw.githubusercontent.com both answer 200 through it, and the 403 on /repos/ carries an Anthropic message naming an `add_repo` affordance that Cowork does not provision. The GitHub Repo connector holds a **user-to-server GitHub App token** — `get_me` succeeds while repository creation returns *Resource not accessible by integration* — and it 404s this private repository even with the Claude App granted all repositories, which is a known unfixed bug class. The connector registry carries no third-party GitHub connector, and claude.ai's custom-connector dialog exposes no header field, so a PAT cannot be supplied. **Drive is not reachable from the container at all** — googleapis.com and drive.google.com fail to connect rather than 403.
- **Not done, by design:** the four re-derivations after this family in DEF-143 item 11's order (**27a-02's seven entries next**); register 375's *computed by the press at every build*, not run here because appf.py is a member and the press is not exercised by this instrument (docket 38); the *without exception* half of L11298, still carried as a candidate and still not scored, this family's count word being neither. The chat-67 hold stands throughout: **findings are recorded and never repaired.**

### W-191 — chat 151-B (Cowork), second segment — 27a-02, the seven entries Appendix G says rest on three of its sections (DEF-143 item 11's seventh re-derivation), instrument r2-27a.py; BUILD181 -> BUILD182

- **Second segment of chat 151-B, opened on a closed one.** W-190 seated 26b-02 / 26b-03 and closed BUILD181; this entry seats 27a-02 and closes BUILD182. **The DEFERRED and DOCKET blocks are keyed to the chat that wrote them and this chat has now written two, so they are DEF-151b and the chat-151-B second delta**; the W numbers stay sequential and this is W-191. M's loop ruling of W-190 governed throughout: opened from Drive, worked with Graphify as the substrate for state, closing by writing only the small parts back.
- **Gate:** not re-run at the open of this segment. GATE-ch151Bclose (03:29 UTC) had already run the full gate against a fresh re-extract of BUILD181 — census byte-identical, run --core 5/5, MANIFEST OK 346 listed / 347 extracted, **69 of 69 goldens reproducing**, verdict PASS — and the tree was not touched between that certificate and this segment. **INFERRED, and named as such: no gate step was re-executed to open the segment.**
- **Segment A — done.** Located by reading before anything was designed: DOCKET item 9(b) / 35's 27a-02 line, DEFERRED's record of what is owed, READ-ch27a's full entry, then Appendix G entire (L11361-L11406, 46 lines, 30 DATA rows), the three rows at L11396 / L11397 / L11400, **Transitions.md §8.2 (L792-L811), §8.4 (L827-L868) and §10.4c (L1237-L1269)**, the seven Register entries 1375 / 1399 / 1403 / 1519 / 1523 / 1535 / 1551 in full, and the Mathematical Compendium's *modular ledger* block (L2406-L2417) with its prior-art table. **What was owed was a reading of the seven for DEPENDENCE, the probe having refused to score on itself, and it is taken.**
- **27a-02 STANDS, and the reading is stronger than the probe.** CONVENTION DEP is stated before anything is scored: an entry rests on a section if it NAMES the source (DEP-N) or its own deciding sentence is an instance or consequence of the section's MECHANISM (DEP-M), with the deciding phrase asserted present in that entry's text; undecidable entries are carried as NEAR and never assigned. Of the **eight (row, entry) pairs the column asserts, 0 are DEP-N and 0 are DEP-M.** Six are another mechanism entirely — 1375 twice (A.define's dual), 1535 (a Nilsson parity check), 1551 (a spectra capture), 1399 (two tower conventions), 1403 (the Λ_T corridor's endpoints). Two are carried as an upper bound: **1519**, whose treewidth-one result is the same arity mechanism read from the low side but credited to Freuder and to register 507 rather than to §8.2, and **1523**, whose conflict-graph measure is arity again and which names **M.C2 — an object of §10.4d, not of §8.2**.
- **27a-07 NEW: the compendium and the appendix disagree about what rests on §10.4.** The Mathematical Compendium's *modular ledger* names **T 10.4e (App. G)** as its source and rests on registers **1019, 1020, 1022, 1035, 1483, 1484**; G row 10.4c names **1399 and 1403**, and the two sets are **disjoint**. The appendix is the volume that calls itself *the address*. DEFERRED recorded the block's list as five and it is six — **register 1020 was omitted from the record's list**, corrected here.
- **27a-08 NEW: 1403 is the only one of the seven carrying a WARNING** — its eight a values are RECONSTRUCTIONS held in HANDOFF.md and not measurements — **and row 10.4c prints it as an object resting on a computed result without restating the qualification.** Every WARNING on every cited entry was read, which is what docket 9(b) / 35 asks, and 1403 is the only one.
- **Census:** four rows engaged. 717 and 1231 closed in the ch27a unit, the earlier line governing (G0b) — 1231 is the appendix's own *entry bodies are never edited*, which this reading depends on rather than disputes. **1349 closed here NOT A DEFECT** (register 1375's *the monotonising ordering always exists* states a fact about the construction, and the entry itself reports the measurement that does not generalise). **293 engaged and NOT closed**: this reading took the *modular ledger* block's citation list, not its eleven numerals, and a row is not closed on a reading that did not take it.
...
### W-194 — chat 151-R (Claude Code, repository) — the Register read end to end, entries 1–1792, thirteen units plus a whole-volume sweep; instruments r2-reg1a…r2-reg12, r2-regsweep, r3-em; BUILD184 -> BUILD185

- **What this segment is, named plainly.** It is not a Method chat. It is a **Claude Code session on the repository**, opened while chat 151-B worked in Drive and closing after chat 152 had opened on R3. It is keyed **151-R** so that it claims no chat number ahead of a live one, and its W number stays sequential: **W-194**. Everything below was measured in this container against the seated members, and every instrument it seats reproduces its golden here.
- **The chat-150 repository-store ruling is suspended (W-190) and this segment does not reopen it.** M suspended it for as long as GitHub is unreachable from Cowork; that is still true. This segment works the other way round — the container reaches **both** Drive and GitHub — so it also **carried BUILD181, BUILD182, BUILD183 and BUILD184 back from Drive into the store**, each rebuilt from its `REBUILD-BUILDNNN.md` rather than transcribed, each part checked by md5 and never by size, each golden regenerated by *running* its instrument. BUILD184 came back at 5,977,919 B / `d4350094449e3a4bf488315e21199bd4` / 67,533 lines / 357 members, every figure its REBUILD document publishes.
- **M's scope ruling of 3 September is SEATED here** (RULINGS-R2.md). Put to him with three routes and their measured costs, he chose the third: **the Register in full under the chat-81 cadence, the other four compendia by class sweep**. DEF-141 item 15 closes as a question; chat 67 and chat 127 item 2 are reconciled rather than either being reopened. It had been staged unseated since chat 151-B and is seated now.
- **THE REGISTER IS READ END TO END, ENTRIES 1–1792.** Thirteen units in source order plus a whole-volume mechanical baseline, `r2-reg1a` to `r2-reg12`, `r2-regsweep`. **All fourteen instruments reproduce their goldens in this container.** Findings: **reg1-01…reg1-06** (front matter), **reg2-01** (the genesis block), **reg3-01** (a broken headline inside the superseded block), **reg4-01**, **reg7-01 / reg7-02** (the split-emphasis class), **reg8-01 / reg8-03** (the withdrawn entries), **reg12-01…reg12-03** (the remainder 395–1792), **reg13-01**. Units 5, 6, 9, 10 and 11 recorded **zero deviations**: entries 199–289, 290–326, 327–361 and 362–394 stand exactly as printed.
- **Every computational claim re-derived in the read reproduced.** Entries 4/11/12/39/46/47/50/73/74 (the Λ₈ tower, 976 cells over 8 coordinates under 7 Heaviside constraints), 258/267/268/281, 301, 334, 336, 393/394, and the Λ₉–Λ₁₃ ladder 1,654 / 2,535 / 13,585 / 70,905 / 199,130. **Not one live mathematical defect surfaced in this volume.** Where a number and a reconstruction disagreed, **G0c held every time and the fault was the reconstruction's** — a rank read as a sequence position, ℓ lifted without f, a §-resolver ending in `\s+`, and the front-matter arithmetic at reg1-04.
- **reg1-04 was taken backwards first, and the correction is the record's.** The reading first scored the Register's closing line. The three blocks must sum to the printed total and 94 + 70 + 1,470 = 1,634 ≠ 1,635, so **the front matter is the defective half**. The correction came from `tools/register_counts.py`, written by another session on this repository, whose docstring had independently reached reg1-04 and docket 30's convention that **an entry is a heading, not a number**. First time in the read that the *reading* rather than an instrument was at fault.
- **reg8-02 was WITHDRAWN as false, and the census had the answer all along.** It concluded that no part of the body cites a withdrawn entry. Measured on the right nets, `entry 1000` (1 site) and `R 1002` (3 sites) do; the net was `registers?\s+N` and it was too narrow. Census rows 8, 15, 16 and 17 carried the sites. **Recorded, not quietly dropped.**
- **The 110 withdrawn entries are RECOVERED byte-exact** into `WITHDRAWN-RECOVERED.md`, out of `The_Method_1_6_BUILD10_main_and_register.md` in the mirror, **110 of 110 verified**. Graphify holds none of them: `graphify_find("1725")` returns 0, the local graph carries 4,793 nodes and **0 from markdown files**, and both indexes are stale. The archive is the witness, not the graph.
- **reg13-01, and it supersedes the framing of reg7-01 and reg8-03.** The word-by-word C9 pass took all **223** remaining rows individually and closed every one **not a defect**, and rows 1420/1421 led to **entry 1617**, live, which states that the emphasis class was diagnosed, repaired — *"Thirteen unbalanced entries become five"* — and its five-case skip list *"now written into `register_review.py`"*. Three things follow, all measured: the notation exception reg7 derived by hand **is** 1617's skip list; the seated Register carries **22** unbalanced entries where 1617 left five, **16 of the 22 written after it**, with 313, 604 and 747 standing before it and still unrepaired; and **`register_review.py` is not a member**, against the chat-68 ruling. **The class is not one the record diagnosed and lost. It is one the record diagnosed, repaired, recorded, and re-accumulated — because the instrument that enforced the repair never entered the bundle.**
- **`R3-CLASS-EM.md` and `r3-em.py` are seated, and NOTHING IS APPLIED.** The chat-67 hold stands and RUL-128 item 1 puts the mathematics first, so this class waits. Dry-run and measured: **repairs 246, declines 29, delta +360 B, 0 lines, exactly 246 lines differing, reverse guard recovers `79aaf239a42c1649e914b7cba9d5ce2a`**. The instrument caught its own fault in the dry run — it read each body at `RL[pos+1]` and wrote to `NL[pos]`, the blank line above, for +195,358 B with the originals untouched — and wrote nothing, which is what a dry run and a reverse guard are for. The spec now cites 1617 as the prior repair and its skip list as the prior notation ruling, and states that **R3 seats a checker into the gate first or does not apply this class at all**.
- **Census:** **269 rows closed across thirteen files**, every row falling in entries 1–1792, each carrying its own token and context. reg13 alone carries 223. Two sub-kinds named in the C9 pass and worth the record: the token as part of a **name** (`always-lower` / `always-upper`, rows 1359/1360) and the token as a claim **being refuted** (rows 1409, 1250) — stating a universal in order to knock it down is not asserting it.
...
### W-201 — chat 153 (repository session) — R3's first two classes executed on the store head; the re-anchoring finding; BUILD92/BUILD191 -> BUILD94/BUILD192

- **Opened on the wrong base, and stood down.** The session's first four commits built on b695e48 (BUILD90 + BUILD180) and produced BUILD91/BUILD92 main and BUILD181/BUILD182 compendia. `HANDOFF-152-RESUME.md` records that base as superseded: chat 151-R had carried chat 152's work to BUILD92 + BUILD191, 424 members, and **`origin/main` does not track the store**. Four collisions resulted — Register 1793/1794 (chat 152's, and the handoff had anticipated exactly this: *r3-wl also claimed them and re-takes*), 1795, a second BUILD92 main (`0c2d4d94` against the store's `ac49200f`), and `RUL-152-PENDING.md`, which DEF-152 item 9 rules a stale duplicate of RUL-151R. **M ruled: re-take onto the store.** The superseded line stays in git history as a sibling, exactly as 151-R kept chat 152's BUILD181.
- **Segment A — class WL (16z-01) re-taken as `r3-wl2`.** BUILD92 -> BUILD93. The successor HANDOFF-152-RESUME section 6 item 1 owed; `r3-wl` is seated and is never edited in place. Numbering MEASURED and asserted in the instrument: highest seated 1796, **1797 left alone** because `REGISTER-QUEUE-APPEND-cypher-audit.md` stages it for the unseated `audit_math.py`, so the class takes **1798 and 1799**. M's chat-128 wording carried unchanged. Ten count-asserted substitutions on the main volume, zero on the Register, two entries appended; reverse guards recovered main `4aef772b` and bundle `ac49200f`; 232 press anchors unmoved.
- **Segment B — arithmetic 153-01 re-keyed to entry 1800.** BUILD93 -> BUILD94 main, BUILD191 -> BUILD192 compendia. The Mathematical Compendium's *146 of 163 = 92%* now reads **90%**, which is what 146 of 163 is at 0 dp rounding half to even and what **register 1048** states; 92% is 1048's own object for the combined two-claim figure, 198 of 215. **Which of the two the compendium's 92% came from is NOT decided** and entry 1800 says so. One numeral changed.
- **M's Ruling A observed in both.** The entries stand, the repair waits: no count site touched, no `kinds.py --write`, main L7373 untouched. `r3-wl2` asserts `pre == []` so a count site cannot move by accident. The drift is **scored, not asserted** — `r3-wl` exits 1 on the gate for treating extent as an invariant, and extent is data. After the three entries the kinds table prints *a correction* 149 where `kinds.py` counts 156, stale by seven; reg1-04 carries this and R3 repairs it with its class.
- **Segment C — the finding this session exists to record. The first R3 volume text change moved 70 of 86 instruments, and most must be RE-TAKEN rather than re-banked.** `r3-wl2` inserted a seven-line status paragraph at §34.4, so **every main-volume line from 9608 moved by +8**. MEASURED by a clean `gate.py run --all` on a settled tree: **16 OK, 58 output-differs, 12 ERROR**. The baseline HANDOFF-152-RESUME records is 82 OK.
- **`shiftcheck.py`, seated by this close, is the instrument that sorts them.** `close_rebank.py` re-banks by running an instrument and records that a measurement moved; it explicitly does not decide whether the movement is a finding. That leaves the hole R3 falls into 35 more times: **an instrument that hard-codes a line number reads the wrong site after any insertion above it, and re-banking it banks the wrong reading, after which the gate reports it green forever.** shiftcheck asks, per changed line, whether the change is accounted for by a reason **declared on the command line** — `--shift PIVOT:DELTA:MAX` (bounded above so byte counts and md5 fragments are not silently explained) and `--subst OLD=NEW`. It runs instruments through `gate.py`'s own runner imported by path, writes no golden, edits no instrument and touches no volume.
- **The authoritative split, MEASURED** (shift 9608:8:11856 plus thirteen declared substitutions — three member md5s, the Register's line count and entry totals, the kinds table, and two genuine consequences of the repair: `withdraw` 57 -> 58 sites and *stated by the law itself* -> *stated by the form itself*): **UNCHANGED 16, EXPLAINED 14, UNEXPLAINED 44, UNRUNNABLE 12.** The 14 are re-banked in this build. **The reading bill is 56**, and it is owed before any further R3 class lands.
- **Three times the instrument corrected me, and each correction is the reason it exists.** (i) I called `r2-ch16t` and `r2-ch17e` positional; both move 11856 -> 11864, the main volume's **end-exclusive** span bound one past its 11855 lines, so the declared window was one too tight. (ii) **`r2-ch17e` is a genuine finding**: it computes its site list correctly (11832 -> 11840) but carries **11832 hard-coded inside a printed string**, so it now asserts *L11832 is R.7* when R.7 is at 11840 — re-banking would have banked a false statement. (iii) That refutes the static scan I had proposed to size the job by: a **reporting-only** literal is not automatically safe, because a report can assert a falsehood as readily as a read can target the wrong line. **The checker is the arbiter, not the scan.**
- **A limitation of the shift window, recorded rather than worked around.** A numeric window cannot tell a main-volume line number from a same-range number belonging to another file: `r2-ch16x` prints *PP lines: 11372*, which the window wrongly proposes to shift to 11380. The tool therefore errs toward UNEXPLAINED, which is the safe direction and costs reading, never silence. **An EXPLAINED verdict is only as good as the declared reasons, and every one of them is printed in the report.**
- **Not done, by design:** the 44 UNEXPLAINED readings and the 12 successors, which are R3's next unit and are listed in DEF-153B; DEF-152 items 1-9; every other class of the 37-item docket. No instrument was edited to make a discrepancy disappear (G0c), and no golden outside the EXPLAINED 14 was written.
- **A process fault of mine, recorded.** The session's first `gate.py run --all` was backgrounded and then had `method/members/` rewritten underneath it by the two builds; its failures were artefacts of that overlap. It was discarded and re-run on a settled tree, and the numbers above are from the clean run. **A gate run and a build must never overlap.**

### W-202 — chat 153 — the fourteen goldens shiftcheck proved safe are re-banked; BUILD193 -> BUILD194

- **Only the fourteen shiftcheck returned as EXPLAINED are re-banked**, under the reasons declared in W-201: `--shift 9608:8:11856` and thirteen named substitutions. They are `kinds`, `r2-24a`, `r2-25b`, `r2-ch16a`, `r2-ch16c`, `r2-ch16f`, `r2-ch16k`, `r2-ch16o`, `r2-ch16q`, `r2-ch16t`, `r2-ch24a`, `r2-ch26a`, `r2-reg11a`, `r2-reg5a`. Every difference in each is accounted for by a reason printed in the report; none is a finding.
- **`close_rebank.py` regenerates each by running its instrument**, refuses a copy, refuses a non-zero exit and refuses a golden that did not move. The reverse guard restores every re-banked golden and must reproduce BUILD193's own md5 before anything is written.
- **Nothing else is re-banked.** The 44 UNEXPLAINED and the 12 UNRUNNABLE stay red, and the gate stays honestly red at 56. Each is a reading, and DEF-153B carries them.

### W-203 — chat 153 — the first re-anchored successor, and the retirement problem it exposes; BUILD194 -> BUILD195

- **`r2-ch17e2` is seated, proved, and banked.** It is the first of the 56 readings R3 owes. `r2-ch17e` printed **`L11832 is R.7`** as a literal inside a string; `r3-wl2`'s +8 shift moved the site and not the literal, so the seated instrument now asserts a falsehood and **re-banking it would bank that falsehood**. The successor derives the address from what the line already computes. `r2-ch17e` is seated and is never edited in place (chat 68), so this is a successor and not an edit. Golden `r2-ch17e2.out` banked, 12,178 B, md5 39f830e2, 104 lines.
- **`tools/proveanchor.py` is the guard, and it earned itself on its first use by REFUSING my first attempt.** Re-anchoring changes how an instrument finds its site, which can silently change what it measures — exactly G0c. So a successor is trusted only if, run against the corpus as it stood **before** the shift, it reproduces its predecessor's banked golden **byte-exact**; the change is then provably addressing and nothing else. It stages the old bundles to a scratch tree with a splice guard, points `/home/claude/members` at it because seated instruments hard-code that path, and restores the symlink on exit including on failure.
- **My first re-anchor was WRONG, and the refusal is what found the true referent.** I read `L11832 is R.7` as naming R.7's heading and resolved it to the heading, which is at **11806**; the golden says 11832. **11832 is the KAM site that falls inside R.7** — the third element of `kam`, which that very line prints. The sentence reads *the site at 11832 is R.7's*, not *R.7 begins at 11832*. Re-anchored to `[i for i in kam if i >= r7_head]`, it reproduces byte-exact on the old bytes and reports **11840** on the new, with R.7's heading at 11814 — both moved +8 together. **A plausible re-anchor is not a proved one**, and without the guard this would have been seated wrong and banked.
- **The retirement problem, recorded and NOT solved by patching the gate.** `gate.py run --all` runs every `NAME.out` in `members/` and has no retirement mechanism, so **a superseded instrument stays in the run set and stays red forever.** That is tolerable at three known-red and untenable at 56. The project's standing stance is not to patch `gate.py` — HANDOFF-152-RESUME makes exactly that point about its `MAIN` constant being a default taken by a flag, *nothing is patched to make this work* — and `gate.py` is a seated member besides.
- **The disposition, an instrument decision (§1) and taken here:** a new member **`RETIRED.tsv`** records `superseded → successor`, the reason, and the bundles the successor was proved on. The gate keeps running the predecessor and keeps reporting it red; the red is now **accounted for** rather than merely known. A filter over gate output is not built yet — one retiree does not earn a tool, and it will be built when the retired set is large enough to warrant it. **What must never happen is re-banking a superseded instrument to silence it**, because its output is false by construction.
- **Not done, by design:** the other 55 readings. The expensive ones are named in DEF-153C — `r2-ch28a` (60 literals), `r2-ch16z` (53), `r2-ch26b` (34), `r2-ch23a` (33), `r2-ch17d` (32), `r2-ch17c` (31). DEF-152 items 1–9 remain queued behind the readings on M's ruling.

### W-204 — chat 153 — two more re-anchored successors, and a defect the shift exposed rather than caused; BUILD195 -> BUILD196

- **`r2-ch16v2`** — `r2-ch16v` printed `MAIN[9891]` by fixed index, the `## 36.` heading, where register 1359's cut three-body chapter is reported. The heading moved +8 and the index did not, so the seated instrument prints the wrong line. Re-anchored to `heading_line(MAIN, '36')`, which the instrument already imports and which the standing method requires — exact token, body occurrence, never prefix. Golden `r2-ch16v2.out` 9,486 B, md5 9087f791.
- **`r2-ch16u2`** — `r2-ch16u` scanned `range(9727, 9735)`, a **fixed window** over §35.1, for its Chapter 33 / Chapter 27 sites. The section moved and the window did not, so the sibling site vanished from the report entirely. Re-anchored to `body_range(MAIN, '35.1')`, which the instrument already defines: **a window that follows its section cannot be moved by an insertion above it.** Golden `r2-ch16u2.out` 15,292 B, md5 b40ee741.
- **Both PROVED on the pre-shift bundles** by `tools/proveanchor.py`, byte-exact against their predecessors' goldens, before either was banked. On the current corpus `r2-ch16u2` reports the site at L9737 where the golden had L9729 — the same text, moved +8 — and `r2-ch16v2` prints the Chapter 36 heading rather than the paragraph that now occupies the old index.
- **A reading error of mine, recorded because it nearly cost a correct successor.** I read the checker's *normalised* banked line (`main L9737`) as though it were the raw golden (`main L9729`) and briefly concluded `r2-ch16u2` was reporting a shifted site as unshifted. It was not: the golden says 9729, the normalisation adds the declared +8, and the successor produces 9729 on the old bytes and 9737 on the new. **A tool's normalised output is not the record; the record is the banked file.**
- **reg7-A NEW — a defect the shift exposed and did not cause.** `r2-reg7a` computes `100 × len(split_e) / len(pos)` — the **live** entry-heading count — while its own sentence names *"of the 1,635 entries"*. MEASURED: the live count was **1639** before this chat's three entries and is **1642** now; 275/1635 and 275/1639 both print 16.8 %, so the mismatch was invisible, and 275/1642 prints **16.7 %**, at which point the computed and the named denominator part. **The instrument divides by one number and names another**, against the standing method's *check the arithmetic of every ratio and percentage, and name the denominator*. Recorded, not repaired: its successor is owed with the rest.
- **Not done, by design:** 52 readings remain (40 UNEXPLAINED after these two, 12 UNRUNNABLE). `shiftcheck` re-run on a settled tree with the protections and substitutions declared reports **UNCHANGED 31, EXPLAINED 2, UNEXPLAINED 42, UNRUNNABLE 12** before this build. DEF-152 items 1–9 remain queued behind the readings on M's ruling.

### W-205 — chat 153 — three more successors, and two instruments cleared by fixing the checker instead; BUILD196 -> BUILD197

- **`r2-ch16n2`, `r2-ch16p2`, `r2-ch22a2` seated, each PROVED byte-exact on the pre-shift bundles before banking.** `r2-ch16n` carried a hard-coded site **list** — three of its nine entries (9722, 9894, 11853) sit above the insertion point and named the wrong lines; the six below it did not move and are left exactly as they were. `r2-ch16p` named two citing lines (11555, 11799) by literal and started a sweep at the References body (11503) by literal; all three are now resolved by their own text, and its §32.2 window at 9030 is **below** the insertion point and is deliberately untouched. `r2-ch22a` printed *Chapter 23 citation claim, L10377* inside a string while computing the Nesterov sites beside it.
- **Every anchor is resolved from the site's own text**, through a local helper that asserts the substring matches **exactly one** line. An anchor that matches two lines, or none, fails the run rather than picking one — a resolver that guesses is worse than a literal, because it is wrong silently.
- **Two instruments needed no successor at all: the fault was in my checker.** `r2-ch16r` prints a section body with an absolute line number on **every** line, so after the shift every line differs and `difflib` re-pairs them by content into unbalanced hunks (−65/+57, then +0/+8) that could not be zipped; a fully explained shift looked structural. **When both outputs have the same number of lines, position is the truth** — line *i* answers for line *i* — and `shiftcheck` now compares positionally in that case and never through `difflib`. `r2-ch19a` prints *L10063 vs P9714*, a Prints & Proofs line, and the protection pattern required a `PP ` prefix; widened to `\bP\d{4,5}\b`. **Both now return EXPLAINED.**
- **The lesson, twice in one segment: establish that an instrument is mis-TARGETED before writing a successor, not merely mis-COMPARED.** A successor written for `r2-ch16r` would have re-anchored something that was never wrong, and `proveanchor` would have passed it, because it would have reproduced the old bytes faithfully. The guard proves a successor faithful; it cannot tell you the successor was needed.
- **Re-banked in this build, all four EXPLAINED under the declared reasons:** `r2-ch16r`, `r2-ch19a`, `r2-ch20b`, `r2-reg8a`.
- **Not done, by design:** the remaining readings. After this build the bill is 37 UNEXPLAINED and 12 UNRUNNABLE. The expensive six are untouched — `r2-ch28a` (60 literals), `r2-ch16z` (53), `r2-ch26b` (34), `r2-ch23a` (33), `r2-ch17d` (32), `r2-ch17c` (31). DEF-152 items 1–9 remain queued behind them on M's ruling.

### W-206 — chat 153 — the four goldens cleared by the checker fix are re-banked; BUILD197 -> BUILD198

- **Four only: `r2-ch16r`, `r2-ch19a`, `r2-ch20b`, `r2-reg8a`.** Each returns EXPLAINED under the reasons declared in W-205 — `--shift 9608:8:11856`, the Prints & Proofs protections, and the named substitutions for the Register's growth. None of the four needed a successor: `r2-ch16r` and `r2-ch19a` were cleared by correcting `shiftcheck` itself, not the instruments.
...
### W-208 — R3 — Book repairs and corrections — eleven goldens re-banked by running their instruments; BUILD200 -> BUILD201

- **The estate at BUILD98, re-run in full and cached:** 9 UNCHANGED, 0 EXPLAINED, 73 UNEXPLAINED, 13 UNRUNNABLE. `shiftcheck` explains nothing even with both of this session's bands declared, because the estate carries drift from builds that predate the session — `r2-reg9a` moves §6.2 L1594 -> L1596, a +2 far above either pivot — and explaining it would need a per-vintage band map back through every build since each golden was banked. That is the wrong route. `close_rebank.py` re-banks by RUNNING the instrument, which needs the anchors right rather than the shift explained.
- **Triage of the 73, digits masked, then the digits classified:** 16 change in numbers only, 41 change in text, 15 change in line count. Of the 16, eleven change ONLY in line references (every changed number >= 1000, re-found by `heading_line`/`section_span` or by Register entry number) or in the entry count that six appended entries move (r2-reg8a: 1667 -> 1673, 1..1800 -> 1..1806). Those eleven are re-banked here. Five carry a count or a value change and are readings, not re-banks: `r2-ch19b` (a count 7 -> 0), `r2-ch16k` (54 -> 55, 83 -> 84), `r2-reg7a` (8 -> 7), `r2-ch21a` (deltas 9/10/33 alongside the recognised ones), and `kinds` (156 -> 161, 23 -> 24), which is the Register's kind census and belongs with `reg1-04`, sequenced after the readings on M's ruling.
- **Why "numbers only" was checked and not trusted.** A figure changes both when the volume moves under a correct anchor and when the instrument reads the wrong line and reports a wrong figure of ordinary shape; `r2-ch17e` did exactly that this session, asserting L11832 is R.7 when R.7 stood at L11840. So each of the eleven was checked for how it anchors: every 4-5 digit literal in the chapter readers is in a docstring and none is in code, and the Register readers key on `### NNN` headings, which appends at the end do not move. The printed line numbers are OUTPUT, not input.
- **Not done, by design:** 41 text-changed and 15 line-count goldens remain readings; 13 UNRUNNABLE remain successors. `reg1-04` and `kinds` wait on the readings. Ruling A holds: no front-matter count is touched and `kinds.py --write` is not run.

### W-209 — R3 — Book repairs and corrections — ten more goldens re-banked by running; the positional class named; BUILD201 -> BUILD202

- **The second re-bank, same discipline as W-208.** Of the 40 text-changed goldens, six change only in a member md5 the instrument prints as a fingerprint plus line references — `r2-24a`, `r2-26c2`, `r2-27a2`, `r2-28a3`, `r3-em`, and `r2-25b`, whose one other change is the Register's WARNING count 39 -> 40, which is the WARNING the sixth-value build placed on entry 1332. Four more change only in line references and in counts the appended entries move: `r2-ch16k` (entry headings 1635 -> 1641, both computed by `len(ent)`; withdrawal-word entries 54 -> 55, entry 1806 quoting §D.2's scale), `r2-ch16y` (13 -> 14 sites for "106 of 106" and 4 -> 5 for "eighteen resets", both entry 1804), `r2-ch21a`, and `r2-reg12` (the remainder unit 91 -> 100 entries). Ten re-banked here; 21 in all across W-208 and W-209.
- **The positional class, named with evidence.** The remaining text-changed and line-count goldens are one class: an instrument reads a literal line and prints what is there, and what is there has moved. `r2-ch16p2` declares `A, B = 9030, 9156` and prints `L9030 ### 32.2 Self-reference` banked and `L9030 Register 373.` now. `r2-ch17f` declares Chapter 36 as `L9892–L9936` and Chapter 36 now begins at L9922, so its window straddles §35's tail and its "read on the join" check flips True -> False. `r2-ch16q` reports a six-row table as five and `r2-ch16u2` reports "Six vs 4 MISMATCH": the table at L9174–L9182 still has six rows and §35.5's ledger seven; the windows moved, the tables did not. `r2-ch16c` finds Schmid at [8574] where it found [8572, 8573]: L8575 fell off the window's far edge. `r2-ch18a` resolves its eight adjunction sites semantically, every one moved +2, and then reads `L4806` by literal and labels it Theorem 17.1 — L4806 now holds "**And a theorem in the other direction". `r2-ch19b` finds the same eleven code lines moved +30 and attributes "A.19's rank table 0 lines" where it said 7 — a sub-window that no longer contains them, so the banked sentence would be false and it is held. These are W-207's "positional invariant" class and need successors cross-checked against an independent resolution, not re-banks. About 45 instruments including the 13 UNRUNNABLE.
- **A near-miss, owned.** `r2-ch18a` was in the re-bank list on a residual check that printed the first hundred characters of each differing line; the false label sits past the hundredth. The first BUILD202 was built with it, caught by the golden coming out 47 bytes shorter — a pure line shift cannot shrink a golden — and discarded before install. The check now prints the divergence point in full.
- **The guard's blind spot, found on my own work.** `r2-ch17e2`, seated this session as `r2-ch17e`'s successor and proved by `proveanchor` byte-exact on the pre-shift bundle, still reads `'E(Λ₃) = 0' in M[6999]` — a literal index. The text is at L7002 now and the check returns False. `proveanchor` reproduces a predecessor's golden on the bundles it was proved against; a positional read that was CORRECT at proof time passes it and fails on the next shift. The tool proves re-anchoring reproduced the old reading; it cannot prove the new anchor is semantic. Recorded as DEF-153N.
...
### W-214 — R3 — Book repairs and corrections — the positional class re-anchored by generator: forty-four successors seated, each proved byte-exact on the bytes its predecessor was banked against. BUILD207 -> BUILD208

- **One shape, one tool.** Every instrument of the class pins a unit as a literal line; `tools/reanchor.py` replaces each literal with the text of the line it pointed at in the golden's own banking bundle, resolved at run time by content, and `tools/proveanchor.py` proves the successor on those bytes. Thirty-nine proved on BUILD90 + BUILD184, five on BUILD94 + BUILD198 (the successors chat 153 seated after r3-wl2's shift), two with one stated exception each (`r2-ch16b2`'s self-check of its own source; `r2-ch34re2`'s verdict line, now tolerant of a sentence §34.6 no longer prints). READ-succ2-153R.md carries the table.
- **The tool's blind spot, met and named.** A number in a line's range that is not a line — an entry number, a count, a seed — anchors to that line's text and proves on the old bytes regardless. The first round let `ent[1717]` and `rbody(1722)` through and they failed live; the keep lists and an auto-keep for entry-number contexts are the operator's half of the contract, and the contract is in the docstring.
- **What the goldens now carry.** Line references and member statistics; counts the appended entries move; and in seven successors the content R3 changed under the instrument — §34 at BUILD96, §32.1's readouts and Appendix G's rows at BUILD100 — banked as the volume now stands, each named in the READ.
- **Held, still:** `r2-26c2`, `r2-ch18b`, `r2-reg1a`, `r2-reg7a`, `kinds`, `r2-regsweep`, `r2-26b`, `r3-wl`, for the reasons W-213 and DEF-151r3 state. The predecessors are superseded, not withdrawn.
