...
print('  L9646 "1.120, 1.049, 1.022, 1.002 at n = 3, 4, 5, 6": numerals %d, n-values %d; ioi L1759 p row %s'
      % (len(re.findall(r'\d\.\d{3}', MAIN[_L('**And it is a limit approached along n**, not a constant: the p row of Λ_t reads')])), 4, re.findall(r'\d\.\d{3}', IOI[1758])))
...
print('    register 1355 headline: "%s" -- a fourth blocked fit, named nowhere in the unit' % norm(REG[_L(' slack was zero. **Two points are not a pattern**, and it is stated sharply enough to be attacked; if')])[:90])
