...
# r2-ch26b2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch26b.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (4 anchors); nothing else changes. r2-ch26b.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch26b.out byte-exact on those bundles (G0c)
...
print('  D.5.3 L10616 "Clause (iii) is the boundary between this appendix and Appendix F": F.2 (iii) L11263 "a function of the ground alone"; D.5.3 "(iii)" %d' % cnt(M[_L('### D.5.3 What makes something an element, and the five that test it'):_L(" will make this appendix's closure claim weaker in exchange. Registers 232 and 233.")], r'\(iii\)'))
...
print('  §32.1.4 row\'s context (L8972–L8976):'); [print('    L%d %s' % (i + 1, M[i].rstrip()[:110])) for i in range(_L('  Appendix D, unfibred                       3   4     four'), _L('  the audit set, at four coordinates         4  17     seventeen', 1))]
