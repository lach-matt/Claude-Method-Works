...
# r2-ch25b2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch25b.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (7 anchors); nothing else changes. r2-ch25b.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch25b.out byte-exact on those bundles (G0c)
...
show([70, _L('**Search before deriving closed the endgame.** §E.5 exists because nine things')], 'SEARCH BEFORE DERIVING outside the unit (E.7 L11209 *§2.10 … now does*):')
show([_L(' **And the reorderability material claims nothing.** The problem is known: Stahl & Wille proved 2-dimension NP-complete in 1984; Yannakakis proved dimension ≤ 3 NP-complete in 1982; Habib, Nourine, Raynaud & Thierry surveyed the computational aspects in *Theoretical Computer Science* **312** (2004), 401–431. **The work reported in Chapter 29 was done before that literature was located**, and is retained only for what it records about method.'), _L(' The language is NP-complete — it contains exactly-one-of-three and no Schaefer class covers it, with'), _L(' §E.6 records that the complexity question may already be settled in Ryter & Schmid 1994 or')], 'NP-complete / Ryter sites in §29.8, §30.3.8, §30.3.9 (E.6 L11196 *This book has not read it*):')
...
show([_L(" cells, exactly three quarters at every dimension. Rival's 1973 bound on maximal sublattices is \\|K\\| ≤ (3/2)\\|L\\|; the"), _L(" And this is Rival's theorem. His 1973 bound is \\|K\\| ≤ (3/2)\\|L\\| for a maximal sublattice;"), _L(" Chapter 15, and the 3/2 bound on maximal sublattices contains §30.3's measured 3/4.")], '3/2 in §30.3.4 and 3/2 · 3/4 in Appendix G (E.7 L11204 *Rival\'s 3/2 bound contains the measured 3/4*):')
