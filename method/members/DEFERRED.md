...
## Chat 153 --- deferred out of the cypher build and the Λ audit (Claude Code repository session; not a section read; not a Method chat)

1. **Λ_spectra's superseded state cannot be checked, and the check is closed as unavailable rather than owed.** Register 1177's *70% of the faces and 28% of the corners* is measured against Λ_spectra at E(order) = 6,195. Λ_spectra proper closes at E = 0 now, and the pre-Hund cell set is not in the tree. Docket 15's class, retired-basis, at a new object. If a superseded cell set exists outside the tree the check becomes available again; nothing else makes it so.
2. **The violation index is not buildable from what is printed, confirmed.** Its nine letters and rungs reproduce the 19,440 box exactly, and the arity-2 variant `NEC >= 3 -> U >= 1` reproduces E = 0. The printed arity-3 constraint taken as a definition gives 18,792 cells at E = 648 against the recorded 2,370 at E = 30, so the constraint is a model applied to the index and not the index. No cell set was fabricated to close the gap.
3. **Λ₁₁ to Λ₁₃ are owed two envelopes, not compute.** The pruned search reaches 47 million ambient cells. What is missing is `2J_c <= φ̂(k)` and `2K <= 2J_c + 2f_max` stated to the precision a program needs; the volumes say the exact sets are not the obvious ones. Λ₈, Λ₉, Λ₉′ and Λ₁₀ are fixtures and reproduce at 976, 1,654, 1,561 and 2,535, each with E = 0 in all five operator-bearing languages, which is stronger than the order-only figure recorded.
4. **No index in the Index of Indices carries a recorded per-language geometry E,** so `geometry` stays ADOPTED rather than PINNED. What stands behind it is E = 0 on Λ, on the box ordering and at every reconstructible tower stage, where the compendium independently states the polytope's integer points ARE the lattice, together with readings distinct from every other language on four open indexes — Kreuzer–Skarke 545 against order's 540, the nuclide chart 12 against 9, the channel index 75, the periodic table 83.
5. **Λ's own seed is not computed and is not owed.** Recorded at 7; the minimum-seed problem is NP-hard and a greedy search did not find a 7-cell generator within budget. That is a limit of the search, not evidence against the 7. The box seed law `d + c - 2` and its Carathéodory lower bound are asserted in the selftest and hold.
6. **The compendia MANIFEST was stale between the two closes of this chat** and is repaired by this one. `close_main.py` reports the staleness rather than repairing it, because the manifest is the compendia bundle's to regenerate; any future main build leaves the same two rows stale until the next `close.py` run passes `--main` the new path.
7. **Next:** the audit covers Λ and the objects reconstructible beside it. The same instrument applied to the Physics Compendium, the Spectra Compendium and the Index of Indices has not been run, and the Mathematical Compendium is covered only where its claims are about Λ.

## Chat 153 --- deferred out of the audit of the mathematics across five volumes (Claude Code repository session; not a section read; not a Method chat)

1. **The Register is not audited.** 1,635 entries at BUILD91, 1,642 at BUILD92; the largest volume and the one the other five cite. The census carries 1,556 rows against it. Every instrument the audit needs now exists; what is owed is the pass.
2. **Three claims are unreconstructible from print and are recorded, not skipped.** Lambda_const's *E = 0 at 2 of 576* needs the per-constant (role, carrier) assignment, printed in neither the Mathematical Compendium nor the Index of Indices. Lambda_spectra's *596 channels across 70 spectra, 2,269 interior cells* needs the channel data, which is not in the tree. Lambda_phys's *objects rest on it* counts are defined by register 1740's rule over the Mathematical Compendium's own dependency graph, which is encoded nowhere a program can read.
3. **Entry 1799 leaves a question the table cannot answer.** How many of the six 7p rows fail the 0.083 clearance cannot be read off, because the unwitnessed-twelve table prints each band's range and not its rows. Printing the per-row margins settles it and is the repair the entry names.
4. **E = 1 is unreachable for Lambda_law** --- the defect distribution over all 120,960 orderings runs 0 then 2 upward. Measured this chat, recorded nowhere in the volumes, and not a defect.
...
## Chat 153 --- deferred out of the main volume's structural pass (Claude Code repository session; not a section read; not a Method chat)

1. **No volume has been read entry by entry or line by line.** Every pass this chat is structural or numeric, against figures the volumes themselves record. The per-entry read of the Register and the per-section read of the main volume are the chat-81 cadence's work and are untouched by any of it.
2. **Entry 1801's repair is two characters twice** --- the `## ` marker on the contents lines for chapters 20 and 21. It is deliberately not applied here: the audit adjudicates nothing, and a contents line is main-volume text.
3. **The audit family is complete across six volumes and is re-runnable.** `audit_lambda.py`, `audit_math.py`, `audit_mc.py`, `audit_pc.py`, `audit_sc.py`, `audit_ioi.py`, `audit_main.py`, `register_counts.py`. Each exits 1 while a finding stands, so a repair is confirmed by the exit code changing.
4. **Sixteen checks failed and were the audit's own fault, against nine findings.** Every one reconciled exactly, and in each case from a convention the volumes state in their own words --- Appendix A's own second sentence, section IX's second sentence, the note under Lambda_phys's table, `kinds.py`'s own docstring. That ratio is the reason a FAIL is read twice before it is believed, and it should be expected to continue.
5. **What remains unreconstructible is unchanged:** Lambda_const's 576 orderings, Lambda_spectra's 596 channels, Lambda_phys's *objects rest on it* counts, Lambda_spectra's superseded state, the violation index's cells, and the two tower envelopes above Lambda_10.
