...
# r2-ch20a2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch20a.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (17 anchors); nothing else changes. r2-ch20a.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch20a.out byte-exact on those bundles (G0c)
...
mp = norm(' '.join(M[_L('  argument; the rest are data and live where data belongs.*'):_L('  1763, 1768).')])); sc = S[993]
...
print('  main L10195 opens with the volume title and `Part II`: %s; SC Part II heading `# II · THE CHANNELS` at S%d: %s' % ('Part II' in M[_L('  argument; the rest are data and live where data belongs.*')], p2, S[p2 - 1]))
...
print('  1,442 − 1,105 = %d (Chapter 24 L6970: "Of the 1,442, the 1,105 added in this work"); the appendix\'s 763 + 1,105 = %d; 763 + 1105 − 1442 = %d' % (_L('        before it suspects the text') - _L(' every build. **These are readouts and are marked as such** — a number the artefact computes about', 1), 763 + _L(' every build. **These are readouts and are marked as such** — a number the artefact computes about', 1), 763 + _L(' every build. **These are readouts and are marked as such** — a number the artefact computes about', 1) - _L('        before it suspects the text')))
...
for n in (_L(' He(2), Ne(10), Ar(18). **E measures what that choice costs to specify, and the difference is'), _L(' That matters because a closure defect measured once can always be blamed on the boundary. **Here the'), _L(' boundary was moved four times and the nine cells did not move**, so they are a property of the'), _L(" **Seven constraints, four origins, and nothing else.** No constraint in Λ comes from anywhere but Pauli, the hydrogenic radial solution, counting, or angular-momentum coupling. That exhaustiveness matters in Part III, where it is the reason the object's properties can be attributed to *indexing* rather than to physics smuggled in through a bound.", 1), _L(' not appear here, because a set cut out by a sum bound is not closed.', 1)):
...
print('  1763 names 658 of 813 / 155 / 318 rows: %s; 1768 names 75 of 81 / 35 / 10 no-triple: %s' % ([x in rbody(_L(" **Seven constraints, four origins, and nothing else.** No constraint in Λ comes from anywhere but Pauli, the hydrogenic radial solution, counting, or angular-momentum coupling. That exhaustiveness matters in Part III, where it is the reason the object's properties can be attributed to *indexing* rather than to physics smuggled in through a bound.", 1)) for x in ('658 OF 813', '155', '318')], [x in rbody(_L(' not appear here, because a set cut out by a sum bound is not closed.', 1)) for x in ('75 OF 81', '35 CLOSE', '10 CLOSE NO-TRIPLE')]))
