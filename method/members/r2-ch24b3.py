# r2-ch24b3.py — SUCCESSOR to r2-ch24b2.py (md5 6a7eb0fd0dfd0ecee1ac18bda62fea64), class DEF-153N (W-234): the Register's front
# matter grew by two lines at BUILD110 (rows at L50–L51), so every Register line from 50 moved by +2. The predecessor's two
# Register sites (§4, 'Register L%d:') were r2-ch24b.py's literals 1798 and 1936, which tools/reanchor.py re-keyed on the text of
# the MAIN-volume lines that happened to sit at those numbers (_L(' shown identical …', 1) -> 1798; _L(' antichain, …') -> 1936)
# — a positional read of the Register keyed on another volume. Replacements, and nothing else changes:
#   L105  _L(' shown identical across a twenty-eight-fold range in cell count.', 1)   ->  _Rb(482)   (register 482's body, the line the ORIGINAL r2-ch24b read at L1793 — not the '### 483' the re-anchored predecessor's golden shows)
#   L105  _L(' antichain, or fail to — run at last past three coordinates, …')        ->  _R('### 518')   (the heading the ORIGINAL read at L1931)
# The members directory is H = os.path.dirname(os.path.abspath(__file__)) (already so in the predecessor: _L, load(), r2lib.read_member);
# /home/claude/PP_The_Method_1_6.md (Prints & Proofs) is kept. Golden on the pre-shift bundle (BUILD109) must equal r2-ch24b2.out byte-exact (G0c).
...
# --- Register locator (DEF-153N successor): a Register line found by the entry heading `### n`, never by a number ---
def _R(t, d=0):
    h = [i for i, l in enumerate(R, 1) if l == t]
    assert len(h) == 1, ('re-anchor: Register line text is not unique or is absent', t[:60], h)
    return h[0] + d
def _Rb(n):   # line number of the first non-blank line after `### n` — the entry's body line (rbody's line, not its text)
    i = _R('### %d' % n)
    return next(j for j in range(i + 1, len(R) + 1) if R[j - 1].strip())
...
for n, pat in ((_Rb(482), r'518'), (_R('### 518'), r'518')): print('   Register L%d: %s' % (n, R[n - 1][:220]))
