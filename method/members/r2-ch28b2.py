...
for nm in ('Löwdin', 'Koelling', 'Harmon', 'Griffin', 'Andrew', 'Cowan', 'Pulay', 'Gerratt', 'Mills', 'Madelung'):
    print('  Löwdin block %-9s Chapter 35 %s %d; Register 1701–1712 %d' % (nm, c35, len(re.findall(r'(?<![A-Za-z])' + nm + r'(?![a-z])', j35)), sum(1 for i in range(len(R)) if nm in R[i] and any(R[k].strip() == '### %d' % n for n in range(_L(' measurement rather than of the window — and each of the nine is a real nuclide the chart omits because'), 1713) for k in range(max(0, i - 3), i + 1)))))
