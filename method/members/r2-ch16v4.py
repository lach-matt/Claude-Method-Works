# r2-ch16v4.py — successor of r2-ch16v3.py (md5 80806e1ad8cb6f7cb8e444ba4891ae81), W-235 / DEF-153N: two Register reads, REG[5104] and REG[5016] in the
# original r2-ch16v2, were re-anchored by tools/reanchor.py as REG[_L(<main-volume text>)] — Register indices keyed on MAIN text
# (its documented blind spot) — so after main moved +5 above L1452 the predecessor printed register 1360's heading for 1359's
# body and '### 1337' for register 1336's tail, and its golden banked them. Two replacements: the reads are keyed on the
# entries' own headings (_RE); and the members directory is this script's own. Nothing else changes.
#!/usr/bin/env python3
...
# r2-ch16v3.py — R3 (chat 153-R) — SUCCESSOR to r2-ch16v2.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (10 anchors); nothing else changes. r2-ch16v2.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 94+198 — reproduces r2-ch16v2.out byte-exact on those bundles (G0c)

# --- re-anchoring helper (tools/reanchor.py): a main-volume line found by its own text, never by a number ---
def _RE(n):
    """1-based line of the Register heading '### n', read from this script's own directory — the content key for a Register
    read the predecessor keyed, through _L, on the MAIN volume's text at the same line number (reanchor.py's blind spot)."""
    import os as _o
    global _REG_
    try: _REG_
    except NameError: _REG_ = open(_o.path.join(_o.path.dirname(_o.path.abspath(__file__)), 'The_Method_1_6___The_Register-2.md'), encoding='utf-8').read().split('\n')
    h = [i for i, l in enumerate(_REG_, 1) if l.strip() == '### %d' % n]
    assert len(h) == 1, ('successor: register heading absent or not unique', n, h); return h[0]
def _L(t, d=0):
    import os as _o
    global _LM
    try: _LM
...
a = norm(' '.join(MAIN[_L('the most.', 1):_L('how the method itself did the work.*')]))
b = norm(' '.join(MAIN[_L('Schrödinger equation. It has stood since. This chapter states the challenge, what', -2):_L('the method delivered, and how the method itself did the work.*')]))
...
print('    ' + norm(' '.join(MAIN[_L('*Parts I to VI state the method. This part applies it. Each chapter takes a', -1):_L('the method is the subject — how the method itself did the work.*')])))
print('  register 1359 (reg L5105) fixes Part VII\'s structure:')
print('    ' + norm(REG[_RE(1359) + 1])[:260])
...
print('    reg L5017 tail: ' + norm(REG[_RE(1336) + 1])[-60:])
