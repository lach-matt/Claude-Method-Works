...
# r2-ch18a2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch18a.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (5 anchors); nothing else changes. r2-ch18a.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch18a.out byte-exact on those bundles (G0c)
...
print('  A.6 "adjunction never repairs closure" — where the claim lives: main lines with adjunction/adjoin and their sections:', [(i, L.enclosing(M, i)) for i, l in enumerate(M, 1) if re.search(r'adjunction|adjoin', l, re.I) and i < _L(' Serving PART II — THE LATTICE. The structural theorems the lattice chapters assert and do not prove in', -1)], '; L4806:', norm(M[_L(' **And a theorem in the other direction:**', 1)])[:90], '; Appendix D L10558 names Theorem 17.1 as A.6:', 'Theorem 17.1 is A.6' in M[_L(' apparent candidates are duplicate labels for elements already listed — Theorem 14.1 is A.2 by its own')], '— the index row sends A.6 to §24.2, a table of contributors (docket 9a, wrong target)')
...
print('  A.11 "the bracket of Part V operates along chains" — Part V heading:', [(i, M[i - 1].strip()[:60]) for i, l in enumerate(M, 1) if re.match(r'^# PART V\b', l)], '; A.9 Rota 1964 in the References body:', [(i, M[i - 1][:70]) for i, l in enumerate(M, 1) if i > _L(' The last entry is a cell of the index referring to the index. It is not a joke; it is the fixed point made visible,') and re.search(r'Rota, G', l)], '; "crosscut" in References:', [i for i, l in enumerate(M, 1) if i > _L(' The last entry is a cell of the index referring to the index. It is not a joke; it is the fixed point made visible,') and 'crosscut' in l])
