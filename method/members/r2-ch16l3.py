...
# r2-ch16l3.py — R3 (W-234, DEF-153N) — SUCCESSOR to r2-ch16l2.py (md5 ef7c1889e4351c6fc69276c0a832b21f), the positional class:
# the Register's front matter grew by two lines at BUILD110 (two rows at L50–L51), so every Register line from 50
# moved by +2, and the predecessor's three section-G Register sites — L1366, L1368, L2418, produced by _L() calls
# keyed on MAIN-volume text that happened to sit at those numbers — now print another line's words. Replacements:
#   (1) section G: the three Register sites are keyed on the Register's own text through _R(text) — the words the ORIGINAL r2-ch16l read (its third site, register 648's headline, not the '### 652' the re-anchored predecessor's golden shows) — the entry
#       headings '### 368' and '### 652', and the full text of register 368's ITEM M sentence — never a number;
#   (2) r2lib is imported by path from H = os.path.dirname(os.path.abspath(__file__)), not /home/claude/members;
#   (3) the six volumes (V) and DEFECT-CENSUS.tsv are opened from H, not /home/claude/members;
#   /home/claude/PP_The_Method_1_6.md (Prints & Proofs) is kept. Nothing else changes — not a print, not the order,
# not a word. r2-ch16l2.py is seated and never edited in place (chat 68). G0c: this file must reproduce
# r2-ch16l2.out byte-exact on the pre-shift bundle (BUILD109 main + Register) before it is trusted on the live one.
...
# --- W-234 (DEF-153N): a REGISTER line found by its own text, never by a number ---
def _R(t, d=0):
    import os as _o
    global _LR
    try: _LR
    ...
    h = [i for i, l in enumerate(_LR, 1) if l == t]
    assert len(h) == 1, ('re-anchor: register line text is not unique or is absent', t[:60], h)
    return h[0] + d
...
for i in (_R('### 368'), _R("**ITEM M ANSWERED, AND THE ANSWER IS ABOUT E(X) RATHER THAN ABOUT THE NUMBERS.** *After register 367's constraint and one paragraph written to standard, E(G) stands at 23 — and* **seventeen of those twenty-three are dominated* **, each weaker on all three axes than a cell its own fibre already occupies. Filling them would mean writing worse paragraphs.* **E(X) counts what a structure admits without asking whether occupying it is an improvement* **, so on an ordered index the admitted-and-absent set holds cells below the object's practice as readily as above it. That is* **a third resolution §12's loop does not offer** *— not a number that belongs there, not a missing constraint, but a cell correctly left empty.* **The actionable count is six* **, five of them* *under variation**, which is one computation each at several caps. 368a.* **A figure about the book cannot be printed as prose.** *E(G) read 33, 38, 24, 23 and 21 within one session, and every move was caused by writing into the book it measures — the index is computed from the text, so the analysis of the index is part of its own population. The fixed value is removed from all four places it stood and* **the press recomputes it at every build* **, beside the build-time tripwire of §32.1.2. That is item M's real closure: not resolving the cells but ending the staleness, since a count that changes when it is discussed can only be printed by the thing that prints the book.*"), _R('**THEN EACH COMPENDIUM WAS CHECKED AGAINST ITS OWN CLAIMS, AND TWO FAULTS APPEARED THAT NOTHING ELSE COULD SEE.** **K.twocol cited a dependency on `T.cat`, which does not exist** *— the object is `K.cat`, and the compendium was printing a dependency on nothing.* **And two different entries were both numbered 368* **, one on Q item M and one on figures about the book.* *No audit caught it because audit 19 checks that entries ASCEND, and a duplicate satisfies ascending.* **Audit 19 now checks uniqueness, and the second is renumbered 368a.**')):
...
rows = [l for l in open(os.path.join(H, 'DEFECT-CENSUS.tsv'), encoding='utf-8').read().split('\n')[1:] if l.strip()]
hdr = open(os.path.join(H, 'DEFECT-CENSUS.tsv'), encoding='utf-8').read().split('\n')[0].split('\t')
