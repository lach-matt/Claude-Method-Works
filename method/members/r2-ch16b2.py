#!/usr/bin/env python3
# r2-ch16b2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch16b.py, re-anchored by tools/reanchor.py: every main-volume line the
...
print('  POPULATION measured %s   printed 2,354   MATCH %s' % (f'{pop:,}', pop == _L(' §12.1 finds the shape is a cylinder over the transfer coordinate. That is not a metaphor, and the')))
...
print('  "47.8 s against 86" -- the 86 carries NO UNIT on the line: %s'
      % M[_L('      2^(d−2), collect signatures, decide by lookup.', 1)].strip()[:96])
...
print('  gives 493 as the only population figure on the line: %s' % M[_L(' Local search through valid orders. The valid set is DISCONNECTED — 1.66 to 2.19 components,')].strip()[:96])
...
for i in (_L('    Three walls: hardness blocked at realisability, tractability blocked at the completeness of'), _L('    the hypergraph, and the FPT bound between them as the best known.')):
