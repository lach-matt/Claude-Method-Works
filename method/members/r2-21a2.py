...
# r2-21a2.py — R3 (chat 153-R) — SUCCESSOR to r2-21a.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (14 anchors); nothing else changes. r2-21a.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-21a.out byte-exact on those bundles (G0c)
...
ptrs = []
for a, b in [SPANS['25.2'], SPANS['25.6'], SPANS['32.5.1'], C3, (E3[0], E4u[0] if E4u else E3[1]), (_L(' Why Rule 4a exists'), _L('    It is an error.', 1)), (_L(' forty-eight.'), _L(' 50. A conservation law that is not one. w·V = 8y′²/y″ claimed independent of h. Exact evaluation')), SPANS['18.6.3']]:
    for i in range(a, b):
...
tot = 0; lines = 0
for label, (a, b) in [('§25.2', SPANS['25.2']), ('§25.6', SPANS['25.6']), ('§32.5.1', SPANS['32.5.1']), ('C.3', C3), ('E.3 item A', (E3[0] + 4, E3[0] + 7)), ('§22.2.1 clause site', (_L(" **Sc VI's ⁴S°*n*s channel is the case that forced the clause.** With δ(4s) = 1.0057 and δ(5s) = 0.9812, the constant form gives **δ̄ = 0.9934 — above δ(5s)**, which a decreasing channel forbids. The resulting estimate falls **outside the deductive bracket its own monotonicity implies** (§25.6)."), _L(" **Sc VI's ⁴S°*n*s channel is the case that forced the clause.** With δ(4s) = 1.0057 and δ(5s) = 0.9812, the constant form gives **δ̄ = 0.9934 — above δ(5s)**, which a decreasing channel forbids. The resulting estimate falls **outside the deductive bracket its own monotonicity implies** (§25.6).", 1))), ('§28 item 49', (_L(' **49. The Sc VI prediction.** Computed with a constant defect δ̄ = 0.9934 exceeding δ(5s) = 0.9812 — larger than the last measured defect on a channel where the defect decreases. **The estimate fell outside the bracket its own channel implies.** Rule 4a now forbids the form; §25.6 states the bracket.'), _L(' **49. The Sc VI prediction.** Computed with a constant defect δ̄ = 0.9934 exceeding δ(5s) = 0.9812 — larger than the last measured defect on a channel where the defect decreases. **The estimate fell outside the bracket its own channel implies.** Rule 4a now forbids the form; §25.6 states the bracket.', 1))), ('§18.6.3 cell claim', (_L(' The Sc VI 6s cell of §25.6 is in Λ — its coordinates are ordinary integers, and the lattice admits it without comment. **Its energy is not**, and no closure property supplies it.'), _L(' The Sc VI 6s cell of §25.6 is in Λ — its coordinates are ordinary integers, and the lattice admits it without comment. **Its energy is not**, and no closure property supplies it.', 1)))]:
    for i in range(a, b):
...
print('  E.3 item A (L11058–L11059) = §25.6.3\'s bracket, window and eV token for token:', all(bounded(f).search(M[_L(' §12.11.1. Both repaired, and audit 15 extended so the pair cannot drift again.')]) or bounded(f).search(M[_L(' A — Sc VI 3s²3p³(⁴S°)6s ³S°₁. Committed prediction, unmeasured. [735,860, 737,380] cm⁻¹ ·')]) for f in ('735,860', '737,380', '13.5615', '13.5895', '91.338')), '| the point value 736,688 is absent from E.3 (DEF-138 item 12 re-measured):', not any(bounded('736,688').search(M[i - 1]) for i in range(E3[0], E3[1])))
...
print('  membership of the printed 4s / 5s / 6s coordinates cannot be tested cell by cell: no 8-tuple is printed at L5355, §25.6 or E.3 (MEASURED: no parenthesised 8-tuple of integers in those spans:', not any(re.search(r'\(\s*\d+(\s*,\s*\d+){7}\s*\)', M[i - 1]) for a, b in (SPANS['25.6'], (_L(' The Sc VI 6s cell of §25.6 is in Λ — its coordinates are ordinary integers, and the lattice admits it without comment. **Its energy is not**, and no closure property supplies it.'), _L(' The Sc VI 6s cell of §25.6 is in Λ — its coordinates are ordinary integers, and the lattice admits it without comment. **Its energy is not**, and no closure property supplies it.', 1)), E3) for i in range(a, b)), ')')
