...
**THE PAPER THAT NAMES THIS BOOK'S CONSTRAINT CLASS IS CITED UNDER TWO DIFFERENT SECOND AUTHORS, AND ONE OF THEM IS A DIFFERENT PERSON.** *Constraint satisfaction over connected row-convex constraints,* **Artificial Intelligence 109(1-2) (1999) 243-271** *— the staircase class of §14.1, the (α,β)-monotone constraints φ̂ recovers — appears sixteen times as* **Deville, Barette & Van Hentenryck** *and six times as* **Deville, Barták & Van Hentenryck**. *The two full bibliographic records agree in title, journal, volume, issue, year and pages, so they name one paper; and Roman Barták is a different worker in the same field, which is why the second form reads without a flinch.* **The published record settles it, three times independently: Kong, Li, Li & Long, *On tree-preserving constraints*, Ann. Math. Artif. Intell. (2017); Zhang & Freuder, *Tractable Tree Convex Constraint Networks*, AAAI-04; and *Exploring Directional Path-Consistency for Solving Constraint Networks* (arXiv:1708.05522) — three reference lists in three typesettings, "Hentenryck, P.V.", "Van Hentenryck, P." and "van Hentenryck, P.", every one giving the second author as Barette, O., with the same volume, issue and pages.** *Nothing published anywhere gives Barták for this paper.* **Five sites take the correction: §14.1's naming of the constraint class and the References entry, which also takes the initial the sources give — Barette, O.; the Mathematical Compendium's *Staircase / connected row-convex* and the prior art of its generation criterion; and the Physics Compendium's prior-art chain for the seed.** *A sixth stands uncorrected and is this entry's own subject: register 401 prints the wrong form in its headline, and is superseded here rather than altered, so both states remain in the record.* **The count is not the evidence and was pointing the other way.** *Three volumes printed the wrong form against one printing the right one, and that one contradicted itself, so by weight of sites the correct volume was the outlier. A majority of citations is not a fact about a name.* **Register 1736 removed ten bibliography rows by matching this book's works against the References by author and year; that match cannot see a wrong year on a real author, and cannot see a wrong author on a real paper.** *This is the first of the second kind.* Registers 400; 401; 1736. (a correction.)"""
