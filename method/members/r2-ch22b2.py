...
# r2-ch22b2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch22b.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (5 anchors); nothing else changes. r2-ch22b.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch22b.out byte-exact on those bundles (G0c)
...
print('   PART VI heading:', [(i, M[i - 1][:60]) for i, l in enumerate(M, 1) if re.match(r'^# PART VI\b', l)], '; L10321 serving line:', M[_L(" Serving PART VI — THE RECORD AND THE REACH. The book's own mathematics as a closed index — seventy-seven", -1)][:70])
...
print('   7 kinds × 6 languages = %d (printed forty-two)' % (len(M[_L('  coordinate          values                                                                         kind')].split('·')) * len(M[_L('  kind                definition · mechanism · formula · theorem · law · method · measurement        fibre')].split('·'))), '; D.2 table rows with ≥3-space gap:', [i for i in range(_L('  coordinate          values                                                                         kind'), _L('  precedent           none found < found                                                             ordered', 1)) if re.search(r'\S {3,}\S', M[i - 1])])
