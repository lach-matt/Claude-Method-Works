#!/usr/bin/env python3
# r2-ch16z3.py — SUCCESSOR to r2-ch16z2.py (md5 80558823a4aa42b3df5c8586dfae7b27), class DEF-153N (W-234): the Register's
# front matter grew by two lines at BUILD110 (rows added at L50–L51, every line from 50 moving by +2), and this instrument read
# the Register at literal line indices. Every literal index is replaced by a content-keyed locator; nothing else changes.
# Replacements (predecessor line numbers):
#   L22   H = '/home/claude/members'                       -> H = os.path.dirname(os.path.abspath(__file__))   (members resolved from this file)
#   L130  IOI[1758]                                        -> IOI[_V(IOI, '| **p** | · | 1.120 | 1.049 | 1.022 | 1.002 | · | **1.0000** |')]
#   L136  REG[5016]                                        -> REG[_R(1336)]   (register 1336's headline, by its heading "### 1336")
#   L141  REG[_L(' slack was zero. **Two points ...')]     -> REG[_R(1355)]   (a Register index keyed to a MAIN-volume line; now register 1355's headline)
#   L143  REG[5020]                                        -> REG[_R(1337)]
#   L145  REG[4996]                                        -> REG[_R(1331)]
#   L148  REG[5032]                                        -> REG[_R(1340)]
#   L190  REG[5032]                                        -> REG[_R(1340)]
# Hard-coded L-numbers inside printed labels (L9624, L9646, "ioi L1759", "register 1331 L4997", "register 1340 L5033", ...)
# are the chat-127 reading's own words and are not reads; they are left as printed (G0c).
...
# r2-ch16z2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch16z.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (47 anchors); nothing else changes. r2-ch16z.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch16z.out byte-exact on those bundles (G0c)
...
def _L(t, d=0):
    ...
    return h[0] + d
# --- DEF-153N (W-234) content-keyed locators: a Register entry found by its heading "### N", a volume line by its text ---
def _R(n):
    h = [i for i, l in enumerate(REG, 1) if l == '### %d' % n]
    assert len(h) == 1, ('re-anchor: Register heading is not unique or is absent', n, h)
    assert REG[h[0]] == '' and REG[h[0] + 1] != '', ('re-anchor: entry layout is not heading / blank / headline', n)
    return h[0] + 1  # 0-based index of the headline: heading at h-1, blank at h, headline at h+1
def _V(V, t):
    h = [i for i, l in enumerate(V) if l == t]
    assert len(h) == 1, ('re-anchor: volume line text is not unique or is absent', t[:60], h)
    return h[0]  # 0-based index
...
A, B = _L('### 34.5 The corridor'), _L('three-body problem but a statement about which pairs exist — Chapter 36 takes this up.*', 3)
...
print('  L9624 "resets eighteen times" / L9625-L9626 "(8) ... (6) ... (4)": 8+6+4 = %d; returns listed: %s -> %d'
      % (8 + 6 + 4, re.findall(r'\b[A-Z][a-z] after [A-Z][a-z]\b', norm(MAIN[_L('**subshell opening** (8), an **aufbau exception** (6), or the **return from one**')] + ' ' + MAIN[_L('(4) — Tc after Mo, Tb after Gd, Bk after Cm, Hg after Au. **It never resets')])), len(re.findall(r'\b[A-Z][a-z] after [A-Z][a-z]\b', norm(MAIN[_L('**subshell opening** (8), an **aufbau exception** (6), or the **return from one**')] + ' ' + MAIN[_L('(4) — Tc after Mo, Tb after Gd, Bk after Cm, Hg after Au. **It never resets')])))))
...
print('  L9646 "1.120, 1.049, 1.022, 1.002 at n = 3, 4, 5, 6": numerals %d, n-values %d; ioi L1759 p row %s'
      % (len(re.findall(r'\d\.\d{3}', MAIN[_L('**And it is a limit approached along n**, not a constant: the p row of Λ_t reads')])), 4, re.findall(r'\d\.\d{3}', IOI[_V(IOI, '| **p** | · | 1.120 | 1.049 | 1.022 | 1.002 | · | **1.0000** |')])))
tbl = [i for i in range(_L('| term | source |'), _L('| **the endpoints** | arithmetic on the above |', 1)) if MAIN[i - 1].startswith('|')]
...
print('  L9685 "The domain protocol blocked four fits" -- fits named in L9685-L9690: %s'
      % re.findall(r'blocks the ℓ-spread[^,]*|the amplitude Gaussian[^,]*|six occupancy slopes[^—]*', norm(' '.join(MAIN[_L('parameters were competing to represent them.*', 1):_L('**and passes the same two objects measured per cell.**')]))))
rep = re.search(r'Replayed against today:(.*?)\*', REG[_R(1336)]).group(1)
...
print('    register 1355 headline: "%s" -- a fourth blocked fit, named nowhere in the unit' % norm(REG[_R(1355)])[:90])
print('  L9692 "Λ_chem dissolved two residuals": named -> 1.029 factor, missing f test = 2; register 1337 "Not derived": %s'
      % re.findall(r'a common factor of 1\.029|no f test', REG[_R(1337)]))
print('  L9698 "Seven of eight rules give 106/106, and random interior points on 200 of 200 seeds": register 1331 L4997 -> %s'
      % re.findall(r'seven give 106/106|200 of 200 seeds|74/106', REG[_R(1331)]))
...
print('    register 1340 L5033 keeps ℓ(ℓ+1) [gate, collapse, barrier, Seaton, t(ℓ)] and 2ℓ+1 [1/(2ℓ+1), h₀√(2ℓ+1)] as TWO objects: %s'
      % re.findall(r'1/\(2ℓ\+1\)[^,]*|h₀√\(2ℓ\+1\)[^,]*|ℓ\(ℓ\+1\) a new discovery[^,]*', REG[_R(1340)]))
...
print('  "the cypher" -> §33 "%s" at L%d; "the domain protocol" main sites %s (§33 L9486 = %s)'
      % (htitle(MAIN[heading_line(MAIN, '33') - 1]), heading_line(MAIN, '33'), sites(MAIN, r'domain protocol'), enclosing(MAIN, _L('boundary* — the fourth question of the domain protocol, and the one that blocks'))))
...
print('  "the one microscope" (L9702): register 1340 L5033 "%s"' % norm(REG[_R(1340)])[:110])
...
print('  restatements of the unit\'s figures elsewhere in the main volume: 104 of 106 %s ; "the corridor" appendix rows %s'
      % (sites(MAIN, r'104 of 106'), [i for i in range(_L(' constraints before anything was added: E = 0 in all twenty-two fibres. Every fibre and coordinate'), _L('  the limit — Λ_spectra closes at the last species, E = 11  measurement · combinatorics  measured · exhaustive · none found')) if 'corridor' in MAIN[i - 1] or '104 of 106' in MAIN[i - 1]]))
for i in (_L('what that costs:** *what a closure cannot supply is exactly what requires an', 1), _L('what that costs:** *what a closure cannot supply is exactly what requires an', 2), _L('  new element                                            fibre                        coordinates', 1), _L('  a_cross = (√(n−1) + √(n−4))/3                          formula · analysis           proved · exhaustive · none found', -1), _L('  the selection-rule falsification, 200 of 200 seeds     measurement · analysis       measured · sampled · none found', 1)): print('    L%d %s' % (i, norm(MAIN[i - 1])[:120]))

# ---------------------------------------------------------------- 10. attributions
hr('10  ATTRIBUTIONS -- against the ## References BODY (L11503-L11855) and R.7, both directions')
refbody = MAIN[_L(' and its presence is what makes ℛ(index) = index rather than merely index ⊆ ℛ(index).', 1):_L(" not repeated here (register 1736). The two companion papers carry their own reference lists; §R.7's", 2)]
for who in ['Pauli', 'Schrödinger', 'Seaton', 'Löwdin', 'Madelung', 'Gaussian']:
    unit = [i for i in range(A, B + 1) if who in MAIN[i - 1]]
    ref = [_L(' and its presence is what makes ℛ(index) = index rather than merely index ⊆ ℛ(index).', 2) + k for k, l in enumerate(refbody) if who in l]
...
for a_, b_ in [(_L('**All 106 non-empty. Nineteen distinct surds across the whole table. No'), _L("measurement enters — only Pauli's capacity and the node count.**", 1)), (_L('*For every ns/(n−1)d competition the crossing is (√(n−1) + √(n−4))/3, giving'), _L('0.5773503, 1.0000000, 1.2168450, 1.3938270 at n = 4 to 7.*', 1)), (_L('**`a` is carried between elements and resets eighteen times.** Every reset is a'), _L('mid-subshell**, which is why each subshell fills at constant `a`.', 1)), (_L('mid-subshell**, which is why each subshell fills at constant `a`.', 2), _L('### 34.7 The entry point', -1)), (_L('**Measured 1.028 at p across four subshells and 1.785 at d across two: the ratio'), _L('radial Schrödinger equation.*', 1)), (_L('**And it is a limit approached along n**, not a constant: the p row of Λ_t reads'), _L('centrifugal value.**', 1)), (_L('**f is outside the domain and the law says so.** At any f opening p = n−ℓ−1 = 0,', -3), _L('**f is outside the domain and the law says so.** At any f opening p = n−ℓ−1 = 0,', -1)), (_L('**f is outside the domain and the law says so.** At any f opening p = n−ℓ−1 = 0,'), _L('*The missing test is forbidden by the node count, not by missing data.*', 1)), (_L('**The domain protocol blocked four fits.** Λ_phys closes with no parameter of this'), _L('**and passes the same two objects measured per cell.**', 1)), (_L('**The falsification withdrew a result.** Eighteen resets landing on exact surds'), _L('the trajectory was mine.*', 1)), (_L('**And the one microscope caught the largest thing.** ℓ(ℓ+1) had been recorded four'), _L("different discipline's name.**", 1)), (_L('*A Rydberg atom is three bodies: a nucleus, a core of Nₑ−1 electrons, and one'), _L('three-body problem but a statement about which pairs exist — Chapter 36 takes this up.*', 1))]:
