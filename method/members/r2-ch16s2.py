...
# r2-ch16s2.py — R3 (chat 153-R) — SUCCESSOR to r2-ch16s.py, re-anchored by tools/reanchor.py: every main-volume line the
# predecessor pinned as a literal is resolved by the text of the line it pointed at in the bundle its golden was
# banked against (13 anchors); nothing else changes. r2-ch16s.py is seated and never edited in place (chat 68).
# PROVED by tools/proveanchor.py when the line below says so; until then this file is a draft.
# PROVEANCHOR: 90+184 — reproduces r2-ch16s.out byte-exact on those bundles (G0c)
...
bold = [i for i in range(_L(' **The same is true of the other four things this book contributes:**', 1), _L('    The components are old. The sifting is not. Assembling a century of published')) if MAIN[i - 1].lstrip().startswith('**')]
...
for i in range(_L('      102 quantitative claims were extracted from the draft and re-derived independently from'), _L(' located the error. That is ⅅ_def operating on the text.', 1)): print(f'   {i} {MAIN[i-1]}')
...
print(f'  digit-form 1,635 sites main: {numsites(MAIN, _L(' **The nine cells are the pairing and clustering terms of the mass formula, counted.** That is the'))}')
...
print(f'  L9376 calls it a "register of {word} withdrawn claims"; 1,635 == distinct entries {len(nums)}: {len(nums) == _L(' **The nine cells are the pairing and clustering terms of the mass formula, counted.** That is the')}')
...
print(f'   9354 {norm(MAIN[_L(' **The same is true of the other four things this book contributes:**', 1)])}')
...
for i in (_L("    4 — Uncertainty. Fit δ locally by a Ritz expansion and take σ = 2R Z_eff² · SE_pred / ν³ from the fit's"), _L(' A channel is admissible when its levels are separated by more than their uncertainty:')): print(f'   {i} {norm(MAIN[i-1])[:130]}')

# --------------------------------------------------------------------------- 10
hr('10  THE COLLABORATOR PAGE (L9380-L9382): does it name a single pattern and four results?')
print(f'   L7469 {norm(MAIN[_L('  an audit against the built artefact                 3    claims checked against output, not against draft')])[:120]}')
for i in (17, 18, 42, _L("collaborator's note names it: reasoning sound, destination wrong, fixable only by")): print(f'   L{i} {norm(MAIN[i-1])[:120]}')
