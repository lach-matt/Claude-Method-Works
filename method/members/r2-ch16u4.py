#!/usr/bin/env python3
# r2-ch16u4.py — DRAFT SUCCESSOR to r2-ch16u3.py (md5 399eed11db2e254f0cc4203c73104be7), class DEF-153N (W-234): a literal
# Register / Index-of-Indices line read whose content moved. NOT SEATED — see the finding in the report that accompanies it.
# Every literal line index or window the predecessor holds is replaced by a content-keyed locator; nothing else changes:
#   L31   H = '/home/claude/members'                       -> H = os.path.dirname(os.path.abspath(__file__))
#   L103  ('ch.1 mechanism table', 400, 406)               -> _L('| language | mechanism | closure | cost |'), _L('| **documentary** | — | **no mechanism exists** | — |')
#   L103  ('ch.1 closure-operator table', 455, 460)        -> _L('  language                 closure operator ...'), _L('  information              description length ...')
#   L113  MAIN[438]                                        -> MAIN[_L(<L439 text>) - 1]
#   L114  MAIN[452]                                        -> MAIN[_L(' The self-feeding loop and its asymmetry hold in all five languages.') - 1]
#   L165  'reg L4981' / 'reg L5061' (labels)               -> f'reg L{_RE(1327)}' / f'reg L{_RE(1347)}'
#   L166  REG[_L('  the periodic table  36 ...')]           -> REG[_RE(1347) - 1]   (a Register read the predecessor keyed to MAIN text at the same number)
#   L190  'reg L6491' (label); L191 REG[_L('| | order 1 | order 5 |', 1)]  -> f'reg L{_RE(1755)}'; REG[_RE(1755) - 1]   (same)
#   L195  'reg L5017' (label); L196-197 REG[ 5016 ] x2     -> f'reg L{_RE(1336)}'; REG[_RE(1336) - 1]   (register 1336 by its heading "### 1336")
#   L212  'reg L4873' (label); L213 REG[_L(" **Λ's seven constraints ...", 1)]  -> f'reg L{_RE(1300)}'; REG[_RE(1300) - 1]   (same)
#   L224  range(_L(' The same computation on other indices:', 1), _L('  the periodic table ... 90  36  no', 1))
#                                                          -> range(_LI('## Λ_var — the variables'), _LI('## Λ_ryd — the Rydberg series'))   (an IOI window the predecessor keyed to MAIN text)
# _RE(n) is the Register locator: the headline of entry n — the first non-blank line after its heading "### n" — never a number.
# _LI(t) is the predecessor family's _L(text) helper over the Index of Indices member.
# Main-volume L-labels in prints (L9412, L9414-L9421, L439, L453, L5564, ... , ioi L1596-L1613) are documentary and are left as the
# predecessor had them; the main and ioi members are byte-identical between the pre-shift and live bundles.

# --- re-anchoring helpers (DEF-153N successor): a Register entry found by its heading, an ioi line by its own text ---
def _RE(n):
    import os as _o
    global _LR
    try: _LR
    except NameError: _LR = open(_o.path.join(_o.path.dirname(_o.path.abspath(__file__)), 'The_Method_1_6___The_Register-2.md'), encoding='utf-8').read().split('\n')
    h = [i for i, l in enumerate(_LR, 1) if l == f'### {n}']
    assert len(h) == 1, ('re-anchor: register heading is not unique or is absent', n, h)
    for j in range(h[0] + 1, len(_LR) + 1):
        if _LR[j - 1].strip(): return j
    assert False, ('re-anchor: register entry has no body', n)
def _LI(t, d=0):
    import os as _o
    global _LII
    try: _LII
    except NameError: _LII = open(_o.path.join(_o.path.dirname(_o.path.abspath(__file__)), 'The_Method_1_6___The_Index_of_Indices-2.md'), encoding='utf-8').read().split('\n')
    h = [i for i, l in enumerate(_LII, 1) if l == t]
    assert len(h) == 1, ('re-anchor: ioi line text is not unique or is absent', t[:60], h)
    return h[0] + d
...
for lab, a, b in (('ch.1 mechanism table', _L('| language | mechanism | closure | cost |'), _L('| **documentary** | — | **no mechanism exists** | — |')), ('ch.1 closure-operator table', _L('  language                 closure operator            delete                      add'), _L('  information              description length          E_bits stays 0              grows'))):
...
print('  L439 (ch.1): ' + norm(MAIN[_L(' **When a property holds in five languages and fails in the sixth, the sixth EXPRESSION is wrong — not the property.** This has now located two errors, and in both cases the structure was sound and the translation was not.') - 1])[:110])
print('  L453 (ch.1): ' + norm(MAIN[_L(' The self-feeding loop and its asymmetry hold in all five languages.') - 1]))
...
print(f'  registers stating the population: '
      f'1327 (reg L{_RE(1327)}) "eleven closed indexes"; 1347 (reg L{_RE(1347)}) "TWELVE CLOSED INDEXES"')
print(f'  reg L{_RE(1347)}: ' + norm(REG[_RE(1347) - 1])[:150])
...
print(f'  register 1755 (reg L{_RE(1755)}) diagnosis, first 300 chars:')
print('    ' + norm(REG[_RE(1755) - 1])[:300])

# ---------------------------------------------------------------- 9. the domain protocol
hr('9. "the fourth question of the domain protocol" (L9486) read at its target')
print(f'  register 1336 (reg L{_RE(1336)}), the protocol entire:')
print('    ' + norm(REG[_RE(1336) - 1])[:420])
qs = norm(REG[_RE(1336) - 1]).split('Four questions before any fit:')[1].split('.*')[0].split('*')[0]
...
print(f'  register 1300 (reg L{_RE(1300)}) entire, first 400 chars:')
print('    ' + norm(REG[_RE(1300) - 1])[:400])
...
for i in range(_LI('## Λ_var — the variables'), _LI('## Λ_ryd — the Rydberg series')):
