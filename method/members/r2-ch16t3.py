...
# r2-ch16t3.py — DRAFT SUCCESSOR to r2-ch16t2.py (md5 f413b562e920ef866a80c382d3782696), class DEF-153N (W-234): the
# Register's front matter grew by two lines at BUILD110, so every Register line from 50 moved by +2, and every literal
# line index or window here is replaced by a content-keyed locator; nothing else changes. Replacements:
#   H = '/home/claude/members'            -> the script's own directory (os.path.dirname(os.path.abspath(__file__)))
#   'L9356 cites §24.2 ...' (prose)       -> L{_L(<the "cost of a guarantee" line>)}
#   main literals 114,233,239,241,374,558,716 -> _L(<line text>[, d]) each (114 and 241 by a unique neighbour + offset)
#   REG[_L(<main text>)] with label L1929 -> _LV(REG, <register 516's headline, the line the ORIGINAL r2-ch16t read>), label computed
#   IOI[_L(<main text>)] with label L1924 -> _LV(IOI, <the *Role for Λ.* line the ORIGINAL r2-ch16t read>), label computed
#   'main L233-L245' and range(233, 246)  -> the window from _L(<L233 text>) to _L(<L246 text>), labels computed
# NOT PROVED: on the pre-shift bundle (BUILD109) the output differs from r2-ch16t2.out at exactly the three lines whose
# banked labels were already stale before the shift (L9356 / reg L1929 / ioi L1924) — recorded, not repaired (G0c).
...
def _L(t, d=0):
    ...
    return h[0] + d
# --- the same locator over any volume's lines (r2-ch16t3): a Register or IOI line found by its own text, never by a number ---
def _LV(M, t, d=0):
    h = [i for i, l in enumerate(M, 1) if l == t]
    assert len(h) == 1, ('re-anchor: line text is not unique or is absent', t[:60], h)
    return h[0] + d
...
print(f'\n  L{_L(' **The cost of a guarantee.** *V* = 4ν/3 is two lines from the Rydberg formula and has no use in a fitting paradigm. That is the best argument that it is new, and the four unreachable documents of §24.2 are the best argument that it may not be.')} cites §24.2 for "the four unreachable documents". Testing §24.2\'s OWN body:')
...
for i in (_L(' Preface — the shape of the argument', 3), _L(' Serving PART III — THE LAW. The twenty principles — numbered P1–P23 with P10, P12 and P18'), _L(' **Two kinds appear.** *Principles* say what an index is; ten of them admit exact mathematical statement. *Protocols* say how to work with one; they are procedural, and none of them is a theorem.'), _L(' **Two kinds appear.** *Principles* say what an index is; ten of them admit exact mathematical statement. *Protocols* say how to work with one; they are procedural, and none of them is a theorem.', 2), _L(' Three principles govern questions, and they are one principle in three moods. P19 was the'), _L(' Part I indexes the principles and the protocols. §28.8 indexes the process. Each turns a'), _L('### 2.15.3 Why it belongs here rather than among the principles'), _L(' withdrawals, twenty-two prime audits, twenty principles and twenty-four protocols — and the demonstration'), _L(' §1 requires before anything joins the formalised principles.')):
    print(f'   main L{i} {norm(MAIN[i-1])[:118]}')
_r518 = _LV(REG, '**THREE PRINCIPLE NUMBERS HAVE NEVER EXISTED AND THE COUNT SAID TWENTY-THREE.** *P10, P12 and P18 appear nowhere — not in this book, not in the companion, not in any backup — so there are* **twenty** *principles under a numbering that runs to P23.* **P8 does exist* **, stated at §2.15 and nowhere else, which is why an earlier pass here reported four gaps and not three.* *The numbering is left alone and the gap disclosed; renumbering would break every cross-reference in two documents, and inventing three principles to fill the slots would be worse.*')   # the site the ORIGINAL r2-ch16t read (REG[1928] at BUILD90: register 516's headline), not the '### 518' its re-anchored predecessor's golden shows
print(f'   reg  L{_r518} {norm(REG[_r518 - 1])[:118]}')
_i1929 = _LV(IOI, '**Role for Λ.** It supplies, from first principles, the one thing Chapter 6')   # the site the ORIGINAL r2-ch16t read (IOI[1923]: *Role for Λ.*), not the '---' its predecessor's golden shows
print(f'   ioi  L{_i1929} {norm(IOI[_i1929 - 1])[:118]}')
...
_p0, _p1 = _L(' Serving PART III — THE LAW. The twenty principles — numbered P1–P23 with P10, P12 and P18'), _L('  P    a complete index is self-referencing                                    ℛ(Λ) = Λ                                          §16.1,')
print(f'\n   the principles block read at its target (main L{_p0}-L{_p1 - 1}):')
for i in range(_p0, _p1): print(f'    {i} {norm(MAIN[i-1])[:130]}')
print('\nEND r2-ch16t')
