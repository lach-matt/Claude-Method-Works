...
OLD_M = os.path.join(REPO, 'method', 'The_Method_1_6_BUILD116_main_and_register.md')
OLD_M_MD5 = 'dc67f10fee116d39f8fe2336f8b1a6af'
NEW_M = os.path.join(REPO, 'method', 'The_Method_1_6_BUILD117_main_and_register.md')
OUT = os.path.join(REPO, 'method', 'build117') + os.sep
...
ENTRY_NS = tuple(range(1849, 1853))
ENTRIES = """

### 1849

**§24.13'S UPPER END SITS JUST OUTSIDE THE INTERIOR'S WORST CASE, NOT INSIDE IT, AND REGISTER 1844 PRINTS THE WRONG WORD FOR ITS OWN RATIO.** *Register 1844 states:* **"the upper end sits INSIDE the interior's own worst case on the sodium-like sequence (1.04×)"**. *The ratio is right and the word is not.* **On the sodium-like sequence §24.13's table gives the upper end, Z = 4, at 2.5% against the interior's worst case, Z = 2, at 2.4%; 2.5/2.4 = 1.0417, and a ratio above one means the upper end is WORSE — outside that worst case by 4.17%, not inside it.** *The same holds on the lithium-like sequence, where 1.8% against 1.4% gives 1.2857; the entry's own phrase for that one, "within a quarter of it", is the correct shape of statement and "INSIDE" is not.* **Register 1844's finding is untouched.** *The neutral end is still 6.2× and 9.4× worse than the upper end, the four sites are still classified correctly, and the point the entry was making — that the upper end is close enough to the interior to make the neutral's failure the exception — still stands on the corrected wording. What falls is one word, and it falls to the entry's own ratio.* Registers 1844. (a correction.)

### 1850

**THE BICONDITIONAL REGISTER 1847 FAULTS IS PRINTED AT THREE SITES, NOT TWO.** *Register 1847 closes:* **"this is the only one that fails, and it is stated twice."** *The two it names are right — §11.7's caption to Figure 11.2 and §11.8's opening line — and there is a third.* **Chapter 11's pairwise-combination table prints, in its geometry + analysis row, "F is not palindromic ⟺ the poset is not self-dual".** *That is the same biconditional contraposed on both sides, and ¬P ⟺ ¬Q is equivalent to P ⟺ Q, so it carries the direction that fails exactly as the other two do.* **The count is three.** *Nothing else in 1847 moves: the six-element counterexample stands, the direction that holds still holds, and §11.8's conclusion still survives on the contrapositive of it. What the undercount would cost is a repair — one that touched only the two named sites would leave the claim standing in the table.* Registers 1847. (a correction.)

### 1851

**§23 STATES THE ASYMPTOTIC FORM AND ITS ERROR AT TWO SITES, NOT ONE, AND THE SECOND DOES NOT USE THE STRING REGISTER 1845 COUNTED.** *Register 1845 states:* **"One site states the approximation and its error; twenty-two state an identity."** *The site it names is §23's own sentence at the exact rationals. The second is Figure 23.1's caption:* **"The exact cost against its asymptote. The floor 32/11 = 2.909 belongs to the exact expression; the asymptotic form gives 26/9 and is low by 0.69% at ν = 2. They converge by ν = 10."** *26/9 is 4ν/3 + 4/(9ν) evaluated at ν = 2, so the caption states the same approximation and the same error in other lettering — which is precisely why a census of the printed string "4ν/3" cannot see it.* **The qualified sites are two.** *Every number in 1845 is unchanged and so is its finding. What narrows is the reading that a reader meeting one of the unqualified sites has no way to know, since the chapter's own figure caption is one of the places such a reader looks.* Registers 1845. (a correction.)

### 1852

**REGISTER 1848'S GENERAL REASON IS TRUE OF ONE OF THE TWO OPERATORS THIS BOOK CALLS ℛ, AND THE SECTION THAT WOULD SAY WHICH IS ONE OF THE NINE THAT WERE NEVER WRITTEN.** *Register 1848 states, without qualification:* **"with TWO coordinates ℛ(X) = X for every X, so any presentation bundled down to two coordinates reports closure whatever it contains."** *Measured on both operators the book uses under that one name:* **under the pairwise-projection closure — every tuple whose every 2-D projection lies in X's — the statement is exactly true and provable in one line, because at two coordinates the only pairwise projection IS the set; all three folds of 1848's own three cells give E = 0, and 0 of 3,000 random two-coordinate sets have ℛ(X) ≠ X. Under §6.1's envelope closure it is false: {(0,1), (1,0)} on the 2 × 2 box closes to three cells, two of the three folds give E = 1, and 1,550 of those same 3,000 sets have ℛ(X) ≠ X.** *The demonstration 1848 prints is unaffected either way — both operators give E = 1 decomposed, with the single added cell (0,0,1), and E = 0 bundled — so the three cells show what they are said to show under both readings.* **What the entry does not do is name which ℛ its general reason is about, and the volume cannot be asked: §14.5.3, titled "Two pairwise operators, and this book has used one name for both", is a heading with nothing under it — one of the nine.** *Filed as a measurement of both readings and not as a choice between them. Which operator §14.3 intends is subject matter and is not decided here.* Registers 1848. (a correction.)"""
...
print('BUILD116 -> BUILD117: reverse recovers md5 %s == old: True; new %s B md5 %s %s lines'
