...
OLD_M = os.path.join(REPO, 'method', 'The_Method_1_6_BUILD117_main_and_register.md')
OLD_M_MD5 = 'c0a35567749e531873c46f7c9eb544c1'
NEW_M = os.path.join(REPO, 'method', 'The_Method_1_6_BUILD118_main_and_register.md')
OUT = os.path.join(REPO, 'method', 'build118') + os.sep
...
**REGISTER 1848'S DEFECT IS ℛ₄'S AND IT IS PRINTED UNDER THE SYMBOL E, WHICH IS THE CONFUSION §14.5.3 NAMES IN ITS OWN TITLE AND NEVER EXPLAINS; REGISTER 1852 IS SUPERSEDED WITH IT.** *§14.3's claim survives all of this and is stated first:* **folding two chains into one non-chain coordinate does hide a defect from the reconstruction, and the three cells {(0,0,0), (0,1,1), (1,0,1)} on the 2 × 2 × 2 box still demonstrate it — E falls 2 → 0 under ℛ and E₄ falls 1 → 0 under ℛ₄.** *What was wrong is the number, the cell list and the reason.* **Measured on the operator §6.1 defines — Âᵢ(X) = {xᵢ}, φ̂ᵢⱼ(v) = max{xᵢ : xⱼ ≤ v}, ℛ(X) = {x ∈ ∏Âᵢ : xᵢ ≤ φ̂ᵢⱼ(xⱼ)}, which Theorem 14.1 calls "value sets and pairwise monotone bounds" and the Mathematical Compendium's glossary calls the (≤,≤) corner — the decomposed set has E = 2 and the closure adds TWO cells, (0,0,1) and (1,1,1). Register 1848's E = 1 with the single cell (0,0,1) is exactly E₄, the defect under ℛ₄, and the difference between them is the orientation cost the glossary defines: E − E₄ = 1.** *§14.5.3 is titled* **"Two pairwise operators, and this book has used one name for both"** *and is a heading with nothing under it; this entry is what that costs, measured.* **And 1848's general reason is false under both operators, not merely unqualified: {(0,1), (1,0)} on the 2 × 2 box closes under ℛ to all four cells, over 3,000 random two-coordinate sets ℛ(X) ≠ X in 1,847 and ℛ₄(X) ≠ X in 1,120, and only ONE of the three folds of 1848's own cells closes — the other two give E = 2 and E₄ = 1.** *No reason of that shape exists, and §14.3's own qualifier is the operative condition: it says folding into one* **non-chain** *coordinate, not folding to two coordinates as such. That qualifier is corroborated here rather than replaced.* **Register 1852 is superseded and was mine.** *It read 1848's general reason as true of one of two operators and false of the other, and named as the true one a closure defined by raw two-dimensional projections with no monotone envelope — an operator this book does not have. The book's two are ℛ and ℛ₄, and under those the figures are 2 and 1. Measured by `bundling2.py`, whose ℛ is validated against §6.1's own printed E = 36 for the periodic table rather than against itself; its predecessor implemented the projection operator and is left in place, superseded, because it is what produced 1848.* Registers 1848; 1852. (a correction.)"""
...
print('BUILD117 -> BUILD118: reverse recovers md5 %s == old: True; new %s B md5 %s %s lines'
