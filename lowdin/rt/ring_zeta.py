...
def cL(z):
    ...
    return (1/np.pi**2)*((1-np.log(2))+xp*xm/2*chi-np.log(chi)+0.5*(xp**3*np.log(xp)+(xm**3*np.log(xm) if xm>0 else 0.0)))  # Eq.16 as printed (see note)
def eps_r(rs,z,nk=1400,nx=1400):
    ...
    inner=np.trapezoid(f,x,axis=1)
    return 3/(2*np.pi*al*al*rs*rs)*np.trapezoid(inner,k)
