...
import step2_run
import ctypes, os
_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'libshoot_sr.so'))
_lib.shoot_s.restype = ctypes.c_int
_lib.shoot_s.argtypes = [np.ctypeslib.ndpointer(np.float64, flags='C'), ctypes.c_int, ctypes.c_double, ctypes.c_double, ctypes.POINTER(ctypes.c_double)]
def _shoot(q, h, slope):
    """shoot.c with the start exponent free: y1 = y0 exp(slope h), slope = sqrt(q[0]) = local regular exponent
    (-> l+1/2 non-relativistic; -> sqrt((l+1)^2-(Z/c)^2) inside the Coulomb core, the Dirac r^gamma)."""
    y = ctypes.c_double(); nd = _lib.shoot_s(np.ascontiguousarray(q, dtype=np.float64), len(q), h, float(slope), ctypes.byref(y))
...
def eigen_sr(V, l, n, zeta, Z, c=C0, npts=3000, tol=1e-9, Vp=None, Vpp=None):
    ...
    target = n-l-1
    def too_high(E):
        q,_ = qlog(r, Vr, Vpr, Vppr, E, l, c); y, nd = _shoot(q, h, np.sqrt(max(q[0],0.0)))
...
def numerov_wf_sr(V, l, n, zeta, Z, c=C0, npts=3000, Vp=None, Vpp=None):
    ...
    yo = np.zeros(npts); yo[0] = 1e-30; yo[1] = yo[0]*np.exp(np.sqrt(max(q[0],0.0))*h)
