...
asserts every one has at least one study row and exactly one recommendation. The **48 study
rows** were compiled from the published record as known to the session that wrote them; **no
live database was queried** (NREL, DOE and most journal hosts are blocked at this environment's
egress). Every row names its source so it can be checked, and a technology may have more
...
| pilot | 3.0 y | the ~30 MW_th pilot aperture (receiver threshold 0.795, edge loss vs size); the dome against an open aperture on one tower; particle ageing sampled quarterly (R-08); a full-size cold-shell silo cycled daily; the exchanger at 800 °C / 25 MPa; alloy coupons in the real CO₂ loop |
...
is built while the pilot runs, the pilot runs while the module is ordered. Lift and PV need no
further study; the file says so rather than inventing one.
...
| critical | 51.4 | 1.54 | 191 | 5.7 | 1,153 | 35 |
