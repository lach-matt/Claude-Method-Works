...
by an exact byte count and an identical opening. The only change that would close them is a
`drive_sync.py` that can record a file it did not download itself — a change to a program with a
contract, not made here.

The mirror is unchanged, and `PENDING.tsv` is not edited: it is generated by the sync too.
