...
**Nothing was fetched into `drive/`, and that is deliberate.** Seating a file there means a
`MANIFEST.tsv` row, and `MANIFEST.tsv` is generated by `drive_sync.py` and never hand-edited —
`CLAUDE.md` states that without exception. The sync still cannot authenticate headless, so the
manifest still cannot be regenerated here. The route is recorded for whoever has the token; the
mirror is unchanged.
