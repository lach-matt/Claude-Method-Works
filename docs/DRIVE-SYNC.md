...
**That is a statement of the tool's constraint, not a route anyone should take.** There is no Google
token, by the owner's choice: the Colab mount is how this mirror is fetched. Read `--token` as the
reason `drive_sync.py` does not run here, not as a step someone is expected to perform.
...
### The by-id route exists after all, through the MCP connector

**Measured 2026-09-04, later the same day.** `PENDING.tsv` says of those two rows that they are
*"reachable only by Drive id, which needs the API"*, and that the API is out of reach headless. The
first half is right and **the second is no longer true in a session that has the Google Drive
connector**: `get_file_metadata` on a bare file id returns the file, duplicate title and all.

Both were fetched by id and their metadata read:

| pending row | Drive id | reported size | held twin |
|---|---|---:|---|
| `BUILD174…__1NdZxJiVX6A…` | `1NdZxJiVX6AwMqYA1RgI-vNS1hC3okC5h` | 5,321,034 | 5,321,034 |
| `BUILD178…__1Z_L42Kn7Mr…` | `1Z_L42Kn7Mrq0QBPVEhvoOJn1MYFe-DW9` | 5,619,374 | 5,619,374 |

**The duplicate claim moves from stated to corroborated, and no further.** Each reports the byte
count of a twin already mirrored to the byte, and each returns an opening ~2 KB identical to that
twin's — `<<<FILE: BiIII_asd.tsv>>>` and the Bi III level table, character for character. The two
held `BUILD174` copies are themselves byte-identical to each other (`md5 ca9319a8…`), so the
convention of keeping both is not keeping two different things.

**It is corroboration and not proof**, for the same reason `PENDING.tsv` already gives about the
chat exports: the connector returns no `md5Checksum`, and the full bytes were not downloaded and
hashed. A file can share a size and an opening and still differ in the middle — that is exactly the
`RECOVERED-TRUNCATED` hazard in `docs/RECOVER.md`, one level up.

**This is a verification route, not a fetch route, and the difference is the whole point.**

Reading a duplicate by id needs **no token at all** — the connector carries its own authorisation and
it worked here on the first call. What is blocked is the other half. Seating a file in `drive/` means
a `MANIFEST.tsv` row; `MANIFEST.tsv` is generated by `drive_sync.py` and never hand-edited, which
`CLAUDE.md` states without exception; and `drive_sync.py`'s only path to a manifest is its own OAuth
flow. **There is no Google token, by the owner's choice — the Colab mount is the route** — and a
mount cannot see a same-titled duplicate. `--credentials`/`--token` are the tool's only inputs; it
has no mode that ingests a file fetched by some other means.

So the two rows stay **pending by decision**, and the decision is better founded than before rather
than overturned: what was an untested assumption that they duplicate a held twin is now corroborated
by an exact byte count and an identical opening. The change that closes them is a `drive_sync.py` that can record a file it did not download itself,
and that is now built: see **`--adopt`** below.

The mirror is unchanged. **And a correction to my own note of an hour earlier: `PENDING.tsv` is
*not* generated by the sync.** `drive_sync.py` contains no mention of it and never has — the string
`pending` does not occur in the tool. It is a record maintained beside the mirror, which is why
`--adopt` reads it and leaves it alone.

### `--adopt` — recording a file this tool did not download

```
python3 tools/drive_sync.py --adopt <local copy> --adopt-meta <metadata.json> [--adopt-path REPO_PATH]
python3 tools/drive_sync.py --adopt ... --adopt-meta ... --dry-run     # decide nothing, write nothing
python3 tools/drive_sync.py --selftest                                  # no network, no token
```

**Why it exists.** A same-titled duplicate is invisible to a mount and reachable only by file id. It
can therefore be *obtained* — by a Drive connector, or by hand — and still not be *mirrorable*,
because a file in `drive/` needs a `MANIFEST.tsv` row and the manifest is generated here and never
hand-edited. That was a real dead end, and this is the way out of it.

**It contacts Drive not at all.** Both `--adopt` and `--selftest` return from `main()` **before**
`load_google_modules()` is reached — structurally, not by luck — so neither needs a token, and this
container is not meant to have one.

**The metadata is whatever a Drive metadata call returned**, saved as JSON. Both the API's and the
connectors' spellings are accepted (`id`/`fileId`, `title`/`name`, `fileSize`/`size`, `modifiedTime`,
`mimeType`), and the four a manifest row cannot be honest without are required.

**The row is `ok-adopted`, never `ok`,** and that is the heart of it. The download path verifies the
byte count **and** Drive's own `md5Checksum` before writing a row. An adopted file has had only the
byte count checked, because the routes that can reach a same-titled duplicate return no checksum.
Writing `ok` would claim a verification that was never performed — the same discipline that keeps
`RECOVERED` distinct from `RECOVERED-BY-HEADING`, and `RECOVERED` from `RECONSTRUCTED`. `CLAUDE.md`'s
statement that every manifest row reads `ok` is now "`ok`, or `ok-adopted`".

**What it refuses**, each covered by a selftest fixture:

- a local size that disagrees with the metadata, byte for byte;
- an md5 that disagrees with the metadata's, when one is supplied;
- overwriting a mirrored file whose content differs — that is a conflict, not an adoption;
- a repo path already held under a different Drive id;
- and it will not write `ok`.

Re-adopting the identical file is idempotent: the row is replaced, not duplicated.

**The repo path comes from `PENDING.tsv` when it can.** Passing `--adopt-path` overrides it; with
neither, the tool stops rather than guessing. `PENDING.tsv` is read and never written — see the
correction above.

**Verified against the real mirror, writing nothing:**

```
$ python3 tools/drive_sync.py --adopt "drive/The Method Materials/…BUILD174….md" \
      --adopt-meta meta.json --dry-run -v
INFO --adopt: repo path taken from PENDING.tsv: The Method Materials/…__1NdZxJiVX6A….md
would adopt  The Method Materials/…__1NdZxJiVX6AwMqYA1RgI-vNS1hC3okC5h.md
  bytes      5321034  (matches the metadata)
  status     ok-adopted
nothing written.
```

**The two pending duplicates were not adopted in this session, and the reason is cost.** The
mechanism is ready and the metadata is in hand; what is missing is the bytes. `download_file_content`
returns base64 **through the conversation**, so a 5.3 MB file arrives as ~7 MB of base64 — on this
page's own arithmetic, roughly 1.8 million tokens for one file and near 4 million for the pair. For
two copies already corroborated as byte-identical twins of files the mirror holds, that is a bad
trade, and the same reasoning this document applies to the 388 MB exports applies here. **Whoever
runs a mount or a plain authenticated download can adopt both in a minute**, and the tool will refuse
them if they are not what the metadata says.
