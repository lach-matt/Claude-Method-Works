...
life it did. An index cannot. Each of the 73 rows carries a grade 0–3 on each axis, so a result that
...
1. **Every row names a section that exists.** `--selftest` matches all 73 rows against the headings of
...
```
python3 tools/directives.py --selftest     # 73 rows against the papers on disk
