...
pE3 = [i for i, l in enumerate(P, 1) if re.match(r'^E\.3\s', l) or re.match(r'^#{1,4}\s*E\.3\s', l)]; pE4 = [i for i, l in enumerate(P, 1) if re.match(r'^(#{1,4}\s*)?E\.4\s', l)]
pe3 = [(i, ITEM.match(P[i - 1]).group(1)) for i in range(pE3[-1] + 1, pE4[-1]) if ITEM.match(P[i - 1])]
pE12 = [i for i, l in enumerate(P, 1) if re.match(r'^(#{1,4}\s*)?E\.1\.2\s', l)]; pE13 = [i for i, l in enumerate(P, 1) if re.match(r'^(#{1,4}\s*)?E\.1\.3\s', l)]
...
print('   §32.2 body L%d–L%d / span L%d–L%d: |Q| = 7 sites %s ; *seven* %s' % (a, b, a2, b2, sites(r'\|Q\| *= *7', M, a, b2), [i for i in range(a, b2) if has_token(M[i - 1], 'seven')]))
...
loE2, hiE2 = drange('E.2'); e2 = [(i, ITEM.match(M[i - 1]).group(1)) for i in range(loE2 + 1, hiE2) if ITEM.match(M[i - 1])]
print('   E.2 item entries %d %s ; E.1.1 lineage rows naming closures: %s' % (len(e2), [x for i, x in e2], [(i, M[i - 1].strip()[:70]) for i in sites(r'closed', M, *drange('E.1.1'))]))
