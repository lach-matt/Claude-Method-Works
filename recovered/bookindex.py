...
start=[i for i,l in enumerate(L) if l.startswith('## 1. ')][-1]      # body starts at the real Chapter 1 heading
APP='ABCDEF'
...
for i,l in enumerate(L):
    ...
    if m: part={'0':0,'I':1,'II':2,'III':3,'IV':4,'V':5,'VI':6}[m.group(1)]; continue
    m=re.match(r'^## (\d+)\. ',l)
    if m: loc=int(m.group(1)); chpart[loc]=part; continue
    m=re.match(r'^## Appendix ([A-F])',l)
    if m: loc=m.group(1); part=7; chpart[loc]=7; continue
    ...
    if refs: claims+=1
    for r in refs:
        cells.add((code(loc),code(r))); partcells.add((chpart[loc],chpart.get(r,7)))
