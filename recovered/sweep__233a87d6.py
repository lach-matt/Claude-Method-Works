import math, statistics as st, importlib.util as iu
from collections import defaultdict
sp=iu.spec_from_file_location("mr","method_region.py")
import sys
src=open("method_region.py",encoding="utf-8").read()
src=src[:src.index("with State(")]
src=src.replace("from zeno import State, step","")
g={}
exec(src,g)
load=g["load"]; op_S=g["op_S"]; op_R=g["op_R"]; op_W=g["op_W"]; region=g["region"]
config,mults,H=load()
Hin={k:v for k,v in H.items() if region(k[0],k[1],k[2],config)}
S=op_S(Hin,config); R=op_R(set(Hin)); Xs=set(Hin)
print("  THE TOLERANCE SWEEP — where does valuation begin?\n")
print("      the steps: " + " · ".join(f"{k} {v[1]:.3f}" for k,v in S.items()))
...
print()
W=op_W(Hin,S,config,4.0,region); Ws=set(W)
new=sorted(Ws-Xs)
byl=defaultdict(int); byc=defaultdict(int)
for Z,c,l,Sm in new: byl[l]+=1; byc[c]+=1
print(f"  AT tau = 4.0 — where the l step finally fires:\n")
print(f"      {len(new)} cells valued that were not held")
print("      by l     : " + "  ".join(f"{'spdfghik'[l]}:{byl[l]}" for l in sorted(byl)))
print("      by charge: " + "  ".join(f"{c}:{byc[c]}" for c in sorted(byc)))
print(f"      R also places {len((R&Ws)-Xs)} of them, and refuses {len((Ws-R)-Xs)}")