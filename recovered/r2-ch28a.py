...
PC = L.read_member('The_Method_1_6___Physics_Compendium-2.md').split('\n')
IOI = L.read_member('The_Method_1_6___Index_of_Indices-2.md').split('\n')
...
for sec, keys, why in PTR:
    ...
    except Exception as e: print('  §%-9s %-42s UNRESOLVED: %s' % (sec, why[:42], e))
...
if bh:
    ...
    rows = [l for l in B if re.match(r'^\s*(\||[-*·•]|\*\*|\d{4})', l) and re.search(r'(1[6-9]\d\d|20[0-2]\d)', l) and not re.match(r'^\s*\|\s*-', l)]
    rows = [l for l in rows if not re.match(r'^\s*\|\s*(Year|year)', l)]
    yrs = [int(y) for l in rows for y in re.findall(r'\b(1[6-9]\d\d|20[0-2]\d)\b', l)[:1]]
    print('  MC bibliography L%d–L%d: candidate work rows %d (rows with a year, header/separator excluded); years %s–%s' % (b0, b1 - 1, len(rows), min(yrs) if yrs else None, max(yrs) if yrs else None))
    def surnames(lines):
        s = set()
        for l in lines:
            for m in re.finditer(r"(?<![A-Za-z'’\-])([A-Z][a-zäöüéèçøÅ'’\-]+)(?:,\s*[A-Z]\.|\s*\(|\s*&|\s*and\b)", re.sub(r'[*_`]', '', l)): s.add(m.group(1))
        return s
...
print('  rows with member main/all and line in L%d–L%d: %d %s' % (s, len(M), len(inr), [r[:5] for r in inr][:12]))
