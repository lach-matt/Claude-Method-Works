...
THE X-RAY PAIR (R 1427)

At twelve ladders E = 1, the defect at (subvalence, counting, across elements) --
Moseley's ladder, in a direction ionisation does not go. That defect is what
demanded Lambda_xray (R 1378), and Lambda_xray filled it. The two X-ray ladders
enter at the SUBVALENCE seat: Moseley as counting/across, the K-alpha doublet as
coupling/within. Only subvalence closes, and the seat is settled from OUTSIDE the
closure rather than by it, because Lambda_PCA excluded the core independently, on
different coordinates, before this was computed. Fourteen ladders, nine cells,
E = 0.
"""
from itertools import product, permutations

FAMILY = ["species", "state", "x-ray"]
...
LAD = [
 ...
 ("Moseley",                  2, 2, 0, 1, "fixes the line    - R 1427, 98 rows Ne-Fm"),
 ("the K-alpha doublet",      2, 2, 1, 0, "fixes the element - R 1427, KL3 - KL2"),
]
...
if __name__ == "__main__":
    print("  Lambda_ladder -- THE LADDERS AS OBJECTS  (R 1356, 1374, 1375, 1384, 1427)\n")
    print("      Z = Ne + c - 1  and  A = Z + N -- two relations, three ladders each.")
    ...
    print(f"\n  CLOSURE on seat x kind x Zcross")
    ...
    if not defects:
        print("      NO DEFECT -- the subvalence seat that demanded Lambda_xray is filled.")

    print("\n  THE SEAT, SETTLED FROM OUTSIDE THE CLOSURE (R 1427)")
    for s in range(len(SEAT)):
        alt = {(s if nm in ("Moseley", "the K-alpha doublet") else ss, k, z)
               for nm, _, ss, k, z, _ in LAD}
        print(f"      X-ray pair seated at {SEAT[s]:<12} -> E = {minE(alt, axes)}")
    print("      Only subvalence closes. Lambda_PCA excluded the core independently,")
    print("      on different coordinates, BEFORE this was computed.")

    print("\n  CONTINGENCY (R 1383) -- every E is reported with its rate.")
    print("      4 ladders, `fixes` injective : E = 0 FORCED, 2% could refuse")
    print("      14 ladders, nine cells       : 90% of comparable sets refuse (R 1427)")
    ...
    for a, b in [("isoelectronic","the crossing charge -- a_cross is fixed by the pair"),
                 ...
                 ("ionisation","t above c = 1, and Lambda_t's empty cells"),
                 ...
                 ("Moseley","sqrt(E) linear in Z -- slope derived, wall at 14.2%"),
                 ("the K-alpha doublet","the 2p spin-orbit splitting, 4.5 orders")]:
        print(f"      {a:<24}{b}")