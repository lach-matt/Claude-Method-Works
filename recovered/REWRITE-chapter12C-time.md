...
> **REVISION NOTE (this session):** one inclusion added per M's ruling — a new final section, **"The tower's two ends are one bracket"** (result [MC-55]), placed after §12.11.0.12 because it draws on the collapse that section introduces. It carries the delivered 1D↔14D result: the tower's chains form one directed system of monotone gap-free brackets, the 1D chain is the system's terminal object and 14D its limit, and the strict map form is refuted (branching 4→9). Proofs and scripts delivered (03-1D-lands-inside-14D.md; tower.py/tower2.py/tower3.py/factor.py). All prose through [MC-34] is the approved text, unchanged. Guard held: §26.2's rank(log q)=1 (value language) is NOT the 1D end here (index language) and is not fitted to it.
