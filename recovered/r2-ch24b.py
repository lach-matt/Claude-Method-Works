...
ppsub = [(i, P[i - 1].strip()) for i in range(pe + 1, pf) if re.match(r'^ ?E\.\d', P[i - 1]) and not P[i - 2].strip()]
...
rs = next(i for i, l in enumerate(M, 1) if re.match(r'^## References', l)); RB = '\n'.join(M[rs:])
