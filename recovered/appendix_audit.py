...
SRC = open("The Method 1.5.md", encoding="utf-8").read()
def span(x, y):
...
def run():
    ...
    unsourced = [l for l in rows
                 if not re.search(r"\[[A-Z]\]|NIST|ASD|CODATA|AME|arXiv|\d{4}", l)]
    F["B rows sourced"] = len(unsourced)

    # C — every 'recomputed' claim names a script or a section
    rec = re.findall(r"[^\n]{0,110}recomputed[^\n]{0,110}", C)
    bare = [r for r in rec if not re.search(r"\.py|§|build|Appendix", r)]
    ...
    egs = {int(x) for x in re.findall(r"E\(G\)\s*=\s*(\d+)", SRC)}
