...
chk("(1) E(Lambda)=0",len(Rop(S,8))-len(S),0)
    ...
    for n in range(1,4):
      for l in range(0,2):
        ...
        for k in range(1,min(4*l+2,3)+1):
          ...
          t+=(k+1)*inn
    return t
chk("tree factorisation = |Lambda|",tree_count(),len(S))
...
chk("(5) rank=Omega(N(x))",Om(N(x0)),sum(x0))
chk("(6) d=prod(|Di|+1)",tau(N(x0)*N(y0)//(g0*g0)),int(np.prod([abs(x0[i]-y0[i])+1 for i in range(8)])))
chk("(7) d(x,x)=1",tau(1),1)
T=lambda v,Z=1: Z*Z*R/v**2
def we(v,h=1,Z=1):
    a,m,b=T(v-h,Z),T(v,Z),T(v+h,Z); return abs(b-a),abs(m-(a+b)/2)
w,e=we(20); chk("(21) V=4nu/3 asympt",round(w/e,3),round(80/3,3),0.05,True)
...
chk("(25) V=4nu^3/(h(3nu^2-h^2))",sp.simplify(Vab-4*nu**3/(h*(3*nu**2-h**2)))==0,True)
r=sp.symbols('r',positive=True)
chk("(25b) V(r)=4r^3/(3r^2-1)",sp.simplify(sp.cancel(Vab.subs(h,nu/r))-4*r**3/(3*r**2-1))==0,True)
chk("V free of Z,R",{Z,Rs}.isdisjoint(sp.simplify(sp.cancel(Vab.subs(h,nu/r))).free_symbols),True)
def Vp(p,x=20.,hh=1.):
    fq=lambda t:t**p; a,m,b=fq(x-hh),fq(x),fq(x+hh); return abs(b-a)/abs(m-(a+b)/2)
...
wt,et=we(40,1)
chk("w/T=4h/nu",round(wt/T(40),4),0.1,0.02,True)
chk("e/T=3(h/nu)^2",round(et/T(40),6),round(3/1600,6),0.02,True)
...
chk("convex delta lower=0.9567",round(2*d5-d4,4),0.9567)
chk("convex bracket=[735860,737380]",(round(Ef(d5)),round(Ef(2*d5-d4))),(735860,737380))
