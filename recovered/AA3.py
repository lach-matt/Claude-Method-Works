...
def chk(n,g,w,tol=0,rel=False):
    ...
    print("  %-44s%18s%18s%7s"%(n,str(g)[:18],str(w)[:18],"ok" if ok else "FAIL"))
print("="*90); print("  AUDIT 3 — EVERY EQUATION"); print("="*90)
print("\n  %-44s%18s%18s%7s"%("equation","computed","stated",""))
print("  "+"-"*86)
R=109737.3
...
rkf=lambda x: sum(x)
chk("rank(a∨b)+rank(a∧b)=rank(a)+rank(b)",
    all(rkf(jn(a,b))+rkf(mt(a,b))==rkf(a)+rkf(b) for a,b in combinations(L,2)),True)
tot=0
for qv in sorted({z[3] for z in S}):
    Sq={z for z in S if z[3]==qv}
    tot+=len({tuple(z[i] for i in (0,1,2,7)) for z in Sq})*len({tuple(z[i] for i in (4,5,6)) for z in Sq})
chk("Σ_q |A(q)||B(q)| = |Λ|",tot,len(S))
...
def tau(m):
    ...
    for p in PR:
        ...
        c*=e+1
    return c
x0,y0=L[500],L[700]; g0=gcd(N(x0),N(y0))
chk("rank=Ω(N(x))",Om(N(x0)),sum(x0))
chk("d=∏(|Δi|+1)",tau(N(x0)*N(y0)//(g0*g0)),int(np.prod([abs(x0[i]-y0[i])+1 for i in range(8)])))
def tree_count():
    t=0
    for n in range(1,4):
      for l in range(0,2):
        ...
        for k in range(1,min(4*l+2,3)+1):
          ...
          t+=(k+1)*inn
    return t
chk("tree factorisation=|Λ|",tree_count(),len(S))
...
chk("w·V=(16/3)T",round(wt*wt/et,1),round(16/3*T(40),1),0.01,True)
...
chk("convex δ lower=0.9567",round(2*d5-d4,4),0.9567)
...
chk("log₂(32/11)=1.5406",round(log2(32/11),4),1.5406)
chk("log₂V at ν=40=5.74",round(log2(we(40)[0]/we(40)[1]),2),5.74)
z=sp.symbols('z'); rk=Counter(sum(x) for x in S); Fp=sum(v*z**k for k,v in sorted(rk.items()))
chk("F(1)=|Λ|",int(Fp.subs(z,1)),len(S))
...
chk("rank(log q)=1",bool(np.linalg.svd(Mc,compute_uv=False)[1]<1e-12),True)
Msun=1.989e30
rH=lambda a,m: a*(m/(3*Msun))**(1/3)
chk("Hill radius Jupiter ≈ 0.355 AU",round(rH(5.203,1.898e27),3),0.355,0.002)
aJ=5.2028
chk("3:1 resonance at 2.50 AU",round(aJ*(1/3)**(2/3),2),2.50,0.01)
chk("2:1 resonance at 3.28 AU",round(aJ*(1/2)**(2/3),2),3.28,0.01)
print("\n"+"="*90)
print("  RESULT: %d passed, %d FAILED, of %d"%(len(P),len(F),len(P)+len(F)))
for n,g,w in F: print("     FAIL %-42s got %s  stated %s"%(n,g,w))