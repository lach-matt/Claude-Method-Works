...
nums_unit = Counter(n for l in UL for n in re.findall(r'(?<![\d,.])\d{1,3}(?:,\d{3})+(?![\d,])|(?<![\d,.])\d+(?![\d,])', l))
...
sp = lambda r: r[1]['species'].rstrip('*').strip(); el = lambda r: sp(r).split()[0]
...
def numset(t): return set(n for n in re.findall(r'(?<![\d,.])\d{1,3}(?:,\d{3})+(?![\d,])|(?<![\d,.])\d+(?![\d,])', t))
...
print('  "earlier verification" across the six volumes:', sites(r'earlier verification'))
...
srcs = M[b1s:b1e - 1]; spec_txt = norm(' '.join(l[30:] for l in srcs if re.match(r'^  \S', l) and not re.match(r'^  \d{4}|^  JPCRD', l)))
