...
# ---- AND THE PARTS RESIDUE, WHICH IS A SPECIFICATION AND NOT A GAP -------
# The author's point, and it is right: a part is DESIGNED TO SPEC and built
# to meet the mathematics. It is the OUTPUT of the engineering phase, not an
# input the plant needs. Criterion 3 asks whether the thing needs feeding,
# and a specification is not a feed.
#
# BUT A PART IS MADE OF SOMETHING, and that is the form in which the question
# survives. So the test is not "is it a part" -- it is WHAT MATERIALS DOES
# THE PARTS STREAM CONSUME, AND ARE THEY ORDINARY. Steel, nickel alloy,
# graphite and electrode stock are ordinary: no criterion about a plant's
# self-sufficiency was written to exclude the existence of industry.
#
# ONE MATERIAL IN THE STREAM IS NOT ORDINARY, and it hides in the PROCESS
# rather than in the part. Fission products live in the salt as chlorides and
# leave as chlorides, taking chlorine with them -- and every atom of that
# chlorine is Cl-37 enriched. See parts_chlorine_kg_per_year().
HM_MOLAR_G = 239.0            # SOURCED: Pu-239, the nuclide that fissions
CL_PER_HEAVY_ATOM = 3.0       # stoichiometric: UCl3 / PuCl3
CL37_MOLAR_G = 36.966         # SOURCED
...
def parts_chlorine_kg_per_year():
    """An UPPER BOUND on the Cl-37 that leaves with removed fission products.

    A BOUND AND NOT A PROCESS FIGURE, and the difference is the whole of its
    honesty. Each fission destroys one PuCl3 and frees three chlorines; the
    two fission products that replace it carry chlorine out when they are
    removed, and charge conservation caps what they can carry near what the
    parent held. The actual figure needs the fission products' valence
    distribution and the process holdup, and this file computes neither."""
    fissions_mol = burnup_kg_per_year() * 1000.0 / HM_MOLAR_G
    return fissions_mol * CL_PER_HEAVY_ATOM * CL37_MOLAR_G / 1000.0


def chlorine_inventory_t():
    """Read from the row that states it, never restated."""
    for _z, mat, val, _u, _st, _sup, _n in bill():
        if "chlorine, Cl-37" in mat:
            return val
    raise ValueError("no chlorine row in the bill")


def parts_chlorine_share_per_year():
    return parts_chlorine_kg_per_year() / (chlorine_inventory_t() * 1000.0)


# WHAT THE TWO REPLACED ROWS ARE MADE OF. Stated rather than computed,
# because a bill of materials for a part that has not been designed would be
# an invention; what is computable is whether each material is ORDINARY, and
# that is the only thing criterion 3 needs from this list.
PARTS_MATERIALS = (
    ("fission-product removal, salt side",
     "nickel alloy vessel, graphite and refractory-metal electrodes, "
     "ordinary vacuum plant", True),
    ("fission-product removal, salt side",
     "Cl-37 leaving with the removed products, unless the waste form "
     "returns it", False),
    ("krypton-85 capture, cryogenic charcoal",
     "activated charcoal, regenerated by warming rather than consumed; "
     "steel storage cylinders for the gas", True),
)
...
def report_criterion3():
    ...
    print("    not cryogen. This file called that 'holds on every material")
    print("    stream, fails only on replacement parts' -- and the second")
    print("    half of that sentence is itself wrong. See below.")
    print()
    print("    AND THE PARTS ARE NOT THE GAP THIS FILE FIRST CALLED THEM.")
    print("    A part is DESIGNED TO SPEC and built to meet the mathematics.")
    print("    It is the OUTPUT of the engineering phase, not an input the")
    print("    plant needs, and the criterion asks whether the thing needs")
    print("    FEEDING. A specification is not a feed. Saying 'fails on")
    print("    parts' treated the next phase's deliverable as this phase's")
    print("    shortfall, which is a category error and is corrected here.")
    print()
    print("    WHAT THE QUESTION SURVIVES AS IS SHARPER AND ANSWERABLE: A")
    print("    PART IS MADE OF SOMETHING. So the test is not 'is it a part',")
    print("    it is WHAT MATERIALS DOES THE PARTS STREAM CONSUME AND ARE")
    print("    THEY ORDINARY.")
    print()
    for item, mats, ordinary in PARTS_MATERIALS:
        mark = "ordinary  " if ordinary else "NOT ordinary"
        print(f"      {mark}  {item}")
        _w = mats
        while _w:
            _cut = _w[:58] if len(_w) > 58 else _w
            if len(_w) > 58:
                _cut = _w[:_w.rfind(" ", 0, 58)]
            print(f"                    {_cut}")
            _w = _w[len(_cut):].lstrip()
    print()
    print("    Steel, nickel alloy, graphite and electrode stock are")
    print("    ORDINARY, and no criterion about a plant's self-sufficiency")
    print("    was written to exclude the existence of industry. The")
    print("    charcoal is regenerated by warming rather than consumed.")
    print()
    print("    ONE MATERIAL IN THE STREAM IS NOT ORDINARY, AND IT HIDES IN")
    print("    THE PROCESS RATHER THAN IN THE PART. Fission products live in")
    print("    the salt as chlorides and leave as chlorides, and every atom")
    print("    of that chlorine is Cl-37 enriched -- the second long-lead")
    print("    purchase in the whole plant.")
    print()
    print(f"      bound on Cl-37 leaving   {parts_chlorine_kg_per_year():.0f}"
          " kg/yr")
    print(f"      against an inventory of  {chlorine_inventory_t():.0f} t")
    print("        -- and that inventory is the row as it stands, which")
    print("           fuelchoice.py found UNDERSTATES it: the row counts the")
    print("           NaCl portion and misses the three Cl per U in UCl3, so")
    print("           the true holding is nearer 219 t. Using the smaller")
    print("           figure makes the share below an OVER-estimate, which is")
    print("           the direction a bound should err in, and the finding is")
    print("           recorded there rather than repaired here.")
    print(f"      which is                 "
          f"{100.0 * parts_chlorine_share_per_year():.3f} % a year,"
          f" {100.0 * PLANT_LIFE_Y * parts_chlorine_share_per_year():.1f} %"
          " over the life")
    print()
    print("    THAT IS A BOUND AND NOT A PROCESS FIGURE, and the difference")
    print("    is the whole of its honesty. Each fission destroys one PuCl3")
    print("    and frees three chlorines; charge conservation caps what the")
    print("    two fission products can carry out near what the parent held.")
    print("    The actual figure needs their valence distribution and the")
    print("    process holdup, and this file computes neither.")
    print()
    print("    AND THE ELECTRICAL ROUTE ALREADY RETURNS IT, which is the")
    print("    second reason to have specified it. Electrowinning deposits")
    print("    the lanthanide as METAL at the cathode and evolves chlorine")
    print("    at the anode, which goes back to the salt; distillation")
    print("    returns the salt with its chlorine in it. A CHLORIDE waste")
    print("    form carries the chlorine out of the building. An OXIDE waste")
    print("    form, or metal, leaves it behind.")
    print()
    print("      REQUIREMENT 4   the waste form shall return chlorine to the")
    print(f"                      salt. Unmet, it is a"
          f" {parts_chlorine_kg_per_year():.0f} kg/yr Cl-37 line and")
    print(f"                      {100.0 * PLANT_LIFE_Y * parts_chlorine_share_per_year():.0f}"
          " % of the inventory over the life. Met, criterion 3")
    print("                      closes on materials with nothing left over.")
    print()
    print("    THAT IS EXACTLY THE KIND OF THING THE ENGINEERING-MATERIALS")
    print("    PHASE SETTLES, and stating it as a requirement is what this")
    print("    phase owes that one. The residue is not a gap in the design;")
    print("    it is a line in the next specification.")
    print()
    ...
    print("      PARTS               ordinary materials, specified downstream.")
    print("      THE ONE CONDITION    Requirement 4, on the waste form.")
    print()
    print("    Read literally -- no input of ANYTHING -- the criterion still")
    print("    cannot be met by any physical object, because a machine that")
    print("    never needs a spare part is not a machine. Read as it was")
    print("    plainly meant -- no fuel, no feedstock, no consumable -- IT IS")
    print("    MET, CONDITIONAL ON REQUIREMENT 4. That is a stronger")
    print("    statement than this file made a pass ago and it is earned by")
    print("    arithmetic rather than by wording: the criterion was not")
    print("    reworded, the design was changed and then the residue was")
    print("    computed instead of shrugged at.")
    print()
...
def selftest():
    ...
    # THE PARTS RESIDUE, and the correction to this file's own verdict: a
    # part is designed to spec and is the next phase's OUTPUT, not this
    # phase's shortfall. What survives is what the parts stream CONSUMES.
    check("the report corrects 'fails on parts' as a category error",
          "category error" in _c3)
    check("every parts material is graded ordinary or not",
          all(isinstance(o, bool) for _i, _m, o in PARTS_MATERIALS))
    check("  -- and exactly one is NOT ordinary",
          sum(1 for _i, _m, o in PARTS_MATERIALS if not o) == 1)
    check("  -- and it is the chlorine",
          all("Cl-37" in m for _i, m, o in PARTS_MATERIALS if not o))
    # THE BOUND, and that it is stated as one.
    check("the chlorine bound is a real fraction of the inventory",
          0.0 < parts_chlorine_share_per_year() < 0.02)
    check("  -- and over the life it is material rather than a rounding",
          PLANT_LIFE_Y * parts_chlorine_share_per_year() > 0.05)
    check("  -- and it scales with the burn-up, so it is a computation",
          abs(parts_chlorine_kg_per_year()
              / (burnup_kg_per_year() * 1000.0 / HM_MOLAR_G
                 * CL_PER_HEAVY_ATOM * CL37_MOLAR_G / 1000.0) - 1.0) < 1e-9)
    check("the report calls it a bound and not a process figure",
          "A BOUND AND NOT A PROCESS FIGURE" in _c3)
    check("  -- and says what it would take to compute the real one",
          "valence distribution" in _c3)
    check("  -- and notes the inventory row it divides by is understated",
          "UNDERSTATES" in _c3)
    check("the residue is stated as a requirement on the waste form",
          "REQUIREMENT 4" in _c3 and "waste form shall return chlorine" in _c3)
    check("  -- and it is handed to the next phase rather than left open",
          "ENGINEERING-MATERIALS" in _c3)
