...
def render(g):
    ...
    a("sixteen hours of store cannot move June into December: the store empties on winter nights")
    ...
    a("water route returns year-round through pump-turbines (the author's decision, 2026-09-11), the reservoir holding")
    a("weeks rather than a season. **The author chose both** (`both.py`): the water returns half the shortfall, the plant the rest.")
    a("")
    ...
    a("| water alone (Title III) | field and store at design; pump-turbines on the water | no feasible point | — | no feasible point | — | — |")
    ...
    a("")
    a(f"Water alone is not a route on this load: with the field and store at design, no point both closes")
    a("and lifts its water inside the plant's own surplus, because the block that closes the evening eats the")
    a("spill the lift runs on. Water with a larger field is, and that is the adopted route: it costs")
    a(f"{bm['title1_b'] / m['mid']['cost_b']:.2f}× / {bc['title1_b'] / m['critical']['cost_b']:.2f}× the mirrors on Title I's account, and with the water withheld it leaves {bm['no_water']['unserved']:.0%} to the")
    a(f"grid and with the field at design {bm['no_field']['unserved']:.0%}, against {B.scan('mid')['as_sized']['unserved']:.0%} with neither — two levers that fail differently.")
    a(f"It makes **{bm['maf']:.2f} to {bc['maf']:.2f} million acre-feet a year** of water the mirrors do not, delivered year-round.")
    a("Title II's side of it, priced by `aquacost.py` with the water's")
