...
hdr = [i+1 for i in range(9171,9180) if MAIN[i].startswith('| ') and 'recomputed' in MAIN[i]]
sep = [i+1 for i in range(9171,9180) if re.match(r'^\|[\s:|-]+$', MAIN[i])]
data = [i+1 for i in range(9171,9180) if MAIN[i].startswith('| ') and i+1 not in hdr]
...
print('  L9222 "This chapter was written as Chapter 31" -- the enclosing chapter of L9222 is:',
      max([c for c in body_ch if max(body_ch[c]) <= 9222], default=None))
