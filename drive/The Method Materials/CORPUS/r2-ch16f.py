...
print('      §18.5 body L5294-L5302 carries no forward notice: "corrects" sites in it = %d'
      % len([i for i in range(5294, 5303) if re.search(r'correct', M[i - 1], re.I)]))
...
seg = [t for t in M[8587:8592] if t.strip()]
for i, t in enumerate(seg, 8588):
...
rows = [t for t in M[8598:8605] if t.strip()]
for i, t in enumerate(M[8598:8605], 8599):
    if t.strip(): print('      L%d %s' % (i, t.strip()[:100]))
print('      row sections: %s' % re.findall(r'§(\d+(?:\.\d+)*)', '\n'.join(rows)))
print('      chapter of the sentence: enclosing(L8597) = %s' % (enclosing(M, 8597),))
...
for ln, word in ((8584, 'eight'), (8585, '18'), (8585, 'seven'), (8623, '18 of 18'),
                 (8633, 'six planets'), (8662, 'Three objects'), (8683, 'three conditions')):
...
for i in range(7630, 7646):
    if M[i - 1].strip():
        print('      main L%d %s' % (i, M[i - 1].strip()[:110]))
for i in range(7550, 7575):
