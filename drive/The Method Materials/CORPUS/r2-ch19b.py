...
print('  whole-line italics:', ul(r'^\s*\*[^*].*\*\s*$'), '; markdown table rows (a | at line start outside 4-space code lines):', [i for i in ul(r'^\s*\|') if not M[i - 1].startswith('    ')], '; code lines (4-space):', ul(r'^    \S'), '(A.19\'s rank table %d lines, A.19.0\'s displayed definition %d lines, A.19.1\'s two rows)' % (len([i for i in ul(r'^    \S') if i < 10113]), len([i for i in ul(r'^    \*\*') if 10113 < i < 10153])))
...
adm = [(i, L.enclosing(M, i), norm(M[i - 1])[:150]) for i, l in enumerate(M, 1) if re.search(r'admissib', l, re.I) and i < 9937]
...
mono = [(i, L.enclosing(M, i), norm(M[i - 1])[:150]) for i, l in enumerate(M, 1) if re.search(r'monotone', l) and re.search(r'admiss|both directions|single|one coordinate|in each coordinate|upper|lower', l, re.I) and i < 9937]
