...
# Faults self-caught before banking: (1) an unescaped % in a format string; (2) *tightest* as a bare word is not a test — the figure is 1.40 cm⁻¹; (3) the DATA-row convention below is the rewrite.
...
# table's DATA rows are the lines of the whitespace-aligned table that begin at the conclusion column AND carry a ≥3-space
# column gap (a bare continuation word at that column, or a deeper-indented line, is joined to the row above) — the DATA-row set is
...
while M[k - 1].strip():
    ...
    if ind <= col + 1 and re.search(r'\S {3,}\S', l): tab.append([k, l])   # fault 3 rewrite: a DATA row carries a ≥3-space column gap; a bare continuation word (L10244 *sequence*) does not
...
show('tightest 1.40 cm⁻¹ / 1.4 cm⁻¹ shift', r'1\.40? cm', ctx=120)
print('    *tightest* within 40 chars of *bound*:', out(r'tightest.{0,40}bound|bound.{0,40}tightest'))
