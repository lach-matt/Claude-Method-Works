...
A, B = 9716, 9805
assert heading_line(M, '35') == 9716 and body_range(M, '35.3')[1] == 9806 and heading_line(M, '35.4') == 9806
...
u5 = re.findall(r'[+−-]\d+\.\d+', ' '.join(U[9766 - A:9770 - A])); r1705 = rbody(1705)
...
print('  unit values %s; register 1705 point values %s; equal: %s; all positive: %s; Z list %s'
      % (u5, r5, [x.replace('−', '-') for x in u5] == [x.replace('−', '-') for x in r5], all(x.startswith('+') for x in u5), re.findall(r'Z = ([\d, ]+)\)', ' '.join(U[9766 - A:9770 - A]))))
