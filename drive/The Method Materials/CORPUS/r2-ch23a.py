...
join = ' '.join(l.strip() for l in M[10319:10323])   # the serving line wraps L10321–L10322; read on the whitespace-normalised join
print('   the serving line (join L10320–L10323) prints', re.findall(r'seventy-seven elements[^.;]*?twenty-four fibres', join)[:1], '; D.5.1 L10525 sums to twenty-seven (arithmetic):', 1+1+1+2+3+1+5+3+2+1+3+2+2)
...
print('   singleton fibres of the twenty-seven (count = 1):', sorted(k for k, v in Counter((x[2], x[3]) for x in elems).items() if v == 1), '; the nine listed at L10545–L10553 omit definition · order (a closed index, §14.1) — the second clause of L10540–L10541 is a judgement, not a count')
...
ROW = re.compile(r'^\s*(.+?)\s{2,}(definition|mechanism|formula|theorem|law|method|measurement) · (order|combinatorics|analysis|complexity|physics|algebraic geometry)\s{2,}(.+?)\s*$')
for sec in ('D.5.6', 'D.5.7', 'D.5.8', 'D.5.9', 'D.5.10'):
    # a row here can carry a 2-space gap before its coordinates when the fibre column is long (witnessed below): the row is
    # parsed on its own grammar — element ⟨gap⟩ kind · language ⟨gap⟩ status · verification · precedent — not on the ≥3-space rule
    lo, hi = drange(sec); rr = []; narrow = []
    for i in range(lo + 1, hi):
        m = ROW.match(M[i - 1])
        if m and m.group(1).strip() not in ('new element', 'element'):
            rr.append((i, m)); gaps = [len(g) for g in re.findall(r' {2,}', M[i - 1].strip())]
            if min(gaps) < 3: narrow.append(i)
    for i, m in rr: elems.append((sec, m.group(1).strip(), m.group(2), m.group(3), coords(m.group(4)), i))
    print('   %s table: DATA rows %d ; rows with a column gap < 3 spaces (chat-134 rule would drop them): %s' % (sec, len(rr), narrow))
...
def perkind(els):
    g = defaultdict(list); [g[x[2]].append(x[4]) for x in els]; return {k: E_of(v, boxes(v, False)['A']) for k, v in g.items() if E_of(v, boxes(v, False)['A'])}
print('   kind-only excess by kind, A.5 sampled:', perkind(cum['D.5.4']), '; A.5 exhaustive:', perkind(X32e), '(printed kind only E = 2; Register 233 the same)')
...
for a, b in itertools.product([(2, 2, 1), (2, 1, 1), (2, 2, 0), (2, 1, 0)], repeat=2):
    E = fibreE(swap(swap(base65, '3B.five', a), '3B.pot', b))
    if sum(E.values()) == 4: print('   D.5.9 first run E = 4 reproduces with 3B.five', a, '3B.pot', b, dict(E))
print('   D.5.9 first-run search over 16 (3B.five, 3B.pot) coordinate pairs with status measured: pairs giving E = 4 printed above (none printed = none)')
