...
for n in WARN:
    for tag, i in CIT[n]:
        j = join2(dict(VOL)[tag], i); corr = [c for _, _, cs in WARN[n] for c in cs]
        byvol[(tag, 'CARRIES' if (re.search(QUAL, j, re.I) or any(re.search(r'(?<!\d)%d(?!\d)' % c, j) for c in corr)) else 'SILENT')] += 1
print('\nTally by volume:', dict(sorted(byvol.items())))
print('A-CLASS (SILENT citers OUTSIDE the Register — the reader-facing restatements):', [(n, tag, i) for n in sorted(WARN) for tag, i in CIT[n] if tag != 'reg' and not (re.search(QUAL, join2(dict(VOL)[tag], i), re.I) or any(re.search(r'(?<!\d)%d(?!\d)' % c, join2(dict(VOL)[tag], i)) for _, _, cs in WARN[n] for c in cs))])
for n, tag, i in [(n, tag, i) for n in sorted(WARN) for tag, i in CIT[n] if tag != 'reg']:
    print('  outside-Register citer %s L%d cites (phrase set ∩ WARNING\'d):' % (tag, i), sorted(cited(dict(VOL)[tag][i - 1])[0] & set(WARN)))
...
for tag, i, a, b in WIDE: print('   %s L%d %d–%d spans WARNING\'d entries %s and absent-under-both %s' % (tag, i, a, b, [n for n in sorted(WARN) if a <= n <= b], [n for n in absentAB if a <= n <= b]))
